...
test.describe("Phase F4 — CRUD lifecycle parity (real browser)", () => {
  test("Category: create → appears in list → edit → delete (full lifecycle)", async ({
    page,
  }) => {
  ...
  test("Recipient: create → appears → re-renders without errors", async ({
    page,
  }) => {
    await page.goto("/recipients");
    await expect(
      page.getByRole("heading", { level: 1, name: /recipients/i }),
    ).toBeVisible();
  ...
  test("Planned payment: create → appears in table", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { level: 1, name: /planned payments/i }),
    ).toBeVisible();

    const unique = `F4Pln_${Date.now()}`;
    const account = `F4Account_${Date.now()}`;

    await page.getByRole("button", { name: /new payment/i }).click();
    const dialog = page.getByRole("dialog", { name: /new planned payment/i });
    await expect(dialog).toBeVisible();
    await dialog.getByLabel(/^name \*/i).fill(unique);
    ...
    await dialog.getByLabel(/^bank account \*/i).click();
    ...
    await dialog.getByRole("button", { name: /^create payment$/i }).click();
...
test.describe("Phase F4 — Mutation invalidation parity", () => {
  test("After creating a recipient, navigating away and back still shows it", async ({
    page,
  }) => {
    await page.goto("/recipients");
    await expect(
