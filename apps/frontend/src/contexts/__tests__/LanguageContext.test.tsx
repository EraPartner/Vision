...
describe("LanguageContext", () => {
    ...
    it("interpolates variables in translation strings once locale loads", async () => {
        ...
        await waitFor(() => {
            expect(result.current.t("addInv.assetTitle", { assetClass: "Stock" })).toBe("Add Stock");
        });
    });
    ...
    it("returns nl translation when language is nl", async () => {
        const { result } = renderHook(() => useLanguage(), { wrapper: makeWrapper("nl") });
        await waitFor(() => {
    ...
    it("switching back to en after nl restores en translations", async () => {
        ...
        const { result } = renderHook(() => useLanguage(), { wrapper: CaptureWrapper });
        await waitFor(() => expect(result.current.t("addInv.back")).toBe("Back"));
        act(() => outerSet?.("nl"));
        await waitFor(() => expect(result.current.t("addInv.back")).toBe("Terug"));
        act(() => outerSet?.("en"));
        await waitFor(() => expect(result.current.t("addInv.back")).toBe("Back"));
    });
});
