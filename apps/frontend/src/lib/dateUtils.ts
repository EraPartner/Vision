...
export function formatDate(
    ...
    switch (pattern) {
        ...
        case "d MMM":
            return `${date.getDate()} ${MMM}`;
        case "d MMM yy":
            return `${date.getDate()} ${MMM} ${String(y).slice(-2)}`;
        case "d MMM yyyy":
            return `${date.getDate()} ${MMM} ${y}`;
        case "dd MMM yyyy":
            return `${String(date.getDate()).padStart(2, "0")} ${MMM} ${y}`;
        case "MMM d":
            return `${MMM} ${date.getDate()}`;
        default:
...
export function formatDistanceToNow(
    date: Date,
    _options?: { addSuffix?: boolean; locale?: string },
): string {
    const diffMs = date.getTime() - Date.now();
    const absMs = Math.abs(diffMs);
    const rtf = new Intl.RelativeTimeFormat(_options?.locale ?? "en", {
        numeric: "auto",
    });
    ...
    if (absMs < 86_400_000 * 30)
        return rtf.format(Math.round(diffMs / 86_400_000), "day");
    if (absMs < 86_400_000 * 365)
        return rtf.format(Math.round(diffMs / (86_400_000 * 30.44)), "month");
...
/**
 * Strictly parse text written in the configured app date format.
 *
 * date-fns rejects many invalid dates, but the exact format round-trip is the
 * important second guard: it rejects rollover, missing zero padding, trailing
 * text, and a value written in one of the other supported formats. The result
 * is normalized to local midnight so `toYmd` preserves the entered calendar
 * day in every timezone.
 */
export function parseAppDateInput(
    input: string,
    ...
    const pattern = appDateFormatToDateFnsPattern(appDateFormat);
    const parsed = dateFnsParse(trimmed, pattern, new Date(2000, 0, 1));
    if (!dateFnsIsValid(parsed) || dateFnsFormat(parsed, pattern) !== trimmed) {
        return undefined;
    }
...
export function formatDateWithAppSettings(
    date: Date,
    appDateFormat: string,
): string {
...
export function formatDateStringWithAppSettings(
    ...
    try {
        ...
        if (/^\d{4}-\d{2}-\d{2}$/.test(ymdPart)) {
            return formatDateWithAppSettings(
...
export function formatDateTimeWithAppSettings(
    ...
    return `${formatDateWithAppSettings(date, appDateFormat)} ${date.toLocaleTimeString(
        locale,
        {
            hour: "2-digit",
            minute: "2-digit",
...
export function formatDateTimeStringWithAppSettings(
    ...
    // A timestamp represents one instant. Format both its calendar date and
    // clock time in the local zone; combining the source UTC date with a local
    // clock can otherwise produce an impossible pair around midnight.
    if (dateStr.includes("T")) {
...
export function weekStartsOnFromSetting(
