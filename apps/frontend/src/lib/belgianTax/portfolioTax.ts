...
function recordedForYear(
    ...
    for (const txn of inv.transactions) {
        ...
        if (txn.type === txnType)
            terms.push(convert(Number(txn.amount) || 0, txn.currency));
...
export function computeTobAutoEstimate(
    ...
    for (const inv of investments) {
        ...
        for (const txn of inv.transactions) {
            ...
            legs.push(
                Decimal.min(new Decimal(amount).times(params.rate), params.cap),
            );
        }
    }
...
export function computeTacrEstimate(
    ...
    const aggregate = sumDecimal(
        investments.map((inv) =>
            convert(Number(inv.currentValue) || 0, inv.currency),
        ),
    );
...
export function computeRealizedGainSplit(
    ...
    for (const inv of investments) {
        ...
        const subjectToReynders =
            override === undefined ? inv.assetClass === "bond" : override;
        if (subjectToReynders) {
            const portionRaw = inv.reyndersInterestPortion;
            const portion =
                typeof portionRaw === "number" &&
                portionRaw >= 0 &&
                portionRaw <= 1
                    ? portionRaw
                    : 1;
    ...
    return {
        reyndersInterest: reyndersInterest.toNumber(),
        cgtGains: cgtGains.toNumber(),
    };
}
...
export function computeReyndersEstimate(
    ...
    return new Decimal(split.reyndersInterest)
        .times(taxTable.reyndersTaxRate)
        .toNumber();
}
...
export function computeCgtEstimate(
    ...
    const taxable = Decimal.max(
        new Decimal(split.cgtGains).minus(exemption),
        0,
    );
...
export function computeDividendWht(
    ...
    for (const inv of investments) {
        for (const txn of inv.transactions) {
            ...
            incomeTerms.push(amount);
            whtTerms.push(taxes);
            if (txn.dividend_amount_convention === "gross")
                grossTerms.push(amount);
            else if (txn.dividend_amount_convention === "net")
                grossTerms.push(amount + taxes);
            else unknownDividendConventionCount += 1;
        }
    }
    const totalDividendIncome = sumDecimal(incomeTerms);
    const dividendWhtRecorded = sumDecimal(whtTerms);

    const grossDividendBase =
        unknownDividendConventionCount > 0 ? null : sumDecimal(grossTerms);
    ...
    const reclaimCap =
        grossDividendBase !== null
            ? Decimal.min(grossDividendBase, exemption).times(
                  taxTable.dividendWHTRate,
              )
            : null;
    const dividendWhtReclaim = reclaimCap
        ? Decimal.min(dividendWhtRecorded, reclaimCap)
        : null;
    const dividendWhtNetCost = dividendWhtReclaim
        ? Decimal.max(dividendWhtRecorded.minus(dividendWhtReclaim), 0)
        : null;

    return {
        totalDividendIncome: totalDividendIncome.toNumber(),
        dividendWhtRecorded: dividendWhtRecorded.toNumber(),
        grossDividendBase: grossDividendBase?.toNumber() ?? null,
        grossDividendWht: dividendWhtRecorded.toNumber(),
        dividendWhtReclaim: dividendWhtReclaim?.toNumber() ?? null,
        dividendWhtNetCost: dividendWhtNetCost?.toNumber() ?? null,
