...
const convert: ConvertFn = (amount, currency) => (currency === 'USD' ? amount * 0.9 : amount);
...
const investments: PortfolioTaxInvestment[] = [
  {
    ...
    currentValue: 12_000,
    transactions: [
      { type: 'buy', date: '2025-03-01', amount: 10_000, taxes: 35, fees: 5 },
      { type: 'dividend', date: '2025-06-01', amount: 200, taxes: 60 },
  ...
  },
  {
    ...
    realizedGain: 0,
    currentValue: 5_000,
    transactions: [{ type: 'buy', date: '2025-02-01', amount: 5_000, taxes: 0, fees: 2 }],
  },
  {
    id: 3,
    assetClass: 'bond',
    ...
    realizedGain: 300,
    currentValue: 8_000,
    transactions: [{ type: 'buy', date: '2025-01-15', amount: 8_000, taxes: 9.6 }],
  },
  {
    id: 4,
    assetClass: 'stock',
    ...
    transactions: [
      { type: 'buy', date: '2024-12-01', amount: 4_000, taxes: 14, currency: 'USD' }, // prior year — excluded
      { type: 'dividend', date: '2025-05-01', amount: 100, taxes: 15, currency: 'USD' }, // → 90 / 13.5 EUR
    ],
  },
];
...
describe('recorded taxes/fees for a year', () => {
  it('sums per-txn taxes + explicit tax txns, scoped to the year', () => {
    expect(recordedTaxesForYear(investments[0], 2025, convert)).toBeCloseTo(95, 8); // 35 + 60 + 0
    expect(recordedFeesForYear(investments[0], 2025, convert)).toBeCloseTo(5, 8);
    // inv 4: 2024 buy excluded; only the 2025 dividend's 15 USD → 13.5 EUR counts.
    expect(recordedTaxesForYear(investments[3], 2025, convert)).toBeCloseTo(13.5, 8);
  });

  it('enriches with manual adjustments', () => {
    const costs = enrichInvestmentCosts(investments[0], 2025, convert, { taxes: 10, fees: 5 });
...
describe('TOB', () => {
  it('sums recorded buy-leg taxes for the year', () => {
    ...
    expect(computeTobRecorded(investments, 2025, convert)).toBeCloseTo(44.6, 8);
  });
  ...
  it('estimates per-leg TOB by asset class with the per-leg cap', () => {
    ...
    expect(computeTobAutoEstimate(investments, 2025, table2025, convert)).toBeCloseTo(110.6, 8);
  });
});
...
describe('TACR', () => {
  ...
  it('applies the rate once aggregate value crosses the threshold', () => {
    const big: PortfolioTaxInvestment[] = [
      { id: 9, assetClass: 'stock', currency: 'EUR', currentValue: 2_000_000, transactions: [] },
    ];
    expect(computeTacrEstimate(big, table2025, convert)).toBeCloseTo(3_000, 8); // 2_000_000 * 0.0015
  });
});
...
describe('realized-gain split + Reynders + CGT', () => {
  ...
  it('taxes CGT-pool gains above the exemption when active (2026+)', () => {
    const fixture: PortfolioTaxInvestment[] = [
      { id: 10, assetClass: 'stock', currency: 'EUR', realizedGain: 50_000, currentValue: 60_000, transactions: [] },
    ];
    ...
    expect(computeCgtEstimate(split, table2026, 'single', true)).toBeCloseTo(4_000, 8);
    // married: (50000 - 20000) * 0.10
    expect(computeCgtEstimate(split, table2026, 'married_joint', true)).toBeCloseTo(3_000, 8);
  });

  it('honours an explicit subjectToReynders=false override on a bond (stays out of Reynders)', () => {
    const fixture: PortfolioTaxInvestment[] = [
      { id: 11, assetClass: 'bond', currency: 'EUR', realizedGain: 1_000, subjectToReynders: false, currentValue: 0, transactions: [] },
    ];
    // pre-2026: direct bond under normal management → neither pool
    expect(computeRealizedGainSplit(fixture, convert, false)).toEqual({ reyndersInterest: 0, cgtGains: 0 });
    // 2026+: direct bond enters CGT scope
    expect(computeRealizedGainSplit(fixture, convert, true)).toEqual({ reyndersInterest: 0, cgtGains: 1_000 });
  });
});
