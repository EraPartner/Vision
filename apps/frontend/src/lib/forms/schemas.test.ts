...
import {
    currencyCode,
    fieldErrorsFromZod,
    moneyAmount as createMoneyAmount,
    requiredString,
    requiredTrimmedString,
    ymdDateString,
} from "./schemas";

const moneyAmount = (keys: Parameters<typeof createMoneyAmount>[0]) =>
    createMoneyAmount(keys, "eu");
...
describe("moneyAmount", () => {
    ...
    test("parses the selected locale and rejects a foreign convention", () => {
        ...
        expect(createMoneyAmount(keys, "us").parse("1,234.56")).toBe(1234.56);
    });
    ...
    test("non-finite results (overflowing exponent) are invalid, not silently null", () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success)
            expect(result.error.issues[0].message).toBe("k.invalid");
    });
});
...
describe("fieldErrorsFromZod", () => {
    ...
    test("returns an empty map for no error, and ignores unmapped paths", () => {
        ...
        const other = z
            .object({ note: requiredString("validation.required") })
            .safeParse({ note: "" });
