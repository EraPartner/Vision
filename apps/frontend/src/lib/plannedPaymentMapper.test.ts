...
import {
    mapFromAPI as mapFromAPIWithCurrency,
    mapToCreateAPI,
    mapToUpdateAPI,
    type PlannedPayment,
...
const mapFromAPI = (payment: PlannedTransaction) =>
    mapFromAPIWithCurrency(payment, "EUR");
...
describe("mapFromAPI", () => {
    ...
    it("falls back name to recipient_name, then to 'Unnamed payment'", () => {
        ...
        expect(
            mapFromAPI(makePT({ memo: undefined, recipient_name: undefined }))
                .name,
        ).toBe("Unnamed payment");
    });

    it("falls back a missing currency to the explicit app default", () => {
        expect(
            mapFromAPIWithCurrency(makePT({ currency: undefined }), "CHF")
                .currency,
        ).toBe("CHF");
    });
    ...
    it("strips the time portion from an ISO datetime planned_date", () => {
        const p = mapFromAPI(
            makePT({ planned_date: "2025-02-01T00:00:00.000Z" }),
        );
    ...
    it.each([
        ...
        "parses recurrence_pattern %s as frequency %s",
        (pattern, frequency) => {
    ...
    it("defaults an unrecognized non-numeric pattern to monthly", () => {
        expect(
            mapFromAPI(makePT({ recurrence_pattern: "fortnightly-ish" }))
                .frequency,
        ).toBe("monthly");
    });
    ...
    it("leaves frequency undefined for non-recurring rows even if a pattern is present", () => {
        const p = mapFromAPI(
            makePT({ is_recurring: false, recurrence_pattern: "monthly" }),
        );
    ...
    it("maps null recurrence bounds to undefined and keeps set ones", () => {
        const cleared = mapFromAPI(
            makePT({ recurrence_end_date: null, max_occurrences: null }),
        );
        expect(cleared.end_date).toBeUndefined();
        expect(cleared.max_occurrences).toBeUndefined();

        const bounded = mapFromAPI(
            makePT({ recurrence_end_date: "2026-12-31", max_occurrences: 12 }),
        );
    ...
    it("flattens tags to slugs and defaults executions/tags/count", () => {
        ...
        const bare = mapFromAPI(
            makePT({ tags: undefined, executions: undefined }),
        );
...
describe("mapToUpdateAPI", () => {
    ...
    it("forwards cleared recurrence bounds as explicit null, set ones verbatim", () => {
        expect(
            mapToUpdateAPI({ end_date: null, max_occurrences: null }),
        ).toEqual({
            recurrence_end_date: null,
            max_occurrences: null,
        });
        expect(
            mapToUpdateAPI({ end_date: "2026-12-31", max_occurrences: 12 }),
        ).toEqual({
            recurrence_end_date: "2026-12-31",
            max_occurrences: 12,
        });
        // undefined means "leave unchanged" — the keys must not appear at all.
        expect(mapToUpdateAPI({ amount: 1 })).not.toHaveProperty(
            "recurrence_end_date",
        );
        expect(mapToUpdateAPI({ amount: 1 })).not.toHaveProperty(
            "max_occurrences",
    ...
    it("rebuilds recurrence_pattern from frequency updates", () => {
        expect(mapToUpdateAPI({ frequency: "weekly" }).recurrence_pattern).toBe(
            "weekly",
        );
        expect(
            mapToUpdateAPI({ frequency: "custom", custom_interval_days: 10 })
                .recurrence_pattern,
        ).toBe("every 10 days");
