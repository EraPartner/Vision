...
describe("electron capability detection (absent branch)", () => {
    ...
    it("native UI setters are safe no-ops outside Electron", () => {
        ...
        expect(() => setNativeVibrancy(true)).not.toThrow();
        expect(() =>
            persistSplashTheme({
                background: "0 0% 0%",
                foreground: "0 0% 100%",
...
describe("electron capability detection (present branch)", () => {
    beforeEach(() => {
        ...
        win.electronBackup = {
            runBackup: vi
                .fn()
                .mockResolvedValue({ success: true, file: "/b.visionbak" }),
        ...
        win.electronAPI = {
            ...
            setVibrancy: vi.fn().mockResolvedValue({ success: true }),
    ...
    it("native UI setters call through to the bridge", () => {
        ...
        setNativeVibrancy(true);
        persistSplashTheme({ background: "x", foreground: "y" });
        expect(win.electronAPI).toBeTruthy();
        expect(
            (win.electronAPI as { setLanguage: ReturnType<typeof vi.fn> })
                .setLanguage,
        ).toHaveBeenCalledWith("nl");
        expect(
            (win.electronAPI as { setVibrancy: ReturnType<typeof vi.fn> })
                .setVibrancy,
    ...
    it("backup helpers delegate to the backup bridge", async () => {
        expect(await runBackup("/dest", "{}")).toMatchObject({
            file: "/b.visionbak",
        });
        expect(await selectBackupFile()).toBe("/picked");
        expect(await restoreBackup("/f", { passphrase: "pw" })).toMatchObject({
            success: true,
        });
        expect(await isBackupEncrypted("/f")).toBe(true);
        expect(await selectBackupDir()).toBe("/dir");
        expect(await getBackupEncryptionStatus()).toMatchObject({
            secureStorageAvailable: true,
        });
        expect(await setBackupPassphrase("pw")).toMatchObject({
            available: true,
        });
    });
