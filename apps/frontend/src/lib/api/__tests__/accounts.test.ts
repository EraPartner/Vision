...
import {
    ...
    updateAccount,
    deleteAccount,
    getAccountPortfolioLotRetagPreview,
    mergeAccounts,
    setOpeningBalance,
    reconcileAccount,
...
describe("accounts API client", () => {
    ...
    it("loads the complete close portfolio preview by account id", async () => {
        server.use(
            http.get(
                `${API_BASE}/api/accounts/9/portfolio-lot-retag-preview`,
                () =>
                    ok({
                        account_id: 9,
                        eligible_count: 2,
        ...
        await expect(getAccountPortfolioLotRetagPreview(9)).resolves.toEqual({
            account_id: 9,
            eligible_count: 2,
            transaction_ids: [11, 12],
            limit: 500,
    ...
    it("setOpeningBalance posts the anchor payload and returns the transaction + warning", async () => {
        ...
        server.use(
            http.post(
                `${API_BASE}/api/accounts/4/opening-balance`,
                async ({ request }) => {
                    body = await request.json();
                    return ok({
                        transaction: {
        ...
        const result = await setOpeningBalance(4, {
            balance: 1500,
            date: "2024-01-01",
        });
    ...
    it("reconcileAccount posts the mode and returns the reconciled figures", async () => {
        ...
        server.use(
            http.post(
                `${API_BASE}/api/accounts/4/reconcile`,
                async ({ request }) => {
                    body = await request.json();
                    return ok({
                        mode: "adjustment",
