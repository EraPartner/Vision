...
describe("createWithStatus", () => {
    it("reports wasCreated=true on 201 and unwraps the envelope data", async () => {
        ...
        const res = await createWithStatus<{ x: number }, { id: number }>(
            "/api/cw-test",
            { x: 1 },
        );
...
describe("blob export endpoints", () => {
    it("bulkExportTransactions posts the request and returns a Blob", async () => {
        ...
        server.use(
            http.post(
                `${API_BASE}/api/transactions/bulk-export`,
                async ({ request }) => {
        ...
        const result = await bulkExportTransactions({
            ids: [1, 2],
            format: "csv",
        } as never);
