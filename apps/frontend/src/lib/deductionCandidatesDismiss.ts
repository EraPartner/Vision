 ...
 *
 * Dismissals are a per-browser preference, so they live in localStorage. A dismissal is keyed
...
export const DISMISSED_DEDUCTION_CANDIDATES_STORAGE_KEY =
    "dismissed_deduction_candidates_v1";
...
export function isCandidateDismissed(
    ...
    return state.some(
        (d) => d.year === year && d.deductionType === deductionType,
    );
}
