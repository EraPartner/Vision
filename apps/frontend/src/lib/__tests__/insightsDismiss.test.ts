...
function makeForecast(prominence: "alert" | "standing"): CashForecast {
    return {
        month: "2026-07",
        currency: "EUR",
        monthEndNetCashflow: 1200,
        monthEndNetCashflowLow: 900,
        monthEndNetCashflowHigh: 1500,
...
function makeDigest(
    overrides: Partial<InsightsDigestResponse> = {},
): InsightsDigestResponse {
    return {
...
describe("insightsDismiss", () => {
    ...
    describe("subscription dismissals", () => {
        test("dismissal is permanent — still suppressed a year later", () => {
            ...
            expect(
                filterDigest(digest, state, NOW).newSubscriptions,
            ).toHaveLength(0);
            expect(
                filterDigest(digest, state, daysAfter(365)).newSubscriptions,
            ).toHaveLength(0);
        });
        ...
        test("findingTypes are independent — dismissing 'new' keeps the priceChange visible", () => {
            const digest = makeDigest({
                subscriptionCreep: {
                    new: [makeNew(1)],
                    priceChanges: [makePriceChange(1)],
                },
            });
            const state = dismissSubscription(1, "new");
        ...
        test("dismissal only affects the matching recipient and persists to localStorage", () => {
            ...
            expect(filtered.newSubscriptions.map((f) => f.recipientId)).toEqual(
                [2],
            );
        });
    });
    ...
    describe("category-outlier dismissals", () => {
        test("suppressed within the 14-day window", () => {
            ...
            expect(
                filterDigest(
                    digest,
                    state,
                    daysAfter(OUTLIER_SUPPRESSION_DAYS - 1),
                ).categoryOutliers,
            ).toHaveLength(0);
        });
        ...
        test("re-alerts early when deviation worsens past the margin", () => {
            ...
            expect(
                filterDigest(digest, state, daysAfter(1)).categoryOutliers,
            ).toHaveLength(1);
        });
        ...
        test("un-suppresses after the 14-day window elapses", () => {
            ...
            const digest = makeDigest({ categoryOutliers: [outlier] });
            expect(
                filterDigest(digest, state, daysAfter(OUTLIER_SUPPRESSION_DAYS))
                    .categoryOutliers,
            ).toHaveLength(1);
        });
    ...
    describe("countUndismissed", () => {
        ...
        test("dismissals lower the count; undefined digest counts 0", () => {
            const digest = makeDigest({
                subscriptionCreep: {
                    new: [makeNew(1)],
                    priceChanges: [makePriceChange(2)],
                },
                categoryOutliers: [makeOutlier(10)],
            });
            dismissSubscription(1, "new");
