...
import {
    deriveUnitMath,
    parsePositive as parsePositiveWithFormat,
    roundUnitMath,
    UNIT_MATH_AMOUNT_DP,
    UNIT_MATH_UNITS_DP,
    UNIT_MATH_PRICE_DP,
...
const parsePositive = (value: string) => parsePositiveWithFormat(value, "eu");
...
describe("deriveUnitMath", () => {
    ...
    it("still rejects a difference beyond the backend tolerance", () => {
        expect(
            deriveUnitMath({ amount: 100.02, units: 3, price: 33.33 })
                .isConsistent,
        ).toBe(false);
    });
    ...
    it("rounds derived values to the configured precisions", () => {
        expect(deriveUnitMath({ amount: 1, units: 3 }).derivedPrice).toBe(
            roundUnitMath(1 / 3, UNIT_MATH_PRICE_DP),
        );
        expect(deriveUnitMath({ amount: 1, price: 3 }).derivedUnits).toBe(
            roundUnitMath(1 / 3, UNIT_MATH_UNITS_DP),
        );
        expect(deriveUnitMath({ units: 1 / 3, price: 3 }).derivedAmount).toBe(
            roundUnitMath((1 / 3) * 3, UNIT_MATH_AMOUNT_DP),
        );
    });
