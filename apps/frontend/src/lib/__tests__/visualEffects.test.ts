...
describe("resolveEffectiveTier — session override", () => {
    it("replaces the cap while the cap is active", () => {
        expect(resolveEffectiveTier("standard", true, true, "enhanced")).toBe(
            "enhanced",
        );
        expect(resolveEffectiveTier("enhanced", true, true, "standard")).toBe(
            "standard",
        );
    });

    it("lies dormant on a small display — the synced preference governs", () => {
        expect(resolveEffectiveTier("standard", true, false, "enhanced")).toBe(
            "standard",
        );
    });

    it("lies dormant when auto-adapt is off", () => {
        expect(resolveEffectiveTier("standard", false, true, "enhanced")).toBe(
            "standard",
        );
    });

    it("still caps at reduced when absent", () => {
        expect(resolveEffectiveTier("enhanced", true, true, undefined)).toBe(
            "reduced",
        );
    });
});
...
describe("migrateAppSettings", () => {
    ...
    it("fills missing fields from defaults", () => {
        const migrated = migrateAppSettings({ defaultCurrency: "USD" });
