...
describe("useAccounts", () => {
  it("returns accounts on success", async () => {
    vi.spyOn(apiClient, "getAccounts").mockResolvedValue({ items: [{ id: 1 }], total: 1 } as never);
    const { result } = renderHook(() => useAccounts({ active: "all" }), { wrapper: makeFullWrapper() });
  ...
  it("surfaces an error", async () => {
    ...
    const { result } = renderHook(() => useAccounts(), { wrapper: makeFullWrapper() });
...
describe("useCreateAccount", () => {
  it("calls createAccount and toasts success on success", async () => {
    const spy = vi.spyOn(apiClient, "createAccount").mockResolvedValue({ id: 5 } as never);
    const { result } = renderHook(() => useCreateAccount(), { wrapper: makeFullWrapper() });
    await act(async () => {
  ...
  it("toasts an error on failure (error branch)", async () => {
    vi.spyOn(apiClient, "createAccount").mockRejectedValue(new Error("boom"));
    const { result } = renderHook(() => useCreateAccount(), { wrapper: makeFullWrapper() });
    await act(async () => {
...
describe("useUpdateAccount", () => {
  it("calls updateAccount with id + data", async () => {
    const spy = vi.spyOn(apiClient, "updateAccount").mockResolvedValue({ id: 3 } as never);
    const { result } = renderHook(() => useUpdateAccount(), { wrapper: makeFullWrapper() });
    await act(async () => {
  ...
  it("toasts an error on failure", async () => {
    vi.spyOn(apiClient, "updateAccount").mockRejectedValue(new Error("boom"));
    const { result } = renderHook(() => useUpdateAccount(), { wrapper: makeFullWrapper() });
    await act(async () => {
      await result.current.mutateAsync({ id: 3, data: {} as never }).catch(() => {});
    });
    await waitFor(() => expect(toast.error).toHaveBeenCalled());
  });
});
...
describe("useMergeAccounts", () => {
  it("calls mergeAccounts and toasts success", async () => {
    const spy = vi.spyOn(apiClient, "mergeAccounts").mockResolvedValue({ into: 1, merged: [2], reassigned: {} } as never);
  ...
  it("toasts an error on failure", async () => {
    vi.spyOn(apiClient, "mergeAccounts").mockRejectedValue(new Error("boom"));
    const { result } = renderHook(() => useMergeAccounts(), { wrapper: makeFullWrapper() });
    await act(async () => {
      await result.current.mutateAsync({ targetId: 1, sourceIds: [2] }).catch(() => {});
    });
    await waitFor(() => expect(toast.error).toHaveBeenCalled());
  });
...
describe("useDeleteAccount", () => {
  it("calls deleteAccount and toasts success", async () => {
    const spy = vi.spyOn(apiClient, "deleteAccount").mockResolvedValue(undefined as never);
    const { result } = renderHook(() => useDeleteAccount(), { wrapper: makeFullWrapper() });
    await act(async () => {
  ...
  it("toasts an error on failure", async () => {
    vi.spyOn(apiClient, "deleteAccount").mockRejectedValue(new Error("409"));
    const { result } = renderHook(() => useDeleteAccount(), { wrapper: makeFullWrapper() });
    await act(async () => {
...
describe("useTags", () => {
  it("returns tags on success", async () => {
    vi.spyOn(apiClient, "getTags").mockResolvedValue({ items: [{ id: 1, slug: "t" }], total: 1 } as never);
    const { result } = renderHook(() => useTags({ is_active: true }), { wrapper: makeFullWrapper() });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
...
describe("useCreateTag", () => {
  it("calls createTag on success", async () => {
    const spy = vi.spyOn(apiClient, "createTag").mockResolvedValue({ id: 1, slug: "t" } as never);
