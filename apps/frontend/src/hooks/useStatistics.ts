...
type MonthlySummaryPayload = Awaited<
    ReturnType<typeof getAggregationMonthlySummary>
>["data"];
type CategoryPivotPayload = Awaited<
    ReturnType<typeof getAggregationCategoryPivot>
>["data"];
type RecipientInsightsPayload = Awaited<
    ReturnType<typeof getAggregationRecipientInsights>
>["data"];
type RecipientByYearPayload = Awaited<
...
export function mapToStatisticsData(
    ...
    const yearlyComparison = Array.from(yearMap.values()).sort(
        (a, b) => a.year - b.year,
    );

    const catMap = new Map<string | number, CategoryMonthlyData>();
    for (const [period, items] of Object.entries(
        categoryPivotPayload.categoryPivot ?? {},
    )) {
        for (const item of items as CategoryPivotItem[]) {
            ...
            const incomeAmount =
                item.income ?? (item.total > 0 ? item.total : 0);
            const expenseAmount =
                item.expense != null
            ...
            cd.expenseMonths[period] =
                (cd.expenseMonths[period] ?? 0) + expenseAmount;
            cd.incomeMonths[period] =
                (cd.incomeMonths[period] ?? 0) + incomeAmount;
    ...
    const categoryPivot = Array.from(catMap.values()).sort(
        (a, b) => b.total - a.total,
    );

    const topRecipients: RecipientSpending[] = (
        recipientInsights.topMerchants ?? []
    )
        .slice(0, 20)
        .map((m) => ({
            name: m.name,
    ...
    const topRecipientsByYear: Record<string, RecipientSpending[]> =
        Object.fromEntries(
            Object.entries(recipientByYear.recipientsByYear ?? {}).map(
                ([year, recs]) => [
                    year,
...
export function useStatistics() {
    ...
    const hasExclusions =
        effectiveExcludedCategoryIds.length > 0 ||
        settingsExcludedRecIds.length > 0;
    ...
    const monthlySummaryFilteredQuery = useFilteredStatisticsQuery(
        aggregationKeys.monthlySummaryFiltered(
            targetCurrency,
    ...
    const categoryPivotFilteredQuery = useFilteredStatisticsQuery(
        aggregationKeys.categoryPivotFiltered(
            targetCurrency,
            effectiveExcludedCategoryIds,
            settingsExcludedRecIds,
        ),
    ...
    const recipientByYearFilteredQuery = useFilteredStatisticsQuery(
        aggregationKeys.recipientByYearFiltered(
            targetCurrency,
            effectiveExcludedCategoryIds,
            settingsExcludedRecIds,
        ),
    ...
    const recipientInsightsFilteredQuery = useFilteredStatisticsQuery(
        aggregationKeys.recipientInsightsFiltered(
            targetCurrency,
            effectiveExcludedCategoryIds,
            settingsExcludedRecIds,
        ),
