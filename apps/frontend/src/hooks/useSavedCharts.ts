...
export function useCreateSavedChart() {
    ...
    const { t } = useLanguage();
    return useMutation({
        mutationFn: (payload: SavedChartCreate) =>
            apiClient.createSavedChart(payload),
        onSuccess: () => {
...
export function useUpdateSavedChart() {
    ...
    return useMutation({
        mutationFn: ({
            id,
            ...payload
        }: { id: number } & Partial<SavedChartCreate>) =>
            apiClient.updateSavedChart(id, payload),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: savedChartKeys.all });
        },
        onError: (error: Error) => {
            toast.error(t("charts.updateFailed"), {
                description: apiErrorToMessage(error, t),
            });
        },
    });
...
export function useDeleteSavedChart() {
    ...
    return useMutation({
        ...
        onError: (error: Error) => {
            toast.error(t("charts.deleteFailed"), {
                description: apiErrorToMessage(error, t),
            });
        },
    });
