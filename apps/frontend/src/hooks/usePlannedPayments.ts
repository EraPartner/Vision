...
export function usePlannedPayments(showInactive: boolean = false) {
    ...
    const { currency: defaultCurrency } = useCurrencyFormatSettings();
    const queryKey = [
        "plannedTransactions",
        showInactive,
        defaultCurrency,
    ...
    const mapPayment = useCallback(
        (payment: Parameters<typeof mapFromAPI>[0]) =>
            mapFromAPI(payment, defaultCurrency),
        [defaultCurrency],
    );
    ...
    const setList = useCallback(
        (updater: (prev: PlannedPayment[]) => PlannedPayment[]) => {
            queryClient.setQueryData<PlannedPayment[]>(queryKey, (prev) =>
                updater(prev ?? []),
            );
        },
        [queryClient, showInactive, defaultCurrency], // eslint-disable-line react-hooks/exhaustive-deps
    );
    ...
    const addMutation = useMutation({
        ...
        onSuccess: (created) => {
            setList((prev) => [...prev, mapPayment(created)]);
            invalidateUpcoming();
        },
    ...
    const updateMutation = useMutation({
        ...
        onSuccess: (updated, { id }) => {
            setList((prev) =>
                prev.map((p) => (p.id === id ? mapPayment(updated) : p)),
            );
            invalidateUpcoming();
        },
    ...
    const toggleMutation = useMutation({
        mutationFn: (id: number) => {
            ...
            return apiClient.updatePlannedTransaction(id, {
    ...
    return {
        ...
        error: queryError
            ? queryError instanceof Error
                ? queryError.message
                : "Failed to fetch planned payments"
            : null,
        refetch,
    };
}
