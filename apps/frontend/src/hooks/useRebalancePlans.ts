...
export function useRebalancePlans() {
    ...
    const save = useMutation({
        mutationFn: (next: RebalancePlan[]) =>
            apiClient.saveSetting(SETTING_KEY, next),
        onError: (error: Error) => {
            toast.error(t("rebalance.plan.saveFailed"), {
    ...
    const upsertPlan = async (plan: RebalancePlan) => {
        ...
        toast.success(
            t(exists ? "rebalance.plan.updated" : "rebalance.plan.saved"),
        );
        return plan;
    };
