...
function ComposedSeriesLayerInner<Datum>({
    ...
    const scaleFor = (s: ComposedSeries<Datum>) =>
        s.axis === "right" && rightScale ? rightScale : leftScale;
    ...
    return (
        <>
            ...
            {series.map((s, i) => {
                ...
                const base =
                    yS(Math.max(0, (yS.domain() as number[])[0])) ??
                    innerHeight;
                return (
...
const ComposedSeriesLayer = memo(
    ComposedSeriesLayerInner,
) as typeof ComposedSeriesLayerInner;
...
function Inner<Datum>({
                ...
                for (const d of data) {
                    if (s.type === "candlestick") {
                        ...
                        if (hi != null && Number.isFinite(hi))
                            values.push(hi as number);
                        if (lo != null && Number.isFinite(lo))
                            values.push(lo as number);
                    } else {
                        const v = s.accessor?.(d);
    ...
    const rightScale = useMemo(
        () => (hasRight ? buildYScale("right") : null),
        [buildYScale, hasRight],
    );
    ...
    const safeHoverIdx =
        hoverIdx != null && hoverIdx < data.length ? hoverIdx : null;
    const keyboardNav = useChartKeyboardNav({
        pointCount: data.length,
        index: safeHoverIdx,
        onIndexChange: setHoverIdx,
        onClear: handleLeave,
    });

    const hoverDatum = safeHoverIdx != null ? data[safeHoverIdx] : null;
                    ...
                    hoverDatum && tooltipTitle
                        ...
                        : hoverDatum
                          ? // App date format, not the browser locale, for the fallback title
                            String(
                                xAccessor(hoverDatum) instanceof Date
                                    ? formatDateWithAppSettings(
                                          xAccessor(hoverDatum) as Date,
