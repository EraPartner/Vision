...
import { formatPeriodShort, type PivotValueMode } from "./statisticsUtils";
                    ...
                    <tr className="border-b border-border/50 bg-muted/30">
                      ...
                      {filteredPeriods.map((p) => {
                        ...
                        return (
                          <td
                            key={p}
                            className={`text-right py-2 px-3 tabular-nums font-semibold ${val === 0 ? "text-muted-foreground/40" : ""} ${val < 0 ? "text-destructive" : ""}`}
                          >
                            {val === 0 ? "—" : formatCurrency(val)}
                          </td>
                        );
                      })}
                      <td className={`text-right py-2 px-3 font-bold tabular-nums ${group.total < 0 ? "text-destructive" : ""}`}>
                      ...
                      <tr key={cat.categoryId} className="border-b border-border/50 hover:bg-muted/50 transition-colors">
                        ...
                        {filteredPeriods.map((p) => {
                          ...
                          return (
                            <td
                              key={p}
                              className={`text-right py-2 px-3 tabular-nums ${val === 0 ? "text-muted-foreground/40" : ""} ${val < 0 ? "text-destructive" : ""}`}
                            >
                              {val === 0 ? "—" : formatCurrency(val)}
                            </td>
                          );
                        })}
                        <td className={`text-right py-2 px-3 font-medium tabular-nums ${cat.filteredTotal < 0 ? "text-destructive" : ""}`}>
                ...
                <tr className="border-t-2 border-border font-bold">
                  ...
                  {filteredPeriods.map((p) => (
                    <td key={p} className="text-right py-2 px-3 tabular-nums">
                      {formatCurrency(columnTotals[p] || 0)}
                    </td>
                  ))}
