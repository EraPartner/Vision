...
const METRIC_GROUPS: { titleKey: string; metrics: MetricDescriptor[] }[] = [
  {
    ...
    metrics: [
      ...
      { key: "pegRatio", labelKey: "research.metric.pegRatio", format: "ratio" },
      { key: "priceToBook", labelKey: "market.priceBook", format: "ratio" },
    ],
  },
  {
    titleKey: "research.fundamentals.groupProfitability",
    metrics: [
      { key: "profitMargin", labelKey: "research.fundamentals.profitMargin", format: "pct" },
      { key: "grossMargin", labelKey: "research.metric.grossMargin", format: "pct" },
      { key: "operatingMargin", labelKey: "research.metric.operatingMargin", format: "pct" },
      { key: "returnOnEquity", labelKey: "research.fundamentals.roe", format: "pct" },
    ],
  },
  {
    titleKey: "research.fundamentals.groupLeverage",
    metrics: [
      { key: "debtToEquity", labelKey: "research.metric.debtToEquity", format: "ratio" },
      { key: "currentRatio", labelKey: "research.metric.currentRatio", format: "ratio" },
      { key: "quickRatio", labelKey: "research.metric.quickRatio", format: "ratio" },
      { key: "interestCoverage", labelKey: "research.metric.interestCoverage", format: "ratio" },
    ],
  },
  {
    titleKey: "research.fundamentals.groupCashGrowth",
    metrics: [
      { key: "freeCashFlow", labelKey: "research.metric.freeCashFlow", format: "largeNum" },
      { key: "fcfYield", labelKey: "research.metric.fcfYield", format: "pct" },
      { key: "revenueGrowth", labelKey: "research.metric.revenueGrowth", format: "pct" },
      { key: "earningsGrowth", labelKey: "research.metric.earningsGrowth", format: "pct" },
    ],
  },
  {
    titleKey: "research.fundamentals.groupDividendSize",
    metrics: [
      { key: "dividendYield", labelKey: "market.divYield", format: "pct" },
      { key: "payoutRatio", labelKey: "research.metric.payoutRatio", format: "pct" },
      { key: "marketCap", labelKey: "market.marketCap", format: "largeNum" },
      { key: "revenue", labelKey: "research.fundamentals.revenue", format: "largeNum" },
...
export function ResearchFundamentalsTab({ symbol, enabled }: ResearchFundamentalsTabProps) {
  ...
  if (result?.meta.source === "unavailable" || !result?.data) {
    return <ResearchUnavailableNote provider={result?.meta.provider ?? null} />;
  }

  if (!f) {
    return <p className="py-4 text-center text-sm text-muted-foreground">{t("research.fundamentals.none")}</p>;
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-2">
        {f.sector ? (
          <span className="text-xs text-muted-foreground">{f.sector}</span>
        ) : <span />}
      ...
      <div className="rounded-xl border border-border/60 bg-muted/20 p-4">
        <h3 className="mb-3 text-sm font-semibold">{t("research.scorecard.title")}</h3>
              ...
              {visible.map((m) => (
                <div key={String(m.key)} className="flex items-center justify-between border-b border-border/50 py-1.5">
                  ...
                  <span className="text-sm font-medium tabular-nums text-foreground">
                    {fmt(m.format, f[m.key] as number | null | undefined)}
                  </span>
                </div>
              ))}
            </div>
