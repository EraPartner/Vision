...
describe('TaxYearSwitcher', () => {
    ...
    it('renders the trigger with the viewed year', () => {
        setup({
            viewedYear: 2024,
            years: [{ year: 2024, isCurrent: false, hasSnapshot: true, hasTransactions: false }],
        });
        render(<TaxYearSwitcher />);
        expect(screen.getByRole('button', { name: /switch tax year/i })).toHaveTextContent(
            'Tax year 2024',
        );
    });
    ...
    it('lists every year from the hook when opened', async () => {
        setup({
            ...
            years: [
                ...
                { year: 2025, isCurrent: false, hasSnapshot: true, hasTransactions: false },
                { year: 2024, isCurrent: false, hasSnapshot: false, hasTransactions: true },
            ],
        });
        const user = userEvent.setup();
        render(<TaxYearSwitcher />);
        await user.click(screen.getByRole('button', { name: /switch tax year/i }));
    ...
    it('calls setViewedYear when a year is selected', async () => {
        const { setViewedYear } = setup({
            ...
            years: [
                { year: 2026, isCurrent: true, hasSnapshot: false, hasTransactions: false },
                { year: 2023, isCurrent: false, hasSnapshot: true, hasTransactions: false },
            ],
        });
        const user = userEvent.setup();
        render(<TaxYearSwitcher />);
    ...
    it('shows the create-snapshot footer when viewing a year without a snapshot', async () => {
        ...
        render(<TaxYearSwitcher />);
        await user.click(screen.getByRole('button', { name: /switch tax year/i }));
        const createBtn = await screen.findByRole('menuitem', { name: /create profile for 2024/i });
    ...
    it('hides the create-snapshot footer when a snapshot already exists', async () => {
        setup({
            ...
            years: [
                { year: 2026, isCurrent: true, hasSnapshot: false, hasTransactions: false },
                { year: 2024, isCurrent: false, hasSnapshot: true, hasTransactions: false },
            ],
        });
        const user = userEvent.setup();
        render(<TaxYearSwitcher />);
        ...
        expect(screen.queryByText(/create profile for 2024/i)).not.toBeInTheDocument();
    });
    ...
    it('hides the create-snapshot footer when viewing the live year', async () => {
        setup({
            ...
            liveYear: 2026,
            snapshotYears: [],
            years: [{ year: 2026, isCurrent: true, hasSnapshot: false, hasTransactions: false }],
        });
        const user = userEvent.setup();
        render(<TaxYearSwitcher />);
        await user.click(screen.getByRole('button', { name: /switch tax year/i }));
        await screen.findAllByRole('menuitem');
        expect(screen.queryByText(/create profile for/i)).not.toBeInTheDocument();
    });
});
