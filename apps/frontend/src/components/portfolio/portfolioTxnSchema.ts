...
type IssueCtx = { addIssue: (issue: { code: typeof z.ZodIssueCode.custom; message: string }) => void };
...
export function addPortfolioTxnSchema({ isBuySell, isGift }: PortfolioTxnFlags) {
  return rawFields.transform((raw, ctx): AddPortfolioTxnParsed => {
    ...
    if (!isGift && (effectiveAmount === undefined || Number.isNaN(effectiveAmount))) {
...
export function editPortfolioTxnSchema({ isBuySell, isGift }: PortfolioTxnFlags) {
  return rawFields.transform((raw, ctx): EditPortfolioTxnParsed => {
    ...
    if (
      !isGift &&
      (effectiveAmount === undefined || Number.isNaN(effectiveAmount) || effectiveAmount <= 0)
    ) {
      return fail(ctx, 'addPortTxn.error.amountRequired');
    }
    if (isGift && effectiveUnits === undefined) {
    ...
    if (!raw.date) {
      return fail(ctx, 'plannedPage.link.pickDate');
    }
