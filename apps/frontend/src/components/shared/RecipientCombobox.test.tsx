...
const selectedRecipient = { ...RECIPIENT_STUB, id: 999, name: "Beyond First Page" };

describe("RecipientCombobox", () => {
  it("resolves the selected label independently and clears search when closed", async () => {
    server.use(
      http.get(`${API_BASE}/api/recipients/999`, () => ok(selectedRecipient)),
      http.get(`${API_BASE}/api/recipients`, ({ request }) => {
        const search = new URL(request.url).searchParams.get("search");
        const items = search ? [] : [RECIPIENT_STUB];
        return ok({ items, total: items.length, limit: 100, offset: 0, links: [] });
      }),
    );
    const onSelect = vi.fn();
    renderWithApp(
      <RecipientCombobox aria-label="Recipient" value={999} onSelect={onSelect} />,
    );

    const trigger = screen.getByRole("combobox", { name: "Recipient" });
    expect(await screen.findByText("Beyond First Page")).toBeInTheDocument();

    await userEvent.click(trigger);
    const search = screen.getByPlaceholderText("Search recipients…");
    await userEvent.type(search, "missing");
    await waitFor(() => expect(screen.getByText("Beyond First Page")).toBeInTheDocument());
  ...
  it("keeps closed deferred rows on one shared list query without per-id detail requests", async () => {
    ...
    server.use(
      http.get(`${API_BASE}/api/recipients`, () =>
        ok({ items: [RECIPIENT_STUB], total: 1, limit: 100, offset: 0, links: [] }),
      ),
    ...
    expect(await screen.findByText(RECIPIENT_STUB.name)).toBeInTheDocument();
