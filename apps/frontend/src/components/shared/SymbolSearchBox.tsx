...
export function SymbolSearchBox({
  ...
  const onInputKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    const currentIndex = resultOptions.findIndex((option) => option.id === activeOptionId);
    let nextIndex: number | undefined;

    if (event.key === "ArrowDown") {
      nextIndex = currentIndex < resultOptions.length - 1 ? currentIndex + 1 : 0;
    } else if (event.key === "ArrowUp") {
      nextIndex = currentIndex > 0 ? currentIndex - 1 : resultOptions.length - 1;
    ...
    <div className={cn("relative", className)}>
      ...
      {open ? (
        <Card className="absolute z-50 top-full mt-2 w-full glass-thick">
          <CardContent ref={listboxRef} id={listboxId} role="listbox" className="p-1">
            <SymbolSearchListContext.Provider value={{ activeOptionId, setActiveOptionId }}>
              {children}
