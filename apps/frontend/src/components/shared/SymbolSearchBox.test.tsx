...
const apple = { symbol: "AAPL", name: "Apple", type: "Equity", exchange: "NASDAQ" };
const microsoft = { symbol: "MSFT", name: "Microsoft", type: "Equity", exchange: "NASDAQ" };

describe("SymbolSearchBox", () => {
  it("wires the input and results as an ARIA combobox", () => {
    render(
      <SymbolSearchBox value="a" onChange={() => undefined} placeholder="Search" open onDismiss={() => undefined}>
  ...
  it("preserves text-editing keys and dismisses the controlled popup with Escape", async () => {
    ...
    render(
      <SymbolSearchBox value="apple" onChange={() => undefined} placeholder="Search" open onDismiss={onDismiss}>
        <SymbolSearchResultItem item={apple} onSelect={() => undefined} />
      </SymbolSearchBox>,
    );
  ...
  it("keeps popup options out of the Tab sequence", async () => {
    ...
    await user.tab();
    expect(screen.getByRole("button", { name: "After search" })).toHaveFocus();
  });

  it("does not turn the shared result row into an orphan option outside a search box", () => {
    render(<SymbolSearchResultItem item={apple} onSelect={() => undefined} />);
