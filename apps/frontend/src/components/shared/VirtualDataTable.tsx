...
const MAX_SCROLL_ENTRIES = 64;
const tableScrollPositions = new Map<string, number>();

function rememberTableScroll(key: string, top: number) {
    tableScrollPositions.delete(key);
    tableScrollPositions.set(key, top);
    while (tableScrollPositions.size > MAX_SCROLL_ENTRIES) {
        const oldest = tableScrollPositions.keys().next().value;
        if (oldest === undefined) break;
        tableScrollPositions.delete(oldest);
    }
}
...
interface VirtualDataTableProps<T> {
    ...
    /** Stable identity within the current history entry for Back/Forward scroll restoration. */
                ...
                rowsInteractive
                    ? (event) => {
                          ...
                          if (
                              ...
                              event.preventDefault();
                              (onRowOpen ?? onRowDoubleClick)?.(
                                  row,
                                  sourceIndex,
                              );
                          } else if (event.key === " ") {
                              event.preventDefault();
                              (onRowQuickLook ?? onRowDoubleClick)?.(
                                  row,
                                  sourceIndex,
                              );
                          }
        ...
        >
            ...
            {hasEditableColumns && (
                <div
                    ...
                    style={{ width: isEditing ? "88px" : "40px", flex: "none" }}
                >
                    {isEditing ? (
...
const MemoizedVirtualizedTableRow = memo(
    VirtualizedTableRow,
) as typeof VirtualizedTableRow;
...
export function VirtualDataTable<T extends Record<string, unknown>>({
    ...
    const tableIdentity =
        scrollRestorationKey ?? title ?? `columns:${columnKeySignature}`;
    const historyScrollKey = `${location.key}:${tableIdentity}`;
    ...
    const handleResizeStart = useCallback(
        (e: React.PointerEvent, colKey: string, currentWidth: number) => {
            e.preventDefault();
            ...
            const handlePointerMove = (ev: PointerEvent) => {
                if (
                    !resizingRef.current ||
                    ev.pointerId !== resizingRef.current.pointerId
                )
                    return;
                const diff = ev.clientX - resizingRef.current.startX;
                const col = columns.find(
                    (c) => c.key === resizingRef.current!.key,
                );
                const minW = col?.minWidth || 60;
                pendingWidth = Math.min(
                    MAX_COLUMN_WIDTH,
                    Math.max(minW, resizingRef.current.startWidth + diff),
                );
    ...
    const handleResizeKeyDown = useCallback(
        (e: React.KeyboardEvent, colKey: string, currentWidth: number) => {
            if (e.key !== "ArrowLeft" && e.key !== "ArrowRight") return;
            e.preventDefault();
            const minWidth =
                columns.find((col) => col.key === colKey)?.minWidth || 60;
            const delta = e.key === "ArrowRight" ? 10 : -10;
            setColumnWidths((prev) => ({
                ...prev,
                [colKey]: Math.min(
                    MAX_COLUMN_WIDTH,
                    Math.max(minWidth, (prev[colKey] ?? currentWidth) + delta),
                ),
            }));
        },
        [columns],
    ...
    const syncRenderedColumnWidth = useCallback(
        (handle: HTMLDivElement, colKey: string, fallbackWidth: number) => {
            const measuredWidth =
            ...
            const width = Math.min(
                MAX_COLUMN_WIDTH,
                Math.max(minWidth, measuredWidth || fallbackWidth),
            );
            setColumnWidths((prev) =>
                prev[colKey] === width ? prev : { ...prev, [colKey]: width },
            );
            return width;
        },
        [columns],
    ...
    const processedRows = useMemo(() => {
        let result: IndexedRow<T>[] = deferredData.map((row, sourceIndex) => ({
            row,
            sourceIndex,
        }));
    ...
    const pendingRestoreRef = useRef<number | null>(
        tableScrollPositions.get(historyScrollKey) ?? null,
    );
    ...
    useEffect(() => {
        ...
        if (target == null || !parentEl) return;

        if (
            parentEl.scrollHeight < target + parentEl.clientHeight &&
            hasMore &&
            !isFetchingMore &&
            onLoadMore
        ) {
            if (loadRequestedForLengthRef.current !== processedRows.length) {
    ...
    useEffect(
        () => () => {
            const parentEl = parentRef.current;
            if (parentEl)
                rememberTableScroll(historyScrollKey, parentEl.scrollTop);
        },
        [historyScrollKey],
    );
    ...
    const focusRowByIndex = useCallback(
        (index: number) => {
            ...
            const clamped = Math.max(
                0,
                Math.min(index, processedRows.length - 1),
            );
    ...
    return (
        <Card className="relative overflow-hidden">
            ...
            >
                ...
                >
                    <div
                        className="flex items-center bg-muted/50 min-h-[40px]"
                        role="row"
                    >
                        {columns.map((col) => {
                            const width = columnWidths[col.key];
                ...
                <div className="flex items-center justify-between border-t px-6 py-3">
                    <p className="text-sm text-muted-foreground">
                        {processedRows.length !== deferredData.length
                            ...
                            : t("table.loadedOf", {
                                  loaded: deferredData.length.toString(),
                                  total: displayTotal.toString(),
                              })}
