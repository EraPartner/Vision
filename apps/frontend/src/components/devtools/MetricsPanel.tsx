import { useQueryMetrics, resetQueryMetrics } from '@/lib/devtools/queryMetrics';
...
export function MetricsPanel() {
    ...
    const cachePct = (metrics.cacheHitRatio * 100).toFixed(1);
    ...
    return (
        <div className="flex flex-col h-full overflow-auto px-3 py-2 gap-3">
            <div className="grid grid-cols-2 gap-2">
                <StatCard label="Total requests" value={String(metrics.totalRequests)} />
                <StatCard
                    label="Error rate"
                    value={`${errorPct}%`}
                ...
                <StatCard
                    ...
                    warn={metrics.cacheHitRatio < 0.3 && metrics.totalRequests > 10}
                />
                <StatCard
                    label="Mutations"
                ...
                <div>
                    ...
                    <div className="space-y-0.5">
                        {metrics.topEndpoints.slice(0, 10).map((ep) => (
                            <div key={ep.endpoint} className="flex items-center gap-2 text-[10px] font-mono">
                                <span className="flex-1 truncate text-foreground">{ep.endpoint}</span>
