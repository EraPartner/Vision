...
export function StatCard({ title, value, change, changeType = "neutral", icon: Icon, trend = "neutral" }: StatCardProps) {
  ...
  return (
    <Card className={`relative overflow-hidden border-none shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br ${trendGradient} backdrop-blur-sm`}>
      ...
      <CardContent>
        <div className="text-3xl font-bold bg-gradient-to-br from-slate-900 to-slate-700 dark:from-white dark:to-slate-300 bg-clip-text text-transparent">
          {value}
        </div>
        {change && (
