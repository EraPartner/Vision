...
export const BackupSection = memo(function BackupSection() {
    ...
    useEffect(() => {
        ...
        apiClient.loadBackupSettings()
            ...
            .catch(() => { /* leave unloaded — saves stay disabled */ })
        ...
        }).finally(() => { if (!cancelled) setEncryptionLoading(false); });

        try {
            setReminderDismissed(window.localStorage.getItem(REMINDER_KEY) === '1');
        } catch { /* ignore */ }
    ...
    if (!apiClient.isElectron()) {
        return (
            <SettingsSection title={t('settings.tab.backup')} description={t('settings.backup.description')}>
        ...
        <>
            <SettingsSection title={t('settings.tab.backup')} description={t('settings.backup.description')}>
                <SettingsGroup label={t('settings.backup.title')}>
                    <SettingRow title={t('settings.backup.directory')} description={t('settings.backup.directoryHint')} layout="stack">
                        <div className="flex gap-2">
                            ...
                            />
                            <Button variant="outline" size="sm" onClick={() => { void handleBrowseBackupDir(); }} disabled={backupLoading} className="shrink-0">
                    ...
                    <SettingRow title={t('settings.backup.backupOnQuit')} description={t('settings.backup.backupOnQuitHint')} htmlFor="backup-on-quit">
                        <Switch
                            id="backup-on-quit"
                    ...
                    <SettingRow title={t('settings.backup.runNow')} description={t('settings.backup.formatNote')}>
                        <Button variant="outline" size="sm" onClick={() => { void handleBackupNow(); }} disabled={backupRunning || !backupDir}>
                            {backupRunning
                ...
                <SettingsGroup label={t('settings.backup.passphrase.title')}>
                    <SettingRow title={t('settings.backup.passphrase.label')} description={t('settings.backup.passphrase.description')} layout="stack">
                        <Input
                            type="password"
                        ...
                        <div className="mt-3 flex gap-2">
                            <Button variant="outline" size="sm" onClick={() => { void handleSaveBackupPassphrase(); }} disabled={backupLoading || savingBackupPassphrase || !encryptionStatus?.secureStorageAvailable}>
                                {savingBackupPassphrase
                                    ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" />
                                    : <CheckCircle2 className="mr-1.5 h-3.5 w-3.5" />}
                                {t('settings.backup.passphrase.save')}
                            </Button>
                            <Button variant="ghost" size="sm" onClick={() => { void handleClearBackupPassphrase(); }} disabled={backupLoading || savingBackupPassphrase || !encryptionStatus?.secureStorageAvailable}>
                                {t('settings.backup.passphrase.clear')}
                            </Button>
                        </div>
                        {!encryptionStatus?.secureStorageAvailable && (
                            <p className="mt-2 text-xs text-destructive">{t('settings.backup.passphrase.unavailable')}</p>
                        )}
                        {encryptionStatus && (encryptionStatus.hasEnvPassphrase || encryptionStatus.hasStoredPassphrase) && !reminderDismissed && (
                            <div className="mt-3 flex items-start gap-3 rounded-lg border border-warning/30 bg-warning/10 px-4 py-3 text-sm">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="mt-1 text-xs text-muted-foreground">{t('settings.backup.passphrase.reminderDesc')}</div>
                                </div>
                                <Button
                                    variant="ghost"
                                    size="sm"
                                    ...
                                    onClick={() => {
                                        try { window.localStorage.setItem(REMINDER_KEY, '1'); } catch { /* ignore */ }
                ...
                <SettingsGroup label={t('settings.restore.title')}>
                    <SettingRow title={t('settings.restore.description')} layout="stack">
                        ...
                        <div className="mt-3 flex gap-2">
                            <Input
                                ...
                                title={restoreFile}
                            />
                            <Button variant="outline" size="sm" onClick={() => { void handleSelectRestoreFile(); }} disabled={restoreRunning} className="shrink-0">
                                <FolderOpen className="mr-1.5 h-3.5 w-3.5" />
            ...
            <AlertDialog open={restoreConfirmOpen} onOpenChange={setRestoreConfirmOpen}>
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction onClick={() => { void handleRestoreConfirmed(); }} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
