...
export default function PlannedPaymentForm({ open, onOpenChange, onSubmit, initial }: Props) {
  ...
  const [endDate, setEndDate] = useState<Date | undefined>(initial?.end_date ? new Date(initial.end_date) : undefined);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="grid gap-4 py-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="grid gap-1.5">
              ...
              <Input id="pp-recipient" placeholder="e.g. Landlord" value={recipient} onChange={(e) => setRecipient(e.target.value)} />
            </div>
            <div className="grid gap-1.5">
              <Label htmlFor="pp-category">Category</Label>
              <Input id="pp-category" placeholder="e.g. HOUSING" value={category} onChange={(e) => setCategory(e.target.value)} />
            </div>
          </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={!name.trim() || !amount || !dueDate}>
