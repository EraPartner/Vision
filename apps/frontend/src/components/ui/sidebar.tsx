...
const SidebarTrigger = React.forwardRef<React.ElementRef<typeof Button>, React.ComponentProps<typeof Button>>(
    ({className, onClick, ...props}, ref) => {
        ...
        return (
            ...
            >
                <PanelLeft/>
                <span className="sr-only">{t("menu.toggleSidebar")}</span>
            </Button>
        );
    },
);
    ...
    ({className, ...props}, ref) => {
        ...
        return (
            <button
                ...
                className={cn(
                    "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] group-data-[side=left]:-right-4 group-data-[side=right]:left-0 hover:after:bg-border/60 sm:flex",
                    "[[data-side=left]_&]:cursor-w-resize [[data-side=right]_&]:cursor-e-resize",
                    "[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize",
                    "group-data-[collapsible=offcanvas]:translate-x-0 group-data-[collapsible=offcanvas]:after:left-full group-data-[collapsible=offcanvas]:hover:bg-foreground/[0.04]",
    ...
    ({className, ...props}, ref) => {
        return (
            <Input
                ...
                data-sidebar="input"
                className={cn(
                    "h-8 w-full shadow-none focus-visible:ring-2 focus-visible:ring-ring/70",
                    className,
                )}
                {...props}
            />
    ...
    ({className, ...props}, ref) => {
        return (
            <Separator
                ...
                className={cn("mx-3 w-auto bg-border/50", className)}
                {...props}
            />
        );
    },
...
const SidebarMenuSub = React.forwardRef<HTMLUListElement, React.ComponentProps<"ul">>(
    ({className, ...props}, ref) => (
        <ul
            ...
            className={cn(
                "mx-3.5 mt-1 flex min-w-0 translate-x-px flex-col gap-0.5 border-l border-border/40 px-2.5 py-0.5",
                "group-data-[collapsible=icon]:hidden",
                className,
            )}
            {...props}
