...
describe("Card material ownership", () => {
    it("does not restate Card-owned material and frame classes at call sites", () => {
        const cardTags = tsxSources(join(process.cwd(), "src")).flatMap(
            (source) =>
                [...source.matchAll(/<Card\b[\s\S]*?>/g)].map(
                    (match) => match[0],
                ),
        );
        expect(cardTags.join("\n")).not.toMatch(
            /\b(?:card-material|glass-thin|glass-regular|premium-frame|micro-lift)\b/,
        );
    });
});
