...
function research<T>(
    data: T,
    meta: { provider: string | null; source: string },
) {
...
describe("ResearchHomePage", () => {
    it("renders the research heading and search box", async () => {
        ...
        expect(
            await screen.findByLabelText(/search any stock/i),
        ).toBeInTheDocument();
        expect(
        ...
        expect(
            await screen.findByRole("heading", {
                level: 3,
                name: /your watchlist is empty/i,
            }),
        ).toBeInTheDocument();
        expect(
            screen.queryByRole("button", { name: /market lookup/i }),
    ...
    it("surfaces 'live data unavailable' when the search source is unavailable", async () => {
        server.use(
            ...
            http.get(`${API_BASE}/api/research/search`, () =>
                research(
                    { items: [] },
                    { provider: null, source: "unavailable" },
                ),
            ),
        );
        const user = userEvent.setup();
        ...
        expect(
            await screen.findByText(/live data unavailable/i),
        ).toBeInTheDocument();
    });
});
