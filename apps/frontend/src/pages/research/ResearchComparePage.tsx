...
export default function ResearchComparePage() {
    ...
    const symbols = useMemo(
        () => parseResearchSymbols(searchParams),
        [searchParams],
    );
    const [activeTab, setActiveTab] = useTabParam(COMPARE_TABS, "performance");
    const selectedRange =
        RANGES.find((option) => option.range === searchParams.get("range")) ??
        RANGES.find((option) => option.range === DEFAULT_RANGE)!;
    const metricKeys = useMemo(
        () => new Set(FUNDAMENTALS_METRICS.map((metric) => metric.key)),
        [],
    );
    const rawSortMetric = searchParams.get("sort");
    const sortMetric =
        rawSortMetric && metricKeys.has(rawSortMetric as FundamentalsMetricKey)
            ? (rawSortMetric as FundamentalsMetricKey)
            : null;
    const {
        searchText,
        setSearchText,
    ...
    const writeSymbols = (nextSymbols: string[]) => {
        setSearchParams(
            (previous) => {
                const next = new URLSearchParams(previous);
                next.delete("symbol");
                for (const symbol of nextSymbols.slice(
                    0,
                    MAX_COMPARE_SYMBOLS,
                )) {
                    next.append("symbol", symbol);
                }
                return next;
            },
            { replace: true },
    ...
    const addSymbol = (symbol: string) => {
        const s = normalizeResearchSymbol(symbol);
        if (s && !symbols.includes(s) && symbols.length < MAX_COMPARE_SYMBOLS) {
            writeSymbols([...symbols, s]);
        }
        setSearchText("");
    };
    const removeSymbol = (symbol: string) =>
        writeSymbols(symbols.filter((candidate) => candidate !== symbol));

    const setSelectedRange = (option: (typeof RANGES)[number]) => {
        setSearchParams(
            (previous) => {
                const next = new URLSearchParams(previous);
                if (option.range === DEFAULT_RANGE) next.delete("range");
    ...
    const { chartData, series, statsRows, correlation } = useMemo(() => {
        ...
        for (const s of perSymbol)
            for (const p of s.rebased) timeSet.add(p.time);
        const times = Array.from(timeSet).sort((a, b) => a - b);

        const lookup = perSymbol.map(
            (s) => new Map(s.rebased.map((p) => [p.time, p.value])),
        );
        const data = times.map((time) => {
            const row: Record<string, number | null> & { time: number } = {
                time,
            };
            perSymbol.forEach((s, i) => {
                row[s.symbol] = lookup[i].get(time) ?? null;
            });
            return row;
        });

        const lineSeries: LineSeries<(typeof data)[number]>[] = perSymbol.map(
            (s, i) => ({
                key: s.symbol,
                label: s.symbol,
        ...
        const returnSeries: ReturnSeries[] = perSymbol.map((s) => ({
            symbol: s.symbol,
            byTime: s.returns,
        }));
        const matrix: (number | null)[][] = returnSeries.map((a, i) =>
            returnSeries.map((b, j) =>
                i === j ? 1 : pearsonOnTime(a.byTime, b.byTime),
            ),
        );
    ...
    const fundamentalsRows = useMemo(() => {
        const rows = symbols.map((symbol, i) => {
            ...
            const data: ResearchFundamentals | null =
                payload?.fundamentals ?? null;
            const scorecard: ResearchScorecard | null =
    ...
    const fundamentalsLoading = fundamentalsQueries.some(
        (q) => q.isFetching && !q.data,
    );

    const fmtMetric = useCallback(
        (metric: FundamentalsMetric, val: number | null | undefined) => {
    ...
    const toggleSort = (key: FundamentalsMetricKey) => {
        setSearchParams(
            (previous) => {
                const next = new URLSearchParams(previous);
                if (sortMetric === key) next.delete("sort");
    ...
    return (
        <PageShell className="">
            <PageHeader
                title={t("research.compare.title")}
                subtitle={t("research.compare.subtitle")}
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                    <TabsList>
                        <TabsTrigger value="performance">
                            {t("research.compare.tabPerformance")}
                        </TabsTrigger>
                            ...
                            <CardContent>
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            <TableHead>
                                                {t("research.compare.symbol")}
                                            </TableHead>
                                            <TableHead className="text-right">
                                                {t("research.compare.return")}
                                            </TableHead>
                                            <TableHead className="text-right">
                                                {t(
                                                    "research.compare.volatility",
                                                )}
                                            </TableHead>
                                            <TableHead className="text-right">
                                                {t(
                                                    "research.compare.maxDrawdown",
                                                )}
                                            </TableHead>
                                        </TableRow>
                                    </TableHeader>
                                            ...
                                            <TableHeader>
                                                <TableRow>
                                                    <TableHead className="whitespace-nowrap">
                                                        {t(
                                                            "research.compare.symbol",
                                                        )}
                                                    </TableHead>
                                                    <TableHead className="whitespace-nowrap">
                                                        {t(
                                                            "research.scorecard.health",
                                                        )}
                                                    </TableHead>
                                                    {FUNDAMENTALS_METRICS.map(
                                                        (m) => (
                                                            <TableHead
                                                                key={m.key}
