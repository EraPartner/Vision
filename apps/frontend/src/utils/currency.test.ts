...
describe("currency formatting is pure", () => {
    test("uses only the configuration supplied to each call", () => {
        expect(formatCurrency(1234.56, "USD", "en-US", 0)).toBe("$1,235");
...
describe("formatCurrency — malformed input degrades like Money / the parts hook", () => {
    ...
    test("valid input still formats normally after a failed call (no poisoned state)", () => {
        ...
        expect(formatCurrency(1234.56, "EUR", "de-DE", 2)).toBe(
            "1.234,56\u00a0€",
        );
    });
});
...
describe("formatCurrencyCompact", () => {
    ...
    test("valid compact formatting still works after a failed call", () => {
        ...
        expect(
            formatCurrencyCompact(1_253_632, "EUR", "en-US", 2).isCompact,
        ).toBe(true);
    });
    ...
    test("keeps de-DE thousands full when compact notation has no abbreviation", () => {
        ...
        expect(result.parts.some((part) => part.type === "compact")).toBe(
            false,
        );
    });
    ...
    test("uses exceptZero consistently in full and compact signed output", () => {
        expect(formatCurrencyCompact(42, "EUR", "en-US", 2, true).display).toBe(
            "+€42.00",
        );
        expect(formatCurrencyCompact(0, "EUR", "en-US", 2, true).display).toBe(
            "€0.00",
        );
...
describe("formatPercent — value scale", () => {
    ...
    test("takes PERCENT UNITS, not a fraction (12.5 means 12.5%)", () => {
        ...
        expect(formatPercent(0.125, { locale: "en-US", digits: 3 })).toBe(
            "0.125%",
        );
...
describe("formatPercent — locale separator (the eu/us bug this fixes)", () => {
    ...
    test("us number format renders a dot decimal", () => {
        expect(
            formatPercent(12.5, { locale: numberFormatToLocale("us") }),
        ).toBe("12.5%");
    });
    ...
    test("ch and in formats follow their own separators", () => {
        ...
        expect(
            formatPercent(1234.5, { locale: numberFormatToLocale("ch") }),
        ).toMatch(/^1['’]234\.5%$/);
        // en-IN uses lakh/crore grouping, not thousands.
        expect(
            formatPercent(123456.5, { locale: numberFormatToLocale("in") }),
        ).toBe("1,23,456.5%");
    });
    ...
    test("the percent sign stays glued to the number in every format", () => {
        ...
        for (const setting of ["eu", "us", "ch", "in"]) {
            expect(
                formatPercent(12.5, { locale: numberFormatToLocale(setting) }),
            ).not.toMatch(/[\s\u00a0\u202f]%/);
        }
    });
});
...
describe("formatPercent — sign convention (exceptZero, matching money)", () => {
    test("signed renders + for positive and - for negative", () => {
        expect(formatPercent(3.2, { locale: "en-US", signed: true })).toBe(
            "+3.2%",
        );
        expect(formatPercent(-3.2, { locale: "en-US", signed: true })).toBe(
            "-3.2%",
    ...
    test("signed leaves zero unsigned — the same call formatCurrency makes", () => {
        expect(formatPercent(0, { locale: "en-US", signed: true })).toBe(
            "0.0%",
        );
    ...
    test("documents the inherited exceptZero pitfall: a loss rounding to zero loses its minus", () => {
        ...
        expect(formatPercent(-0.04, { locale: "en-US", signed: true })).toBe(
            "0.0%",
        );
...
describe("formatPercent — digits", () => {
    ...
    test("minDigits gives 'up to N' — trailing zeros dropped, fractions kept", () => {
        ...
        // grow one.
        expect(
            formatPercent(60, { locale: "en-US", digits: 1, minDigits: 0 }),
        ).toBe("60%");
        expect(
            formatPercent(7.5, { locale: "en-US", digits: 1, minDigits: 0 }),
        ).toBe("7.5%");
        expect(
            formatPercent(7.5, { locale: "de-DE", digits: 1, minDigits: 0 }),
        ).toBe("7,5%");
    });

    test("honours an explicit digit count, padding as well as rounding", () => {
        expect(formatPercent(3.14159, { locale: "en-US", digits: 0 })).toBe(
            "3%",
        );
        expect(formatPercent(3.14159, { locale: "en-US", digits: 2 })).toBe(
            "3.14%",
        );
        expect(formatPercent(3.1, { locale: "en-US", digits: 2 })).toBe(
            "3.10%",
        );
...
describe("formatPercent — degradation", () => {
    test("degrades to a bare value instead of throwing on out-of-range digits", () => {
        ...
        expect(() =>
            formatPercent(12.5, { locale: "en-US", digits: -1 }),
        ).not.toThrow();
        expect(formatPercent(12.5, { locale: "en-US", digits: -1 })).toBe(
            "12.5%",
        );
        expect(formatPercent(12.5, { locale: "en-US", digits: 101 })).toBe(
            "12.5%",
        );
    });
