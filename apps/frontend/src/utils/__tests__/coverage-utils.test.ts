...
describe("getCategoryColor", () => {
    ...
    it("assigns a chart-token color to any real category", () => {
        expect(getCategoryColor("FOOD:GROCERIES")).toMatch(
            /bg-chart-[1-8]\/15/,
        );
    ...
    it("uses the ordered root when a category name itself contains a colon", () => {
        const root = ["FOOD:SPECIAL"];
        expect(getCategoryColor(["FOOD:SPECIAL", "GROCERIES"])).toBe(
            getCategoryColor(root),
        );
        expect(
            getCategoryChartColor(["FOOD:SPECIAL", "GROCERIES", "ORGANIC"]),
        ).toBe(getCategoryChartColor(root));
        expect(categoryColorIndex(["FOOD:SPECIAL", "GROCERIES"])).toBe(
            categoryColorIndex(root),
        );
    });
    ...
    it("chart fill and badge classes agree on the token index", () => {
        ...
        expect(getCategoryColor("FOOD:GROCERIES")).toContain(
            `bg-chart-${index + 1}/15`,
        );
    });
