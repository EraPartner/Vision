...
function mockApi({
    ...
    server.use(
        http.get(`${API_BASE}/api/accounts`, ({ request }) => {
            ...
            return ok({
                items: [SOURCE, TARGET, ARCHIVED],
                total: 3,
                links: [],
            });
        }),
        http.get(
            `${API_BASE}/api/accounts/:id/merge-preview`,
            ({ request, params }) => {
                previewCalls.push({
                    id: String(params.id),
...
describe("MergeAccountDialog (integration, WP-B5 §3 F9 preview)", () => {
    it("lists candidates from the FULL population (active: 'all'), labeling archived ones", async () => {
        ...
        expect(
            screen.queryByRole("option", { name: /old kbc/i }),
        ).not.toBeInTheDocument();
        expect(
            await screen.findByRole("option", { name: "New KBC" }),
        ).toBeInTheDocument();
        // Archived candidates appear, explicitly labeled.
        expect(
            screen.getByRole("option", { name: "Dusty (Archived)" }),
        ).toBeInTheDocument();
    ...
    it("shows counts + projected balance + the interleaved-stamp warning once a survivor is chosen", async () => {
        ...
        expect(
            await screen.findByText(
                /1\.002 transactions \+ 3 planned will move/i,
            ),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/resulting balance .*1\.234,56/i),
        ).toBeInTheDocument();
    ...
    it("omits the interleaved warning when stamps do not interleave", async () => {
        ...
        expect(
            await screen.findByText(/will move; resulting balance/i),
        ).toBeInTheDocument();
        expect(
            screen.queryByText(/statement anchor will be cleared/i),
        ).not.toBeInTheDocument();
    });

    it("marks an incomplete projection and shows the excluded native balance", async () => {
        mockApi({
            preview: {
                ...PREVIEW,
                ...
                projectedBalanceIncomplete: true,
        ...
        expect(
            await screen.findByText(
                /resulting converted balance is incomplete/i,
            ),
        ).toBeInTheDocument();
        expect(
