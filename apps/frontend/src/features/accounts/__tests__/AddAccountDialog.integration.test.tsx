...
function mockCreate() {
    const calls: {
        create: unknown[];
        opening: Array<{ id: string; body: unknown }>;
    } = {
        create: [],
        opening: [],
    };
...
describe("AddAccountDialog (integration, WP-B5 §3 F4+F7)", () => {
    ...
    it("blocks account creation when a non-empty opening balance is invalid", async () => {
        ...
        await user.type(screen.getByLabelText(/^name$/i), "KBC Checking");
        await user.type(
            screen.getByLabelText(/opening balance/i),
            "not-a-number",
        );
    ...
    it("relabels the opening balance to 'Outstanding debt' for liability accounts (§3 F4)", async () => {
        ...
        await user.click(
            await screen.findByRole("option", { name: "Liability" }),
        );
    ...
    it("shows the name helper text and no longer renders the consumer-less fields (§3 F7)", async () => {
        ...
        await openCreateDialog(user);
        expect(
            screen.getByText(/must match your bank import label/i),
        ).toBeInTheDocument();
        ...
        expect(
            await screen.findByText(/whose money this is/i),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/how quickly this money could be spent/i),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/counts toward day-to-day spendable cash/i),
        ).toBeInTheDocument();
        expect(screen.queryByText(/tax wrapper/i)).not.toBeInTheDocument();
        expect(
            screen.queryByText(/holds a cash balance/i),
        ).not.toBeInTheDocument();
        expect(
            screen.queryByText(/multi-currency cash/i),
        ).not.toBeInTheDocument();
    });
