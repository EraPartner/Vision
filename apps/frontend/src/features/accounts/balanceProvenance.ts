 ...
 * (COMPUTED_BALANCE_LATERAL, ADR-094): the most recent stamped bank-statement
 ...
 */
export function useBalanceProvenance(): (fields: BalanceProvenanceFields) => string | null {
    const { tc } = useLanguage();
    const { appSettings } = useAppSettings();

    return useCallback(
        ({ anchor_date, post_anchor_count }: BalanceProvenanceFields): string | null => {
            ...
            if (anchor_date) {
                ...
                return tc('accounts.provenance.anchored', post_anchor_count, {
                    date: formatDateStringWithAppSettings(anchor_date, appSettings.dateFormat),
                });
            }
