...
export function InvestmentCombobox({
    value,
    onSelect,
    disabled,
    className,
    ...
    const investments = useMemo(
        () => (data?.items ?? []).filter((investment) => investment.is_active),
        [data?.items],
    );
    const selected = useMemo(
        () => investments.find((i) => i.id === value),
        [investments, value],
    );
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            ...
            >
                <Command>
                    <CommandInput
                        placeholder={t("combobox.investment.search")}
                    />
                    <CommandList>
                        <CommandEmpty>
                            {t("combobox.investment.empty")}
                        </CommandEmpty>
                        <CommandGroup>
                            {investments.map((inv) => {
                                const label = inv.symbol
                                    ? `${inv.name} (${inv.symbol})`
                                    : inv.name;
                                return (
                                    <CommandItem
