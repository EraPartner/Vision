...
describe("addPortfolioTxnSchema", () => {
    ...
    test("buy with units + price derives the amount", () => {
        const result = addPortfolioTxnSchema(buy).safeParse({
            ...blank,
            units: "10",
            pricePerUnit: "90",
        });
    ...
    test("non-buy/sell types require an amount; zero counts as missing on Add", () => {
        expect(
            firstIssue(addPortfolioTxnSchema(dividend).safeParse(blank)),
        ).toBe("addPortTxn.error.amountRequired");
        expect(
            firstIssue(
                addPortfolioTxnSchema(dividend).safeParse({
                    ...blank,
                    amount: "0",
                }),
            ),
    ...
    test("gift requires units, forces amount/fees/taxes to 0", () => {
        ...
        const result = addPortfolioTxnSchema(gift).safeParse({
            ...blank,
            units: "3",
            fees: "2",
            taxes: "1",
...
describe("editPortfolioTxnSchema", () => {
    ...
    test("a non-gift amount of 0 is rejected on Edit (stricter than Add's parser path)", () => {
        expect(
            firstIssue(
                editPortfolioTxnSchema(dividend).safeParse({
                    ...blank,
                    amount: "0",
                }),
            ),
    ...
    test("a cleared date is rejected, after the amount rule", () => {
        ...
        ).toBe("addPortTxn.error.dateRequired");
    });

    test("gift with units passes; amount may stay undefined and fees/taxes are forced to 0", () => {
        const result = editPortfolioTxnSchema(gift).safeParse({
            ...blank,
            units: "3",
            fees: "2",
        });
    ...
    test("cleared fees/taxes become 0 and cleared FX becomes null (PATCH must send the clear)", () => {
        const result = editPortfolioTxnSchema(dividend).safeParse({
            ...blank,
            amount: "50",
        });
        expect(result.success).toBe(true);
