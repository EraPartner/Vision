...
describe("WatchlistChartDialog", () => {
    ...
    it("shows every localized research range", async () => {
        // Arrange
        setupDefaultChartHandlers();
        const onOpenChange = vi.fn();
        renderWithApp(
    ...
    it.each(["Enter", "save button"] as const)(
        "can update target price with %s",
        async (submitMethod) => {
            // Arrange
            ...
            const targetPriceButton = await screen.findByRole("button", {
                name: /200/i,
            });
    ...
    it("cancels target-price editing without submitting", async () => {
        let patchCount = 0;
        server.use(
            http.get(`${API_BASE}/api/market/chart`, () => ok(CHART_RESPONSE)),
            http.get(`${API_BASE}/api/market/quote`, () => ok(QUOTE_RESPONSE)),
