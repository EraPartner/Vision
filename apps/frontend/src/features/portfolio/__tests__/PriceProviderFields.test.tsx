...
import {
    PriceProviderFields,
    type PriceProviderFormShape,
} from "../PriceProviderFields";
...
function ProviderFieldsHarness() {
    ...
    return (
        <PriceProviderFields
            ...
            showManualPrice
            t={(key) =>
                key === "addInv.provider.manual" ? "Manual translated" : key
            }
        />
    );
}
...
describe("PriceProviderFields catalog", () => {
    it("renders backend catalog membership and order, including a future provider key", async () => {
                ...
                ok({
                    providers: [
                        ...
                        },
                        {
                            key: "server_only",
                            name: "Server Feed",
                            description: "Backend supplied hint",
                        },
                        {
                            key: "yahoo",
                            name: "Yahoo Finance",
        ...
        await waitFor(() => {
            expect(
                queryClient.getQueryData(["investment-providers"]),
            ).toHaveLength(3);
        });
        ...
        expect(
            screen.getByRole("option", {
                name: /Server Feed.*Backend supplied hint/i,
            }),
        ).toBeInTheDocument();
        expect(
            screen.getByRole("option", {
                name: /Yahoo Finance.*addInv.provider.hint.yahoo/i,
            }),
        ).toBeInTheDocument();
    });
    ...
    it("keeps the fallback catalog usable when the catalog request fails", async () => {
        ...
        await waitFor(() => {
            expect(
                queryClient.getQueryState(["investment-providers"])?.status,
            ).toBe("error");
        });
        expect(screen.getByRole("combobox")).toHaveTextContent(
            "Manual translated",
        );
        const price = screen.getByLabelText("addInv.label.currentPrice");
