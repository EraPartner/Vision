...
const isIncluded = (inv: InvestmentSummary): boolean =>
    inv.show_in_ticker !== false;
...
export function PortfolioTicker({ items }: PortfolioTickerProps) {
    ...
    const entries = useMemo<TickerEntry[]>(() => {
        ...
        for (const inv of included) {
            ...
            out.push({
                ...
                pctLabel: formatPercent(changePercent, {
                    digits: 2,
                    signed: true,
                }),
            });
    ...
    const track = useMemo(() => {
        ...
        const durationS = Math.max(
            MIN_DURATION_S,
            entries.length * SECONDS_PER_ITEM,
        );
        const renderTiles = (interactive: boolean) =>
            entries.map((e) => (
                <div
                    key={`${interactive ? "primary" : "duplicate"}-${e.key}`}
...
function TickerManager({ holdings }: { holdings: InvestmentSummary[] }) {
    ...
    const toggle = useMutation({
        ...
        onMutate: async ({ id, show }) => {
            ...
            const prev = queryClient.getQueryData<InvestmentsListResponse>(
                portfolioKeys.investments,
            );
            if (prev) {
                queryClient.setQueryData<InvestmentsListResponse>(
                    portfolioKeys.investments,
                    {
                        ...prev,
                        items: prev.items.map((it) =>
                            it.id === id ? { ...it, show_in_ticker: show } : it,
                        ),
                    },
                );
            }
        ...
        onError: (err, _vars, ctx) => {
            if (ctx?.prev)
                queryClient.setQueryData(portfolioKeys.investments, ctx.prev);
                        ...
                        >
                            ...
                            <Switch
                                ...
                                onCheckedChange={(checked) =>
                                    toggle.mutate({ id: h.id, show: checked })
                                }
                                aria-label={h.name}
                            />
                        </div>
