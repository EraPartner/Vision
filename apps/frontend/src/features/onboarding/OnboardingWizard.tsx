...
import { requestDashboardArrival } from "@/utils/dashboardArrival";
import {
...
function bankMonogram(adapter: { key: string; name: string }): string {
    ...
    const fromName =
        first.length >= 2 ? first.slice(0, 2) : first + second.slice(0, 1);
    const mono =
        fromName ||
        (adapter.key ?? "").replace(/[^\p{L}\p{N}]/gu, "").slice(0, 2);
...
const SUGGESTED_CATEGORIES = [
    {
        general: "FOOD",
        detail: "Groceries",
        detailKey: "onboarding.cat.groceries",
        emoji: "🛒",
    },
    {
        general: "FOOD",
        detail: "Restaurants",
        detailKey: "onboarding.cat.restaurants",
        emoji: "🍽️",
    },
    {
        general: "HOUSING",
        detail: "Rent",
        detailKey: "onboarding.cat.rent",
        emoji: "🏠",
    },
    {
        general: "HOUSING",
        detail: "Utilities",
        detailKey: "onboarding.cat.utilities",
        emoji: "💡",
    },
    {
        general: "TRANSPORT",
        detail: "Fuel",
        detailKey: "onboarding.cat.fuel",
        emoji: "⛽",
    },
    {
        general: "TRANSPORT",
        detail: "Public Transport",
        detailKey: "onboarding.cat.publicTransport",
        emoji: "🚌",
    },
    {
        general: "ENTERTAINMENT",
        detail: "Subscriptions",
        detailKey: "onboarding.cat.subscriptions",
        emoji: "📺",
    },
    {
        general: "ENTERTAINMENT",
        detail: "Hobbies",
        detailKey: "onboarding.cat.hobbies",
        emoji: "🎮",
    },
    {
        general: "HEALTH",
        detail: "Insurance",
        detailKey: "onboarding.cat.insurance",
        emoji: "🏥",
    },
    {
        general: "HEALTH",
        detail: "Pharmacy",
        detailKey: "onboarding.cat.pharmacy",
        emoji: "💊",
    },
    {
        general: "INCOME",
        detail: "Salary",
        detailKey: "onboarding.cat.salary",
        emoji: "💰",
    },
    {
        general: "INCOME",
        detail: "Freelance",
        detailKey: "onboarding.cat.freelance",
        emoji: "💻",
    },
    {
        general: "SAVINGS",
        detail: "Investments",
        detailKey: "onboarding.cat.investments",
        emoji: "📈",
    },
    {
        general: "PERSONAL",
        detail: "Clothing",
        detailKey: "onboarding.cat.clothing",
        emoji: "👕",
    },
    {
        general: "PERSONAL",
        detail: "Gifts",
        detailKey: "onboarding.cat.gifts",
        emoji: "🎁",
    },
] as const;
...
export function OnboardingWizard({
    ...
    const STEPS = [
        {
            key: "welcome",
            label: t("onboarding.step.welcome.label"),
            icon: Sparkles,
        },
        {
            key: "overview",
            label: t("onboarding.step.overview.label"),
            icon: LayoutDashboard,
        },
        {
            key: "categories",
            label: t("onboarding.step.categories.label"),
            icon: Tags,
        },
        { key: "bank", label: t("onboarding.step.bank.label"), icon: Wallet },
        {
            key: "import",
            label: t("onboarding.step.import.label"),
            icon: Upload,
        },
        {
            key: "tour",
            label: t("onboarding.step.tour.label"),
            icon: BarChart3,
        },
        {
            key: "backup",
            label: t("onboarding.step.backup.label"),
            icon: HardDrive,
        },
    ];
    ...
    const FEATURES = [
        {
            icon: Receipt,
            title: t("onboarding.feature.transactions.title"),
            desc: t("onboarding.feature.transactions.desc"),
            path: "/transactions",
        },
        {
            icon: Tags,
            title: t("onboarding.feature.categories.title"),
            desc: t("onboarding.feature.categories.desc"),
            path: "/categories",
        },
        {
            icon: CalendarClock,
            title: t("onboarding.feature.planned.title"),
            desc: t("onboarding.feature.planned.desc"),
            path: "/planned",
        },
        {
            icon: BarChart3,
            title: t("onboarding.feature.statistics.title"),
            desc: t("onboarding.feature.statistics.desc"),
            path: "/statistics",
        },
        {
            icon: TrendingUp,
            title: t("onboarding.feature.portfolio.title"),
            desc: t("onboarding.feature.portfolio.desc"),
            path: "/portfolio",
        },
        {
            icon: LineChart,
            title: t("onboarding.feature.market.title"),
            desc: t("onboarding.feature.market.desc"),
    ...
    const draftStorageFailureShown = useRef(false);

    // Rehydrate on every closed -> open transition. The wizard remains mounted
    // after completion, so mount-only initialization would make Settings'
    // "Restart onboarding" action reopen stale in-memory values.
    useEffect(() => {
        ...
        const draft = readOnboardingDraft(SUGGESTED_CATEGORIES.length);
        setStep(draft.step);
        setSelectedBank(draft.selectedBank);
        setSelectedCategories(new Set(draft.selectedCategoryIndexes));
        setCategoriesCreated(draft.categoriesCreated);
        setImportResult(draft.importResult);
        setReviewBatch(draft.reviewBatch);
        // File objects and transient request state are intentionally never
        // serialized. A restored import step always asks for the file again.
        setFile(null);
        setImporting(false);
    ...
    useEffect(() => {
        ...
        const saved = writeOnboardingDraft({
            version: 1,
            step,
            selectedBank,
            selectedCategoryIndexes: Array.from(selectedCategories).sort(
                (a, b) => a - b,
            ),
            categoriesCreated,
            importResult,
        ...
        if (!saved && !draftStorageFailureShown.current) {
            const message = t("onboarding.draftStorageFailed");
            // Locale chunks load asynchronously. Wait for the translated copy
            // instead of exposing the raw key during the first render.
            if (message === "onboarding.draftStorageFailed") return;
            draftStorageFailureShown.current = true;
            toast.error(message);
        } else if (saved) {
            draftStorageFailureShown.current = false;
        }
    }, [
        open,
        draftReady,
    ...
    // Adapter keys are runtime data and may disappear between releases. Clear
    // a stale selection after the catalog arrives. An unfinished import cannot
    // proceed without a valid adapter, so return it to the bank step; completed
    // and review outcomes remain visible without the original adapter.
    useEffect(() => {
        if (
            !open ||
            !draftReady ||
            ...
            return;
        if (adapters.some((adapter) => adapter.key === selectedBank)) return;
        setSelectedBank("");
        if (step === "import" && !importResult && !reviewBatch) {
            setStep("bank");
        }
    }, [
        open,
    ...
    const goNext = () => {
        if (stepIdx < STEP_KEYS.length - 1) setStep(STEP_KEYS[stepIdx + 1]);
    };
    const goBack = () => {
        ...
        try {
            ...
            if (isReviewRequired(result)) {
                const rows = Object.values(
                    result.match_source_counts ?? {},
                ).reduce((sum, n) => sum + (n ?? 0), 0);
                setReviewBatch({ batchId: result.batch_id, rows });
                toast.info(
                    t("onboarding.toast.reviewRequired", { n: String(rows) }),
                );
                return;
            }
            setImportResult({
            ...
            toast.success(
                t("onboarding.toast.imported", { n: String(result.imported) }),
            );
        } catch (err: unknown) {
            toast.error(
                t("onboarding.toast.importFailed", {
        ...
        try {
            const results = await Promise.allSettled(
                Array.from(selectedCategories).map((idx) => {
                    ...
                    return apiClient.createCategory({
                        general: cat.general,
                        detail: cat.detail,
                    });
                }),
            ...
            if (rejected) {
                toast.error(
                    t("onboarding.toast.categoriesFailed", {
            ...
            toast.success(
                t("onboarding.toast.categoriesCreated", { n: String(created) }),
            );
        } catch (err: unknown) {
            toast.error(
                t("onboarding.toast.categoriesFailed", {
     ...
     */
    const goToReview = (batchId: number) =>
        handleNavigate(`/import/${batchId}/review`);
                    ...
                    <div className="px-6 py-6 min-h-[340px] flex flex-col">
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-center gap-4 animate-in">
                                ...
                                <div className="flex gap-2 mt-2">
                                    ...
                                    >
                                        ...
                                        {t("onboarding.step.categories.label")}
                                    </Badge>
                                    <Badge
                                        variant="secondary"
                                        className="gap-1"
                        ...
                        {step === "import" && (
                            <div className="flex-1 flex flex-col gap-4">
                                ...
                                {importResult ? (
                                    <StepSuccess
                                        title={t("onboarding.import.success", {
                                            n: String(importResult.imported),
                                        })}
                                        subtitle={
                                            importResult.duplicates > 0
                                                ? t(
                                                      "onboarding.import.duplicates",
                                                      {
                                                          n: String(
                                    ...
                                    <StepNeedsReview
                                        title={t(
                                            "onboarding.import.review.title",
                                            { n: String(reviewBatch.rows) },
                                        )}
                                        desc={t(
                                            "onboarding.import.review.desc",
                                        )}
                                        cta={t("onboarding.import.review.cta")}
                                        later={t(
                                            "onboarding.import.review.later",
                                        )}
                                        onReview={() =>
                                            goToReview(reviewBatch.batchId)
                                        }
                                    />
                                ) : (
                                    <>
                                    ...
                                    {[
                                        {
                                            icon: ShieldCheck,
                                            labelKey:
                                                "onboarding.backup.reason.safe",
                                        },
                                        {
                                            icon: HardDrive,
                                            labelKey:
                                                "onboarding.backup.reason.local",
                                        },
                                        {
                                            icon: FolderOpen,
                                            labelKey:
                                                "onboarding.backup.reason.restore",
                                        },
