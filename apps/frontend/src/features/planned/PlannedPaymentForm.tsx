...
export default function PlannedPaymentForm({
    open,
    onOpenChange,
    onSubmit,
    initial,
    ...
    const [currency, setCurrency] = useState(
        initial?.currency ?? appSettings.defaultCurrency,
    );
    ...
    const [dueDate, setDueDate] = useState<Date | undefined>(
        initial?.due_date
            ? parseLocalDateFromYmd(initial.due_date)
            : new Date(),
    );
    ...
    const [loanPaymentDay, setLoanPaymentDay] = useState(
        initial?.loan_payment_day?.toString() ?? "",
    );
    const [customDays, setCustomDays] = useState(
        initial?.custom_interval_days?.toString() ?? "",
    );
    const [endDate, setEndDate] = useState<Date | undefined>(
        initial?.end_date ? parseLocalDateFromYmd(initial.end_date) : undefined,
    );
    const [maxOccurrences, setMaxOccurrences] = useState(
        initial?.max_occurrences?.toString() ?? "",
    );
    ...
    const initialFingerprint = JSON.stringify({
        ...
        isRecurring: initial?.is_recurring ?? false,
        ...
        maxOccurrences: initial?.max_occurrences?.toString() ?? "",
    ...
    useUnsavedChanges(open && formFingerprint !== initialFingerprint);
    ...
    const fieldErrors: FieldErrorMap = {
        ...
        "pp-loan-rate":
            isLoan && !loanRate ? t("validation.required") : undefined,
        "pp-loan-term": !isLoan
            ? undefined
            : !loanTermMonths
            ...
            ...(!isLoan &&
                isRecurring && {
                    ...
                    ...(frequency === "custom" && customDays
                        ? { custom_interval_days: parseInt(customDays) }
                        : {}),
            ...
            and footer in a real <form> (Enter-to-submit) is layout-neutral. */}
                <form onSubmit={handleSubmit} className="grid gap-5" noValidate>
                    <div className="grid gap-4 py-2">
                        ...
                        <div className="grid gap-1.5">
                            <Label htmlFor="pp-name">
                                {t("plannedForm.nameRequired2")}
                            </Label>
                            <Input
                                id="pp-name"
                                placeholder={t("plannedForm.namePlaceholder")}
                                value={name}
                                onChange={(e) => setName(e.target.value)}
                                {...fieldErrorProps(
                                    "pp-name",
                                    visibleErrors["pp-name"],
                                )}
                            />
                            <FieldError
                                field="pp-name"
                                message={visibleErrors["pp-name"]}
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                            <div className="grid gap-1.5 sm:col-span-2">
                                ...
                                <Input
                                    ...
                                    {...fieldErrorProps(
                                        "pp-amount",
                                        visibleErrors["pp-amount"],
                                    )}
                                />
                                <FieldError
                                    field="pp-amount"
                                    message={visibleErrors["pp-amount"]}
                                />
                            </div>
                            <div className="grid gap-1.5">
                                <Label htmlFor="pp-currency">
                                    {t("plannedForm.currency")}
                                </Label>
                                <Select
                                    value={currency}
                  ...
                  overwrite their own accessible names with "Direction". */}
                        ...
                        {/* Due date */}
                        <div className="grid gap-1.5">
                            <Label htmlFor="pp-due-date">
                                {t("plannedForm.dueDate")}
                            </Label>
                            <DatePicker
                                id="pp-due-date"
                                value={dueDate}
                                ...
                                {...fieldErrorProps(
                                    "pp-due-date",
                                    visibleErrors["pp-due-date"],
                                )}
                            />
                            <FieldError
                                field="pp-due-date"
                                message={visibleErrors["pp-due-date"]}
                            />
                        </div>

                        {/* Recipient */}
                        <div className="grid gap-1.5">
                            <Label htmlFor="pp-recipient">
                                {t("plannedForm.recipient")}
                            </Label>
                            <RecipientCombobox
                                id="pp-recipient"
                                value={recipientId ?? null}
                                onSelect={(id) =>
                                    setRecipientId(id ?? undefined)
                                }
                                className="w-full"
                                disabled={loading}
                                portalContainer={portalContainer}
                  ...
                  role="combobox" <div>, which a <label for> cannot associate with. */}
                        ...
                        {/* Bank account */}
                        <div className="grid gap-1.5">
                            <Label htmlFor="pp-bank">
                                {t("plannedForm.bankAccountRequired")}
                            </Label>
                            <AccountCombobox
                                id="pp-bank"
                                value={bankAccount}
                                ...
                                {...fieldErrorProps(
                                    "pp-bank",
                                    visibleErrors["pp-bank"],
                                )}
                            />
                            <FieldError
                                field="pp-bank"
                                message={visibleErrors["pp-bank"]}
                            />
                        </div>
                        ...
                        {isLoan && (
                            <div className="grid gap-3 rounded-lg border p-3 bg-muted/30">
                                <div className="grid gap-1.5">
                                    ...
                                    >
                                        <SelectTrigger id="pp-loan-type">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem value="amortizing">
                                                {t(
                                                    "plannedForm.loanType.amortizing",
                                                )}
                                            </SelectItem>
                                            <SelectItem value="fixed_principal">
                                                {t(
                                                    "plannedForm.loanType.fixedPrincipal",
                                                )}
                                            </SelectItem>
                                            <SelectItem value="interest_only">
                                                {t(
                                                    "plannedForm.loanType.interestOnly",
                                                )}
                                            </SelectItem>
                                        </SelectContent>
                                    </Select>
                                ...
                                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                    <div className="grid gap-1.5">
                                        <Label htmlFor="pp-loan-principal">
                                            {t("plannedForm.loanPrincipal")}
                                        </Label>
                                        <Input
                                    ...
                                    <div className="grid gap-1.5">
                                        <Label htmlFor="pp-loan-rate">
                                            {t("plannedForm.loanRate")}
                                        </Label>
                                        <Input
                                            id="pp-loan-rate"
                                            type="text"
                                ...
                                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                    <div className="grid gap-1.5">
                                        <Label htmlFor="pp-loan-term">
                                            {t("plannedForm.loanTermMonths")}
                                        </Label>
                                        <Input
                                            id="pp-loan-term"
                                            type="number"
                                    ...
                                    <div className="grid gap-1.5">
                                        <Label htmlFor="pp-loan-payment-day">
                                            {t("plannedForm.loanPaymentDay")}
                                        </Label>
                                        <Input
                                            id="pp-loan-payment-day"
                                            type="number"
                                            min={1}
                                            max={31}
                        ...
                        <div className="flex items-center justify-between rounded-lg border p-3">
                            ...
                            <Switch
                                id="pp-recurring"
                                checked={isLoan ? true : isRecurring}
                        ...
                        {!isLoan && isRecurring && (
                            <div className="grid gap-3 rounded-lg border p-3 bg-muted/30">
                                <div className="grid gap-1.5">
                                    ...
                                    >
                                        <SelectTrigger id="pp-frequency">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem value="daily">
                                                {t("plannedForm.freq.daily")}
                                            </SelectItem>
                                            <SelectItem value="weekly">
                                                {t("plannedForm.freq.weekly")}
                                            </SelectItem>
                                            <SelectItem value="biweekly">
                                                {t("plannedForm.freq.biweekly")}
                                            </SelectItem>
                                            <SelectItem value="monthly">
                                                {t("plannedForm.freq.monthly")}
                                            </SelectItem>
                                            <SelectItem value="quarterly">
                                                {t(
                                                    "plannedForm.freq.quarterly",
                                                )}
                                            </SelectItem>
                                            <SelectItem value="yearly">
                                                {t("plannedForm.freq.yearly")}
                                            </SelectItem>
                                            <SelectItem value="custom">
                                                {t("plannedForm.freq.custom")}
                                            </SelectItem>
                                        </SelectContent>
                                    </Select>
                                </div>
                                ...
                                {frequency === "custom" && (
                                    <div className="grid gap-1.5">
                                        <Label htmlFor="pp-custom-days">
                                            {t("plannedForm.repeatEvery")}
                                        </Label>
                                        <Input
                                            id="pp-custom-days"
                                            type="number"
                                            min={1}
                                            placeholder={t(
                                                "plannedForm.customDaysPlaceholder",
                                            )}
                                            value={customDays}
                                            onChange={(e) =>
                                            ...
                                            {...fieldErrorProps(
                                                "pp-custom-days",
                                                visibleErrors["pp-custom-days"],
                                            )}
                                        />
                                        <FieldError
                                            field="pp-custom-days"
                                            message={
                                                visibleErrors["pp-custom-days"]
                                            }
                                        />
                                ...
                                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                    ...
                                    <div className="grid gap-1.5">
                                        <Label htmlFor="pp-max">
                                            {t("plannedForm.maxOccurrences")}
                                        </Label>
                                        <Input
                                            id="pp-max"
                                            type="number"
                        ...
                        {/* Notes */}
                        <div className="grid gap-1.5">
                            <Label htmlFor="pp-notes">
                                {t("plannedForm.notes")}
                            </Label>
                            <Textarea
                                id="pp-notes"
                                placeholder={t("plannedForm.notesPlaceholder2")}
                                value={notes}
                        ...
                        {/* Optional Link */}
                        <div className="grid gap-1.5">
                            <Label htmlFor="pp-url">
                                {t("plannedForm.link")}
                            </Label>
                            <Input
                                id="pp-url"
                                placeholder={t("plannedForm.linkPlaceholder")}
                                value={url}
