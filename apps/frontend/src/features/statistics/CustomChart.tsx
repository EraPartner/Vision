...
function parseDate(period: string, bucket: "monthly" | "yearly"): Date {
    try {
        ...
        return bucket === "yearly"
            ? parseISO(`${period.slice(0, 4)}-01-01`)
            : parseISO(`${period}-01`);
    } catch {
        return new Date();
    }
...
export function CustomChart({
    ...
    const hasRecipients =
        savedChart.recipient_ids.length > 0 || savedChart.all_recipients;
    ...
    const catData = useMemo(() => {
        ...
        const filtered = data.categoryPivot.filter(
            (c) =>
                c.categoryId !== null &&
                (allCategories || selected.has(c.categoryId)),
        );
    ...
    const allPeriods = useMemo(() => {
        ...
        if (savedChart.date_range_start) {
            ...
            periods = periods.filter(
                (p) => p >= (p.length === 4 ? s.slice(0, 4) : s.slice(0, 7)),
            );
        }
        if (savedChart.date_range_end) {
            const e = savedChart.date_range_end;
            periods = periods.filter(
                (p) => p <= (p.length === 4 ? e.slice(0, 4) : e.slice(0, 7)),
            );
        }
        ...
        recipientData,
        tagData,
        savedChart.date_range_start,
        savedChart.date_range_end,
    ]);
    ...
    const entities = useMemo<Entity[]>(() => {
        ...
        for (const cat of catData)
            result.push(
                build(`cat:${cat.categoryId}`, cat.categoryName, cat.months),
            );
        for (const rec of recipientData)
            result.push(build(`rec:${rec.recipientId}`, rec.name, rec.months));
        for (const tag of tagData)
            result.push(build(`tag:${tag.tagId}`, `#${tag.slug}`, tag.months));
        return result;
    ...
    const displayEntities = useMemo<Entity[]>(() => {
        ...
        for (const e of rest) {
            ...
            for (const p of allPeriods)
                months[p] = (months[p] ?? 0) + (e.months[p] ?? 0);
        }
        return [
            ...top,
            {
                key: "other",
                label: t("customChart.other"),
                months,
                total,
                isOther: true,
            },
    ...
    const seriesMeta = useMemo<SeriesMeta[]>(
        () =>
            displayEntities.map((e, i) => ({
                key: e.key,
                label: e.label,
                color: e.isOther ? OTHER_COLOR : getChartColor(i),
            })),
        [displayEntities],
    );
    ...
    const chartData = useMemo<ChartDatum[]>(() => {
        return allPeriods.map((period) => {
            ...
            for (const e of displayEntities)
                values[e.key] = e.months[period] ?? 0;
            return {
                period,
        ...
        () => [
            {
                ...
                accessor: (e: Entity) => e.total,
            },
        ],
        [t],
    );
    const yTick = formatAxisCompact;
    const xTickFmt = (v: unknown) =>
        formatDate(
            v as Date,
            bucket === "yearly"
                ? CHART_DATE_PATTERNS.yearTick
                : CHART_DATE_PATTERNS.monthTick,
            monthLabelLocale,
        );
                    ...
                    <div className="space-y-3">
                        {isRanked ? (
                            <BarChart<Entity>
                                ...
                                colorForIndex={(i) =>
                                    seriesMeta[i]?.color ?? getChartColor(i)
                                }
                            />
                        ) : chartType === "bar" &&
                          chartVariant === "stacked" ? (
                            <StackedBarChart<ChartDatum>
                                ...
                                series={
                                    barSeries as StackedBarSeries<ChartDatum>[]
                                }
                                height={350}
                                valueTickFormat={yTick}
