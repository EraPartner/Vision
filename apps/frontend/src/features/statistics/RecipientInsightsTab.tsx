...
export function RecipientInsightsTab({
    ...
    const { excludedCategoryIds, excludedRecipientIds, exclusionsApply } =
    ...
    const increases = filteredData.monthOverMonth.filter(
        (m) => m.changePercent > 0,
    );
    const decreases = filteredData.monthOverMonth.filter(
        (m) => m.changePercent < 0,
    );
        ...
        <div className="space-y-6">
            {effectiveExcludedRecIds.length > 0 && (
                <Badge variant="secondary" className="gap-1.5">
                    ...
                    {t("insights.excluded", {
                        n: effectiveExcludedRecIds.length,
                    })}
                </Badge>
            )}
            ...
            <div className="grid gap-4 md:grid-cols-3">
                <StatCard
                    ...
                    subtitle={
                        top10[0] ? (
                            <Money amount={top10[0].totalSpend} />
                        ) : (
                            t("insights.noDataFallback")
                        )
                    }
                />
                <StatCard
