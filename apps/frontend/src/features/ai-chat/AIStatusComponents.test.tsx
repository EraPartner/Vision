...
describe("AI status copy", () => {
  it("uses the localized fallback for a tool failure without detail", async () => {
    renderWithApp(<ToolResultCard toolName="portfolio_lookup" result={{ ok: false }} />);

    expect(await screen.findByText("portfolio_lookup: Tool failed.")).toBeInTheDocument();
  });
  ...
  it("does not expose a raw Ollama error as the primary user hint", async () => {
    ...
    expect(await screen.findByText(/install ollama and start it locally/i)).toBeInTheDocument();
