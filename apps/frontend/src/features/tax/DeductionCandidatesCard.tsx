...
import { DEDUCTION_TYPE_PROFILE_FIELDS, type BelgianTaxProfile } from '@/lib/belgianTax';
...
export function DeductionCandidatesCard() {
    ...
    const [appliedTypes, setAppliedTypes] = useState<ReadonlySet<string>>(new Set());
    ...
    const handleConfirm = (group: DeductionTypeGroup) => {
        ...
        const updates: Record<string, number | boolean> = { [mapping.amountField]: group.total };
    ...
    return (
        <Card>
            <CardHeader className="pb-3">
                ...
                <p className="text-xs text-muted-foreground">{t('tax.deductionCandidates.disclaimer')}</p>
            </CardHeader>
            <CardContent className="space-y-3">
                {groups.map((group) => {
                    ...
                    return (
                        <div key={group.deductionType} className="rounded-lg border bg-card p-3">
                            <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0 flex-1">
                                    <p className="text-sm font-semibold text-foreground">
                                        {t(`tax.deductionCandidates.type.${group.deductionType}`)}
                                    </p>
                                    <p className="text-xs text-muted-foreground mt-0.5">
                                        {t('tax.deductionCandidates.fromCategories', { count: group.categoryCount })}
                                    </p>
                                    <ul className="mt-1 space-y-0.5">
                                        {group.categories.map((cat) => (
                                            ...
                                            >
                                                ...
                                                <span className="shrink-0">{fmt(cat.total, currency)}</span>
                                            </li>
                                        ))}
                                    </ul>
                                    <p className="text-xs text-muted-foreground mt-1.5">
                                        {t('tax.deductionCandidates.currentValue', { value: fmt(currentValue, currency) })}
                                    </p>
                                </div>
                                <div className="shrink-0 flex flex-col items-end gap-1.5 text-right">
                                    <span className="text-sm font-bold text-foreground">{fmt(group.total, currency)}</span>
                                    {appliedTypes.has(group.deductionType) ? (
                                        <Badge variant="outline" className="text-xs text-accent border-accent/30 bg-accent/10">
                                            ...
                                            {t('tax.deductionCandidates.applied')}
                                        </Badge>
                                    ) : (
                                        <div className="flex items-center gap-1">
                                            ...
                                            >
                                                {t('tax.deductionCandidates.confirm')}
                                            </Button>
                                            <Button
                                                size="sm"
                                                variant="ghost"
                                                className="h-7 text-xs text-muted-foreground"
                                                onClick={() => handleDismiss(group.deductionType)}
                                            >
                                                {t('tax.deductionCandidates.dismiss')}
                                            </Button>
                                        </div>
                                    )}
                                </div>
