...
export function MonthlyIncomeTaxCard({ data, isLoading, hasIncomeSources, viewedYear }: MonthlyIncomeTaxCardProps) {
  const { t } = useLanguage();
      ...
      <CardContent>
        {isLoading ? (
          ...
          <BarChart
            ...
            categoryTickFormat={(label) => formatMonthTick(label, data[0]?.period ?? label)}
            height={280}
