...
const addTransactionSchema = createAddTransactionSchema("eu");
...
describe("addTransactionSchema", () => {
    ...
    function issueFor(
        form: ReturnType<typeof validForm>,
        path: string,
    ): string | undefined {
    ...
    test("accepts the selected EU format and negative amounts", () => {
        ...
        expect(
            addTransactionSchema.safeParse({
                ...validForm(),
                amount: "1,234.56",
            }).success,
        ).toBe(false);
    ...
    test("empty date, amount, and recipient report the required key", () => {
        expect(
            issueFor(
                { ...validForm(), transaction_date: "" },
                "transaction_date",
            ),
        ).toBe("validation.required");
        expect(issueFor({ ...validForm(), amount: "" }, "amount")).toBe(
            "validation.required",
        );
        expect(
            issueFor({ ...validForm(), recipient_id: "" }, "recipient_id"),
        ).toBe("validation.required");
    });
    ...
    test("unparseable and zero amounts report their dedicated keys", () => {
        expect(issueFor({ ...validForm(), amount: "abc" }, "amount")).toBe(
            "addTxn.invalidAmount",
        );
        expect(issueFor({ ...validForm(), amount: "0" }, "amount")).toBe(
            "addTxn.zeroAmount",
        );
        expect(issueFor({ ...validForm(), amount: "0,00" }, "amount")).toBe(
            "addTxn.zeroAmount",
        );
    });
