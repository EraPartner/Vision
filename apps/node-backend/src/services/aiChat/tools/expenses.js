...
function fetchTransactionsInRange(
  cache,
  { from, to, limit = MAX_ROWS, categoryId = null, recipientId = null },
) {
...
async function loadCanonicalCashflow(cache, from, to, groupBy) {
  ...
  const monthRows = Array.isArray(result) ? result : result.months;
  const buckets = new Map();
  for (const month of monthRows) {
    const monthKey = `${month.year}-${String(month.month).padStart(2, "0")}`;
    const key =
      groupBy === "quarter" ? bucketKey(`${monthKey}-01`, "quarter") : monthKey;
    ...
    entry.income = entry.income.plus(month.total_income ?? 0);
    entry.spend = entry.spend.plus(toDecimal(month.total_spending ?? 0).abs());
    entry.net = entry.net.plus(month.net_amount ?? 0);
    entry.count += Number(month.transaction_count ?? 0);
  ...
  return {
    ...
    rows: [...buckets.values()]
      ...
      .map((entry) => ({
        ...
        spend: roundToCents(entry.spend).toNumber(),
        net: roundToCents(entry.net).toNumber(),
        count: entry.count,
      })),
  };
}
...
export const getSpendByCategory = {
  ...
  description:
    'Total outgoing spend grouped by category within a date range. Use for "biggest category", "what did I spend on groceries", etc.',
  parameters: {
    type: "object",
    properties: {
      ...
      topN: {
        type: "integer",
        description: "Limit to top N categories. Default 10.",
        minimum: 1,
        maximum: 100,
      },
    },
...
export const getMonthlySpend = {
  ...
  description:
    'Income, spend and net totals bucketed by month or quarter. Use for trend questions like "how did my spending change this year".',
  parameters: {
    type: "object",
    properties: {
  ...
  async run(
    ...
    const to = requireDate(args.to, "to");
    assertDateOrder(from, to);
    const groupBy = parseEnum(args.groupBy, "groupBy", ["month", "quarter"], {
      defaultValue: "month",
    });
    ...
    return {
      ok: true,
      data: canonical.rows.slice(0, maxRows),
      meta: {
        from,
        to,
        groupBy,
...
export const getTopRecipients = {
  ...
  description:
    'Recipients ranked by total outgoing spend within a date range. Use for "who do I pay the most", "top merchants", etc.',
  parameters: {
    type: "object",
    properties: {
      from: { type: "string", description: "ISO date, inclusive (YYYY-MM-DD)" },
      ...
      topN: {
        type: "integer",
        description: "Limit to top N recipients. Default 10.",
        minimum: 1,
        maximum: 100,
      },
    },
  ...
  async run(
    ...
    for (const row of rows) {
      ...
      const entry = byRecipient.get(label) || {
        recipient: label,
        total: toDecimal(0),
        count: 0,
      };
...
export const getTransactionsInRange = {
  ...
  description:
    "List raw transactions in a date range, optionally filtered by category or recipient. Use when user asks to see individual transactions, not an aggregate.",
  parameters: {
    type: "object",
    properties: {
      from: { type: "string", description: "ISO date, inclusive (YYYY-MM-DD)" },
  ...
  async run(
    ...
    const categoryId =
      args.categoryId != null
        ? parsePositiveInt(args.categoryId, "categoryId", {
            min: 1,
    ...
    const rows = await fetchTransactionsInRange(cache, {
      from,
      to,
      limit,
      categoryId,
...
export const getMonthlyCategoryBreakdown = {
  ...
  description:
    'Top spending categories for each month in a date range. Use for "what did I spend the most on each month", "category breakdown per month".',
  parameters: {
    type: "object",
    properties: {
      from: { type: "string", description: "ISO date, inclusive (YYYY-MM-DD)" },
      ...
      topN: {
        type: "integer",
        description: "Top N categories per month. Default 5, max 20.",
        minimum: 1,
        maximum: 20,
      },
    },
...
export const searchTransactions = {
  ...
  description:
    "Search transactions by text matching recipient name, memo, or category. Use when the user mentions a specific name, merchant, or keyword.",
  parameters: {
    type: "object",
    properties: {
      query: {
        type: "string",
        description:
          "Search text to match against recipient, memo, or category.",
      },
      from: {
        type: "string",
        description: "Optional ISO date filter start (YYYY-MM-DD).",
      },
      to: {
        type: "string",
        description: "Optional ISO date filter end (YYYY-MM-DD).",
      },
      limit: {
        type: "integer",
...
export const getLargestTransactions = {
  ...
  description:
    'Largest individual transactions in a date range by absolute amount. Use for "biggest purchases", "largest payments", "biggest expenses".',
  parameters: {
    type: "object",
    properties: {
      from: { type: "string", description: "ISO date, inclusive (YYYY-MM-DD)" },
      ...
      direction: {
        ...
        description:
          "Filter to expenses (negative), income (positive), or both. Default expense.",
      },
    },
    required: ["from", "to"],
  },
...
export const getSpendTrendForCategory = {
  ...
  description:
    'Monthly spending trend for a specific category over the past N months. Use for "how has my groceries spending changed", "trend for a category".',
  parameters: {
    type: "object",
    properties: {
      categoryId: {
        type: "integer",
        description:
          "ID of the category to analyse. Use getCategories first if you only have the name.",
        minimum: 1,
      },
      months: {
        type: "integer",
        description: "Number of past months to cover. Default 12, max 36.",
        minimum: 1,
        maximum: 36,
      },
    },
  ...
  async run(
    ...
    const categoryId = parsePositiveInt(args.categoryId, "categoryId", {
      min: 1,
      max: Number.MAX_SAFE_INTEGER,
    });
    const months = parsePositiveInt(args.months, "months", {
      min: 1,
      max: 36,
      defaultValue: 12,
    });
    ...
    const series = Array.from(byMonth.values())
      .map((b) => ({
        bucket: b.bucket,
        total: roundToCents(b.total).toNumber(),
        count: b.count,
      }))
      .sort((a, b) => a.bucket.localeCompare(b.bucket));
...
export const getYearOverYearComparison = {
  ...
  description:
    'Compare spending by category between two calendar years. Use for "how did 2025 compare to 2024", "year over year spending".',
  parameters: {
    type: "object",
    properties: {
      year: {
        type: "integer",
        description: "Primary year (e.g. 2025).",
        minimum: 2000,
        maximum: 2100,
      },
      prevYear: {
        type: "integer",
        description: "Comparison year. Defaults to year - 1.",
        minimum: 2000,
        maximum: 2100,
      },
    },
  ...
  async run(
    ...
    const [currRows, prevRows] = await Promise.all([
      fetchTransactionsInRange(cache, {
        from: `${year}-01-01`,
        to: `${year}-12-31`,
      }),
      fetchTransactionsInRange(cache, {
    ...
    const comparison = Array.from(allCategories)
      .map((category) => {
        const curr = roundToCents(
          currMap.get(category)?.total ?? zero,
        ).toNumber();
        const prev = roundToCents(
          prevMap.get(category)?.total ?? zero,
        ).toNumber();
        const delta = Math.round((curr - prev) * 100) / 100;
        const pctChange =
          prev > 0 ? Math.round((delta / prev) * 10000) / 100 : null;
        return {
          category,
          [String(year)]: curr,
...
export const getUncategorisedTransactions = {
  ...
  description:
    'List transactions that have no category assigned. Use for "what transactions are uncategorised", "what needs categorising".',
  parameters: {
    type: "object",
    properties: {
      limit: {
...
export const getNetCashflow = {
  name: "getNetCashflow",
  description:
    'Net cashflow (income minus expenses) grouped by month or quarter. Use for "am I net positive this quarter", "how much money came in vs went out", "cashflow trend".',
  parameters: {
    type: "object",
    properties: {
      from: {
