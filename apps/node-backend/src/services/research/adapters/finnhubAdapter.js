...
const RANGE_TO_DAYS = makeChartRangeMap([
  1, 5, 31, 93, 186, 366, 731, 1827, 7300,
]);
...
const finnhubAdapter = {
  ...
  async fundamentals(symbol) {
    ...
    return {
      ...
      interestCoverage: num(
        m.netInterestCoverageTTM ?? m.netInterestCoverageAnnual,
      ),
    };
  },
  ...
  async analyst(symbol) {
    const recs = await getJson(
      `${BASE}/stock/recommendation?symbol=${encodeURIComponent(symbol)}&token=${key()}`,
    );
    ...
    const latest = latestRaw
      ? parseOr(recommendationRowSchema, latestRaw, {})
      : undefined;
    ...
    let targetsRaw;
    try {
      targetsRaw = await getJson(
        `${BASE}/stock/price-target?symbol=${encodeURIComponent(symbol)}&token=${key()}`,
      );
    } catch {
  ...
  async news(symbol, { count = 20 } = {}) {
    ...
    const articles = parseOr(looseArray(newsRowSchema), arr, [])
      .slice(0, count)
      .map((n) => ({
        title: n.headline,
        link: n.url,
