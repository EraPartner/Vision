...
const MACRO_CATALOG = Object.freeze([
  {
    provider: "eurostat",
...
function toItem(e) {
  return {
    ...
    source:
      SOURCE_LABEL[/** @type {keyof typeof SOURCE_LABEL} */ (e.provider)] ??
      e.provider,
  };
}
...
export function searchCatalog(provider, query) {
  ...
  return MACRO_CATALOG.filter((e) => e.provider === provider)
    .filter((e) => {
      const hay =
        `${e.title} ${e.region} ${e.keywords.join(" ")}`.toLowerCase();
...
export function catalogEntry(provider, seriesId) {
  return MACRO_CATALOG.find(
    (e) => e.provider === provider && e.seriesId === seriesId,
  );
}
...
export function isValidSeriesId(provider, seriesId) {
  const re =
    SERIES_ID_PATTERN[/** @type {keyof typeof SERIES_ID_PATTERN} */ (provider)];
