...
export function renderTopInvestmentsByCost(data, { currency }) {
  ...
  if (!byInvestment.length) {
    return emptySection({
      ...
      subtitle: "Investments with the highest tax and fee burden",
      heading: "No data",
      message: "No per-investment tax data found for the selected period.",
    });
  }
  ...
  const tableRows = sorted
    .map((inv, idx) => {
      ...
      <td class="num neg">${fmtCurrency(other, currency)}</td>
