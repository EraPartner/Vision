...
export function createCorsMiddleware(getAllowedOrigins) {
  return /** @param {ExpressRequest} req @param {ExpressResponse} res @param {ExpressNextFunction} next */ (req, res, next) => {
    ...
    const originAllowed = getAllowedOrigins().includes(/** @type {string} */ (origin));
