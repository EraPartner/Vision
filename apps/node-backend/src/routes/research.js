...
router.get(
  ...
  /** @param {ExpressRequest} req @param {ExpressResponse} res */ async (
    ...
    const provider = parseResearchParam(
      macroProviderSchema,
      req.query.provider,
    );
...
/** @param {unknown} value */
const requireInstrumentKey = (value) =>
...
router.post(
  ...
  /** @param {ExpressRequest} req @param {ExpressResponse} res */ async (
    ...
    const rows = await researchMappingService.save({
      ...
      mappings:
        /** @type {Array<{ provider: string, providerSymbol?: string, provider_symbol?: string, resolvedName?: string, resolved_name?: string, exchange?: string, currency?: string, status?: string }>} */ (
