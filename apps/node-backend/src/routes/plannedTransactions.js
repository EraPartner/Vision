...
const reminderDaysBeforeField = z
  .unknown()
  .transform((value, ctx) => {
    ...
    if (!Number.isInteger(n) || n < 0 || n > 365) {
      ctx.addIssue({
        code: "custom",
        message: "reminder_days_before must be an integer between 0 and 365",
      });
      return z.NEVER;
    }
    return n;
...
const recurrenceEndDateField = z
  .unknown()
  .transform((value, ctx) => {
    if (value == null) return value;
    try {
  ...
  .superRefine((data, ctx) => {
    ...
    if (!data.is_loan) {
      if (!data.planned_date || data.amount == null) {
        ctx.addIssue({
          code: "custom",
          message: "Missing required fields: planned_date, amount",
        });
      } else {
        ...
        if (!Number.isFinite(amt) || amt === 0) {
          ...
          ctx.addIssue({
            code: "custom",
            message: `amount must be between -${MAX_PLANNED_AMOUNT} and ${MAX_PLANNED_AMOUNT}`,
          });
        }
      }
    } else {
      ...
      if (termMonths && (termMonths < 1 || termMonths > 600)) {
        ctx.addIssue({
          code: "custom",
          message: "loan_term_months must be between 1 and 600 months",
        });
      }
    }
    ...
    if (
      !(data.is_loan && data.recurrence_end_date) &&
    ...
    if (
      !(data.is_loan && data.max_occurrences) &&
    ...
    if (
      !data.is_loan &&
      data.is_recurring &&
      !isValidPattern(/** @type {string} */ (data.recurrence_pattern))
    ) {
      ctx.addIssue({
        code: "custom",
        message: `Invalid or missing recurrence_pattern: ${data.recurrence_pattern}`,
      });
    }
  })
  .transform((data) => {
    ...
    if (!out.is_loan && out.max_occurrences != null)
      out.max_occurrences = Number(out.max_occurrences);
    return out;
  });
  ...
  amount: z
    ...
    .transform((value, ctx) => {
      ...
      if (
        ...
        amountNum === 0 ||
        Math.abs(amountNum) > MAX_PLANNED_AMOUNT
      ) {
        ctx.addIssue({
          code: "custom",
  ...
  recurrence_pattern: z
    ...
    .transform((value, ctx) => {
      if (value && !isValidPattern(/** @type {string} */ (value))) {
        ctx.addIssue({
          code: "custom",
          message: `Invalid recurrence_pattern: ${value}`,
        });
        return z.NEVER;
      }
      return value;
...
function generateLoanScheduleOrThrow(input) {
  try {
    ...
    throw new AppError(`Loan schedule generation failed: ${err.message}`, {
      cause: err,
    });
  }
}
...
function applyLoanPatchDefaults(fields, existing) {
  ...
  if (resultingIsLoan && (loanFieldsChanged || fields.is_loan === true)) {
    generatedLoanSchedule = generateLoanScheduleOrThrow({
      ...
      loan_annual_interest_rate:
        fields.loan_annual_interest_rate ?? existing.loan_annual_interest_rate,
    ...
    fields.loan_regular_payment_amount =
...
router.post(
  ...
  /** @param {ExpressRequest} req @param {ExpressResponse} res */ async (
    ...
    if (data.is_loan) {
      ...
      const generated = generateLoanScheduleOrThrow(
        /** @type {import('../services/calculations/loanSchedule.js').LoanConfig} */ (
          data
        ),
      );
...
router.get(
  ...
  /** @param {ExpressRequest} req @param {ExpressResponse} res */ async (
    ...
    if (!pt)
      throw new NotFoundError(`Planned transaction ${req.params.id} not found`);
...
router.patch(
  ...
  rateLimiter({
    windowMs: 60_000,
    maxRequests: 30,
    keyPrefix: "planned-transactions-patch",
  }),
  /** @param {ExpressRequest} req @param {ExpressResponse} res */ async (
    req,
    res,
    ...
    const loanScheduleDirective = resolveLoanScheduleDirective(
      generatedLoanSchedule,
      fields,
      existing,
    );
    ...
    if (
      ...
      fields.recurrence_pattern !== undefined
    ) {
      const resultingIsLoan =
        fields.is_loan !== undefined ? !!fields.is_loan : !!existing.is_loan;
      ...
      const resultingPattern =
        fields.recurrence_pattern !== undefined
          ? fields.recurrence_pattern
          : existing.recurrence_pattern;
      if (
        !resultingIsLoan &&
        resultingIsRecurring &&
        !isValidPattern(/** @type {string} */ (resultingPattern))
      ) {
        throw new ValidationError(
          `Invalid or missing recurrence_pattern: ${resultingPattern}`,
        );
      }
    }
    ...
    const updated =
      loanScheduleDirective !== undefined
        ? await plannedTransactionService.updateWithLoanSchedule(
            id,
            typedFields,
            loanScheduleDirective,
          )
...
router.post(
  ...
  /** @param {ExpressRequest} req @param {ExpressResponse} res */ async (
    ...
    res.ok(
      formatPlannedTransaction(
        /** @type {HydratedPlannedTransactionRow|null} */ (current),
      ),
    );
  },
);
  ...
  return {
    ...
    max_occurrences:
      row.max_occurrences != null
        ? parseInt(String(row.max_occurrences), 10)
        : null,
    reminder_days_before:
      row.reminder_days_before != null
        ? parseInt(String(row.reminder_days_before), 10)
        : null,
    ...
    loan_principal:
      row.loan_principal != null
        ? toNumber(toDecimal(row.loan_principal))
        : null,
    loan_annual_interest_rate:
      row.loan_annual_interest_rate != null
        ? toNumber(toDecimal(row.loan_annual_interest_rate))
        : null,
    loan_term_months:
      row.loan_term_months != null
        ? parseInt(String(row.loan_term_months), 10)
        : null,
    loan_start_date: ymd(row.loan_start_date) || null,
    loan_payment_day:
      row.loan_payment_day != null
        ? parseInt(String(row.loan_payment_day), 10)
        : null,
    loan_regular_payment_amount:
      row.loan_regular_payment_amount != null
        ? toNumber(toDecimal(row.loan_regular_payment_amount))
        : null,
