...
router.get(
  ...
  /** @param {ExpressRequest} req @param {ExpressResponse} res */ async (
    ...
    const page = parseOptionalPagination(req.query, { maxLimit: 1000 });
    const splits = await splitService.getOwedByRecipient(
      recipientId,
      page ?? {},
    );
    const total = page
...
router.post(
  ...
  /** @param {ExpressRequest} req @param {ExpressResponse} res */ async (
    ...
    const preparedSplits = normalizeBatchSplitInputs(
      /** @type {unknown[]} */ (splits),
    );
    ...
    const created = await splitService.createSplitsBatchAtomic(
      ...
      ({ transaction_id, splits: preparedSplits, actor: resolveActor(req) }),
    );
    res.status(201);
