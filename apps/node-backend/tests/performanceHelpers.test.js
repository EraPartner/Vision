...
const mockGetPortfolioSummary = vi.fn(async () => ({
  summaries: [],
  totals: {},
}));
...
describe("buildPortfolioPerformancePayload spike reconciliation", () => {
  ...
  it("preserves a genuine one-day cash movement instead of re-smoothing the stored total", async () => {
    ...
    expect(payload.snapshots[1]).toEqual(
  ...
  it("keeps the stored decomposition authoritative and derives gain fields from the served value", async () => {
    ...
    expect(Number(cleanedMiddle.value_fx_neutral)).toBeCloseTo(
      storedDecomposedValue,
      8,
    );
    expect(Number(cleanedMiddle.gain_loss)).toBeCloseTo(
      Number(cleanedMiddle.value) - 10_000,
      8,
    );
    ...
    expect(servedMiddle.return_pct).toBeCloseTo(
      (servedMiddle.gain_loss / 10_000) * 100,
      8,
    );
  });

  it("marks only the newest returned snapshot as provisional", async () => {
    const rows = [
      snapshot({
        date: "2026-01-01",
        invested: 100,
    ...
    expect(
      payload.snapshots.map(({ is_provisional }) => is_provisional),
    ).toEqual([false, false, true]);
    expect(payload.snapshots.at(-1).value).toBe(500);
  });
});
