 ...
 * describe at the bottom reads the real DB month, selects the adjacent app-zone
 * boundary instant, and then seeds every fixture from that frozen app clock.
 */
...
describe.skipIf(!hasTestDatabase())(
  "repositories/infoRepositoryForecast (real DB)",
  () => {
    beforeAll(async () => {
      expect(
    ...
    describe("getCashflowComparison", () => {
      ...
      it("applies alias-aware category and recipient exclusions to the live rows", async () => {
        ...
        expect(exclCat.without_planned.find((d) => d.day === 5).average).toBe(
          100,
        );

        // Excluding the PRIMARY recipient also drops rows recorded under its alias.
        const exclRec = await getCashflowComparison(
          [],
          [rec.electrabel],
          "EUR",
        );
        expect(exclRec.without_planned.find((d) => d.day === 5).average).toBe(
          100,
        );
      });
    ...
    describe("getCashflowForecastData", () => {
      it("splits history / currentActual / plannedCurrent / plannedHist on the real window boundaries", async () => {
        ...
        expect(r.plannedHist).toEqual([
          { date: await ymd(monthDay(1, 5)), net: -30 },
        ]);
      });
    });
    ...
    describe("getCashflowForecastDataRolling", () => {
      it("keeps history strictly before the rolling window and only future planned rows", async () => {
        await seedBase();
        await insertTxn({
          dateExpr: "CURRENT_DATE - interval '40 days'",
          amount: "100.00",
        });
        ...
        await insertPlanned({
          dateExpr: "CURRENT_DATE + interval '90 days'",
          amount: "-11.00",
        }); // beyond daysForward

        const r = await getCashflowForecastDataRolling(
          12,
          30,
          60,
          [],
        ...
        expect(r.history).toEqual([
          { date: await ymd("CURRENT_DATE - interval '40 days'"), net: 100 },
        ]);
        expect(r.currentActual).toEqual([
    ...
    describe("ADR-083 internal-transfer exclusion", () => {
      ...
      it("excludes transfer legs from the comparison, matching getAverageVsCurrentSpending", async () => {
        ...
        const current = comparison.without_planned.find(
          (d) => d.day === comparison.current_day,
        );
        ...
        expect(forecast.currentActual).toEqual([
          { date: await ymd("CURRENT_DATE"), net: -100 },
        ]);
      });
      ...
      it("excludes transfer legs from the rolling and by-category surfaces too", async () => {
        ...
        const rolling = await getCashflowForecastDataRolling(
          12,
          30,
          60,
          [],
        ...
        expect(rolling.currentActual).toEqual([
          { date: await ymd("CURRENT_DATE"), net: -100 },
        ]);
        expect(rolling.history).toEqual([
          { date: await ymd(histDate), net: 50 },
        ]);
      ...
      it("counts transfer legs again when includeTransfers is switched on", async () => {
        ...
        const forecast = await getCashflowForecastData(3, [], [], "EUR");
        expect(forecast.currentActual).toEqual([
          { date: await ymd("CURRENT_DATE"), net: -1000 },
        ]);
    ...
    describe("historical average denominator", () => {
      ...
      it("keeps the divisor when a category exclusion empties the oldest month", async () => {
        ...
        expect(exclFood.without_planned.find((d) => d.day === 5).average).toBe(
          10,
        );
      });
    ...
    //
    // These cases choose the adjacent APP_TIMEZONE month boundary whose UTC
    // instant lies in the month Postgres currently reports. In a positive-offset
    // zone that is 00:30 on the first of the next app month; in a negative-offset
    // zone it is 23:30 on the last day of the previous app month. The DB keeps its
    // real, unfaked clock. This reproduces the month disagreement in either
    // direction instead of assuming Brussels is always ahead of UTC.
    //
    ...
    describe("one clock: APP_TIMEZONE anchor across a month rollover", () => {
      // Once the fake app clock is pinned, anchored() resolves these expressions
      // from that one clock. Day 5 of the last two complete app months forms a
      // two-month history; CURRENT_DATE itself is the current-app-month fixture.
      const lastCompleteMonthDay5 =
        "date_trunc('month', CURRENT_DATE) - interval '1 month' + interval '4 days'";
      const monthBeforeDay5 =
        "date_trunc('month', CURRENT_DATE) - interval '2 months' + interval '4 days'";
      const appToday = "CURRENT_DATE";
      ...
      async function pinClockToRollover() {
        const { rows } = await getTestPool().query(`
        SELECT to_char(date_trunc('month', CURRENT_DATE), 'YYYY-MM') AS db_month,
               to_char(date_trunc('month', CURRENT_DATE) + interval '1 month', 'YYYY-MM-DD') AS next_month_first,
               to_char(date_trunc('month', CURRENT_DATE) - interval '1 day', 'YYYY-MM-DD') AS previous_month_last
      `);
        const dbFacts = rows[0];
        const candidates = [
          {
            appToday: dbFacts.next_month_first,
            instant: new Date(
              appDateStringToUtc(dbFacts.next_month_first).getTime() +
                30 * 60_000,
            ),
          },
          {
            appToday: dbFacts.previous_month_last,
            instant: new Date(
              appDateStringToUtc(dbFacts.previous_month_last).getTime() +
                23.5 * 60 * 60_000,
            ),
          },
        ];
        const selected = candidates.find(
          ({ instant }) =>
            instant.toISOString().slice(0, 7) === dbFacts.db_month,
        );
        expect(
          selected,
          "configured APP_TIMEZONE must expose an adjacent-month UTC rollover",
        ).toBeDefined();

        const [appYear, appMonth, appDay] = selected.appToday
          .split("-")
          .map(Number);
        const facts = {
          app_today: selected.appToday,
          app_year: appYear,
          app_month: appMonth,
          app_day: appDay,
          app_days_in_month: new Date(
            Date.UTC(appYear, appMonth, 0),
          ).getUTCDate(),
          db_month: dbFacts.db_month,
        };
        const { instant } = selected;
        vi.useFakeTimers({ toFake: ["Date"] });
        vi.setSystemTime(instant);

        // Guard the premise in both offset directions: the fake app calendar and
        // the real UTC/DB calendar occupy adjacent months.
        expect(todayAppDateString()).toBe(facts.app_today);
        expect(instant.toISOString().slice(0, 7)).toBe(facts.db_month);
        expect(facts.app_today.slice(0, 7)).not.toBe(facts.db_month);
        return facts;
      }
      ...
      it("keeps the last complete month in BOTH the numerator and the divisor", async () => {
        ...
        // The response describes the APP_TIMEZONE month and day.
        expect(r.year).toBe(facts.app_year);
        expect(r.month).toBe(facts.app_month);
        expect(r.current_day).toBe(facts.app_day);
        ...
        expect(
          r.without_planned.find((d) => d.day === facts.app_day).current,
        ).toBe(-10);
        expect(
          r.with_planned.find((d) => d.day === facts.app_day).current,
        ).toBe(-30);
      });
      ...
      it("splits history from currentActual on the app month boundary", async () => {
        ...
        expect(r.history).toEqual([
          { date: await ymd(lastCompleteMonthDay5), net: 100 },
        ]);
        expect(r.currentActual).toEqual([
          { date: await ymd(appToday), net: -10 },
        ]);
      });
      ...
      it("gives getAverageVsCurrentSpending the same clock", async () => {
        ...
        expect(r.current_month.days_elapsed).toBe(facts.app_day);
