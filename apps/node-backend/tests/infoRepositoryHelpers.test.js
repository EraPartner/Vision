...
describe("mvAvailable", () => {
  ...
  it("expires a positive result so a runtime-dropped view self-heals", async () => {
    vi.useFakeTimers();
    try {
  ...
  it("rejects unknown view names instead of interpolating them into SQL", async () => {
    await expect(
      mvAvailable("not_a_real_mv'; DROP TABLE x; --"),
    ).rejects.toThrow(/unknown materialized view/i);
