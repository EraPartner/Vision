...
const data = {
  ...
  grossDividendBase: 291,
  netDividendResult: 203.7,
...
describe("renderFeeBreakdown", () => {
  it("labels rows with asset-class names and totals fees from the top level", () => {
    ...
    expect(html).toContain("<td>Stocks</td>");
...
describe("renderTaxByAssetClass", () => {
  ...
  it("renders convention-dependent dividend metrics as incomplete when a row is unknown", () => {
    const html = renderTaxExecutiveSummary(
      {
        ...data,
        grossDividendBase: null,
        netDividendResult: null,
        unknownDividendConventionCount: 2,
      },
      ctx,
    );

    expect(html).toContain("Incomplete dividend metrics");
    expect(html).toContain("2 dividend rows have");
...
describe("renderBelgianRulesSummary", () => {
  ...
  it("renders client PIT bracket rates without re-scaling percent units", () => {
    ...
    const pctCells = [...html.matchAll(/class="num">([+-][\d.]+)%/g)].map((m) =>
      Number(m[1]),
    );
...
describe("renderTopInvestmentsByCost", () => {
  it("renders the per-investment tax columns and a total cost that includes taxes", () => {
    ...
    expect(html).toContain(eur("5.00")); // other tax
