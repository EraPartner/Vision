...
describe('createCorsMiddleware', () => {
  it('reflects an explicitly allowed origin with the fixed CORS contract', () => {
    ...
    middleware({ method: 'GET', headers: { origin: 'https://app.example' } }, response, next);

    expect(response.getHeader('vary')).toBe('Origin');
    expect(response.getHeader('access-control-allow-origin')).toBe('https://app.example');
    expect(response.getHeader('access-control-allow-credentials')).toBe('true');
    expect(response.getHeader('access-control-allow-methods')).toBe('GET,POST,PUT,PATCH,DELETE,OPTIONS');
    expect(response.getHeader('access-control-allow-headers')).toBe('Content-Type,Authorization,X-Request-Id');
    expect(response.getHeader('access-control-expose-headers')).toBe('X-Request-Id');
  ...
  it('terminates every OPTIONS request with the existing 204 contract', () => {
    ...
    middleware({ method: 'OPTIONS', headers: { origin: 'https://evil.example' } }, response, next);
