...
async function listResolvedCategoryName(txnId) {
  ...
  return {
    effectiveCategoryId: listed.effective_category_id,
    name: rows[0].name,
  };
}
...
describe.skipIf(!hasTestDatabase())(
  "3-level effective-category resolution (real DB)",
  () => {
    beforeAll(async () => {
      expect(
    ...
    describe("monthly summary", () => {
      ...
      it("mv_monthly_summary attributes an alias row to its PRIMARY recipient default category", async () => {
        ...
        expect(
          rows.map((r) => [
            r.category_name,
            Number(r.total_spending),
            Number(r.total_income),
          ]),
        ).toEqual([
    ...
    describe("computeSankeyFlow", () => {
      ...
      async function flow(opts) {
        ...
        return {
          ...
          spending: Object.fromEntries(
            env.data.nodes
              .filter(
                (n) => n.id.startsWith("cat:") || n.id === "__uncategorised__",
              )
      ...
      it("excludes internal transfers by default and includes them when the setting is on", async () => {
        ...
        expect(await flow({})).toEqual({
          income: 3900,
          spending: { "Food: Groceries": 40, __uncategorised__: 900 },
        });
      });
    });
    ...
    describe("transaction export", () => {
      ...
      it("labels an alias row with the category the transactions list resolves", async () => {
        ...
        const { effectiveCategoryId, name } =
          await listResolvedCategoryName(aliasTxn);
      ...
      it("an own category_id still wins over both recipient defaults", async () => {
        ...
        const { effectiveCategoryId, name } =
          await listResolvedCategoryName(ownTxn);
    ...
    describe("planned transactions", () => {
      ...
      const NEXT_MONTH_SQL = () =>
        `(date_trunc('month', ${appTodaySql()}) + interval '1 month' + interval '4 days')::date`;
      ...
      it("getAll / getById / getDueSoon / getForForecast resolve all three levels", async () => {
        const { aliasPlanned, inheritedPlanned, ownPlanned } =
          await seedPlanned();
        ...
        expect(
          Object.fromEntries(items.map((r) => [r.id, r.category_name])),
        ).toEqual(expected);
        expect(
          Object.fromEntries(items.map((r) => [r.id, r.recipient_name])),
        ).toEqual(expectedRecipientNames);
        ...
        expect(
          Object.fromEntries(due.map((r) => [r.id, r.category_name])),
        ).toEqual(expected);
        expect(
          Object.fromEntries(due.map((r) => [r.id, r.recipient_name])),
        ).toEqual(expectedRecipientNames);

        const forecast = await plannedTransactionRepository.getForForecast(3);
        expect(
          Object.fromEntries(forecast.map((r) => [r.id, r.category_name])),
        ).toEqual(expected);
        expect(
          Object.fromEntries(forecast.map((r) => [r.id, r.recipient_name])),
        ).toEqual(expectedRecipientNames);
      });
      ...
      it("search matches the label a planned row actually displays", async () => {
        ...
        const utilities = await plannedTransactionRepository.getAll({
          search: "Utilities",
        });
        expect(utilities.items.map((r) => r.id)).toEqual([inheritedPlanned]);

        const zzz = await plannedTransactionRepository.getAll({
          search: "Zzz:La",
        });
        expect(zzz.items.map((r) => r.id)).toEqual([aliasPlanned]);

        const food = await plannedTransactionRepository.getAll({
          search: "Groceries",
        });
      ...
      it("getPlannedExpensesNextMonth categorises what its sibling repository categorises", async () => {
        ...
        const reference = Object.fromEntries(
          items.map((r) => [r.id, r.category_name]),
        );

        const result =
          await plannedRepository.getPlannedExpensesNextMonth("EUR");
        ...
        // neither rc nor pc.
        expect(
          Object.fromEntries(occurrences.map((t) => [t.id, t.category_name])),
        ).toEqual(reference);
        expect(
          Object.fromEntries(occurrences.map((t) => [t.id, t.recipient_name])),
        ).toEqual({
          [aliasPlanned]: "Electrabel",
          [inheritedPlanned]: "Electrabel",
          [ownPlanned]: "Electrabel",
