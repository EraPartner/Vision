...
describe("Settings storage and retrieval", () => {
  ...
  it("settingsRepository.set should call DB and return saved key/value", async () => {
    ...
    const upsertCall = query.mock.calls.find(
      ([sql]) =>
        typeof sql === "string" && sql.includes("INSERT INTO user_settings"),
    );
...
describe("belgian_tax_profile setting validation (TODO E6)", () => {
  ...
  it("rejects a negative communal surcharge (would become a tax credit)", async () => {
    const res = await put("belgian_tax_profile", {
      ...validProfile,
      communalSurchargePercent: -7,
    });
  ...
  it("rejects negative and absurd money fields", async () => {
    ...
    res = await put("belgian_tax_profile", {
      ...validProfile,
      medicalExpenses: 1e15,
    });
    expect(res.status).toBe(400);
    expect(res.body.error.code).toBe("VALIDATION_ERROR");
  ...
  it("validates each snapshot in the year-keyed snapshots map", async () => {
    let res = await put("belgian_tax_profile_snapshots_v1", {
      2025: validProfile,
    });
    expect(res.status).toBe(200);
    expect(res.body.ok).toBe(true);
  ...
  it("requires snapshot meta to be a year-keyed object map", async () => {
    let res = await put("belgian_tax_profile_snapshot_meta_v1", {
      2025: { history: [] },
    });
    expect(res.status).toBe(200);
    expect(res.body.ok).toBe(true);
  ...
  it("rejects the retired federalPITTotal field in frozen calculations", async () => {
    const canonical = {
      2025: {
        frozenCalculation: {
    ...
    let res = await put("belgian_tax_profile_snapshot_meta_v1", canonical);
    expect(res.status).toBe(200);
    ...
    expect(res.status).toBe(400);
    expect(res.body.error.message).toMatch(
      /federalPITTotal.*no longer accepted/,
    );
  });
