...
function wireQuery(pending) {
  query.mockImplementation(async (sql) => {
    ...
    if (sql.includes('FROM portfolio_import_batches')) {
      return { rows: [{ default_asset_class: null, default_type: null, custom_config: {}, is_brokerage: false }] };
    }
    if (sql.includes('FROM portfolio_import_staging_rows')) return { rows: pending };
...
describe('validateBatch — future-dated rows', () => {
  ...
  it('flags a future-dated row as an error while validating a past-dated one', async () => {
    const pending = [
      { id: 1, row_index: 0, tx_date: new Date('2999-01-01T00:00:00Z'), type_raw: 'buy', units: null, price_per_unit: null, amount: 100, raw_data: 'future' },
      { id: 2, row_index: 1, tx_date: new Date('2020-01-01T00:00:00Z'), type_raw: 'buy', units: null, price_per_unit: null, amount: 100, raw_data: 'past' },
    ];
    wireQuery(pending);
