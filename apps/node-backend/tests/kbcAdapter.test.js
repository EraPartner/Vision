...
describe("KBCAdapter", () => {
  ...
  it("normalizes ISO currency cells and nulls malformed free text", async () => {
    const csv = SAMPLE_KBC_CSV.replace(
      ";BAU IE;EUR;  02026001",
      ";BAU IE;usd;  02026001",
    ).replace(";BAU IE;EUR;  02026001", ";BAU IE;euro;  02026001");
  ...
  it("skips malformed dates", async () => {
    const csv = SAMPLE_KBC_CSV.replace(
      "03/01/2026;INSTANTOVERSCHRIJVING NAAR",
      "INVALID;INSTANTOVERSCHRIJVING NAAR",
    );
    tmpPath = writeTempCSV(csv);
    const txns = await parse(tmpPath);
