...
describeDb("migration 0088: money columns aligned to NUMERIC(18,4)", () => {
  ...
  afterAll(async () => {
    if (pool) {
      ...
      if (txIds.length)
        await pool.query(`DELETE FROM transactions WHERE id = ANY($1)`, [
          txIds,
        ]);
      if (plannedIds.length) {
        await pool.query(
          `DELETE FROM planned_transactions WHERE id = ANY($1)`,
          [plannedIds],
        );
      }
      if (recipientId)
  ...
  it("splits a 4-decimal amount exactly — SUM(splits) = ABS(amount), no tolerance needed", async () => {
    ...
    expect(agg.rows.map((r) => r.original_amount)).toEqual([
      "50.0000",
      "50.0001",
    ]);
  });
  ...
  it("tracks 4-decimal payments to an exact outstanding amount", async () => {
    ...
    await pool.query(
      `INSERT INTO split_payments (split_id, amount) VALUES ($1, '25.0001')`,
      [split[0].id],
    );
    const { rows } = await pool.query(
  ...
  it("0062 split-guard still fires on a shrunk parent and passes an exact 4-dp cover", async () => {
    ...
    await expect(
      pool.query(`UPDATE transactions SET amount = '-99.99' WHERE id = $1`, [
        txId,
      ]),
    ...
    await pool.query(
      `UPDATE transactions SET amount = '-100.0001', balance = '1.2345' WHERE id = $1`,
      [txId],
    );
    const { rows } = await pool.query(
      `SELECT amount, balance FROM transactions WHERE id = $1`,
      [txId],
    );
  ...
  it("holds boundary magnitudes: 14 integer digits at 4 dp", async () => {
    ...
    const { rows } = await pool.query(
      `SELECT balance FROM transactions WHERE id = $1`,
      [txId],
    );
    ...
    const planned = await pool.query(
      `SELECT id FROM planned_transactions WHERE memo = $1`,
      [MARK],
    );
    for (const r of planned.rows)
      if (!plannedIds.includes(r.id)) plannedIds.push(r.id);
  ...
  describe("split guards validate at NUMERIC(18,4) storage precision", () => {
    ...
    it("auto-settles only on an exact 4-dp cover, and a settled split takes no more payments", async () => {
      ...
      let state = await pool.query(
        `SELECT is_settled FROM transaction_splits WHERE id = $1`,
        [splitId],
      );
...
describeDb("migration 0088 legacy-trigger upgrade and downgrade safety", () => {
  ...
  it("removes the legacy blocker and completes the migration chain to head", async () => {
    ...
    `);
    expect(legacyGuard.rows[0]).toEqual({
    ...
    `);
    expect(laterRevisions.rows[0]).toEqual({
      check_0089: true,
      rename_0090: true,
    });
  });
  ...
  it("refuses with a curated row list while a sub-cent split exists, then downgrades clean", async () => {
    ...
    expect(
      err,
      "expected the 0088 downgrade pre-flight to refuse",
    ).not.toBeNull();
    const output = `${err.stderr ?? ""}\n${err.stdout ?? ""}\n${err.message ?? ""}`;
    expect(output).toMatch(/0088 downgrade refused/);
    expect(output).toMatch(
      new RegExp(`transaction_splits id=${split[0].id} amount=0\\.0040`),
    );
    ...
    await db.query(`DELETE FROM transaction_splits WHERE id = $1`, [
      split[0].id,
    ]);
    await alembic("downgrade", "0087_flat_investments_conversion");
