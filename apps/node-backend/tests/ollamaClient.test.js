...
describe('ollama client', () => {
  ...
  describe('chat', () => {
    ...
    it('includes tools in request body when provided', async () => {
      ...
      const client = makeClient(fetchImpl);
      const tools = [{ type: 'function', function: { name: 'getSpend', parameters: {} } }];
      ...
      const fetchImpl = vi.fn().mockResolvedValue(
        jsonResponse({
          message: {
            ...
            content: '',
            tool_calls: [
              { function: { name: 'getSpendByCategory', arguments: { from: '2025-01-01', to: '2025-12-31' } } },
            ],
          },
        }),
      );
  ...
  describe('chatStream', () => {
    ...
    it('handles chunks that split NDJSON lines across reads', async () => {
      const line1 = JSON.stringify({ message: { content: 'foo' }, done: false });
    ...
    it('throws HTTP_ERROR on non-ok response', async () => {
      const fetchImpl = vi
        .fn()
        .mockResolvedValue({ ok: false, status: 503, text: vi.fn().mockResolvedValue('') });
      const client = makeClient(fetchImpl);
    ...
    it('throws NO_BODY when response has no readable body', async () => {
      const fetchImpl = vi
        .fn()
        .mockResolvedValue({ ok: true, status: 200, body: null, text: vi.fn() });
      const client = makeClient(fetchImpl);
