 ...
 * `transactions` row on the sleeve, categorized by configured active IDs.
 *
...
async function seedCashCategories() {
  const { rows } = await pool.query(
    `INSERT INTO categories (general, detail, is_active)
     VALUES ('INCOME', 'DIVIDENDS', true),
            ('INCOME', 'INTEREST', true),
            ('INVESTMENTS', 'FEES', true),
            ('INVESTMENTS', 'TAXES', true)
     RETURNING id, detail`,
  );
  const ids = Object.fromEntries(rows.map((row) => [row.detail, row.id]));
  await pool.query(
    `INSERT INTO user_settings (key, value)
     VALUES ('brokerage_cash_category_ids', $1::jsonb)`,
    [
      JSON.stringify({
        dividend: ids.DIVIDENDS,
...
async function stagePending(
  ...
  const { rows } = await pool.query(
    ...
    [
      ...
      currency,
      note,
      `${rowIndex}|${typeRaw}|${amount}|${currency}`,
    ],
  );
  return Number(rows[0].id);
}
...
describeDb(
  "portfolio import — D6 instrument-less cash rows (real Postgres)",
  () => {
    ...
    it('imports instrument-less dividend/interest/fee/tax rows as signed, categorized ledger rows (fails on the pre-D6 "unresolved instrument" routing)', async () => {
      ...
      expect(routed.map((r) => r.route)).toEqual([
        "cash",
        "cash",
        "cash",
        "cash",
      ...
      expect(routed.map((r) => r.type)).toEqual([
        "dividend",
        "interest",
        "fee",
        "tax",
      ...
      expect(byAmount.get(12.34)).toMatchObject({
        cat_general: "INCOME",
        cat_detail: "DIVIDENDS",
      });
      expect(byAmount.get(1.11)).toMatchObject({
        cat_general: "INCOME",
        cat_detail: "INTEREST",
      });
      ...
      expect(byAmount.get(-2.5)).toMatchObject({
        cat_general: "INVESTMENTS",
        cat_detail: "FEES",
      });
      expect(byAmount.get(-0.4)).toMatchObject({
        cat_general: "INVESTMENTS",
        cat_detail: "TAXES",
      });
      ...
      expect(ptx[0]).toMatchObject({
        investment_id: fx.investmentId,
        type: "dividend",
        amount: 100,
    ...
    it("re-importing the same statement is a no-op for D6 rows (field dedup)", async () => {
      ...
      expect((await commitPortfolioImport({ batchId: batchA })).imported).toBe(
        5,
      );
    ...
    it("rollback restores exactly: D6 ledger rows deleted via the cash route, bystanders untouched", async () => {
      ...
      const { rows: remainingTrades } = await pool.query(
    ...
    it("a −10 fee is not deduped against an unrelated +10 ledger row sharing date and memo (no magnitude leg for D6 kinds)", async () => {
      ...
      const amounts = (await ledgerRows())
        .map((r) => r.amount)
        .sort((a, b) => a - b);
    ...
    it("a USD cash row on the EUR sleeve keeps its native currency and is converted per partition, not summed raw (WP-C2 FX check)", async () => {
      ...
      await stagePending(batchId, 1, {
        typeRaw: "dividend",
        amount: 100,
        currency: "USD",
      }); // 100 USD, instrument-less
      await validateBatch({ batchId });
      await matchBatch({ batchId });
      ...
      const sleeve = accounts.find(
        (a) => Number(a.id) === Number(fx.accountId),
      );
