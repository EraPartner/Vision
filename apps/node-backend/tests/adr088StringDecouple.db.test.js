...
async function desync(table, id) {
  const pool = getTestPool();
  await pool.query(`UPDATE ${table} SET bank_account = 'STALE LABEL' WHERE id = $1`, [id]);
  const { rows } = await pool.query(
    `SELECT account_id, bank_account FROM ${table} WHERE id = $1`, [id],
  );
...
describeDb('ADR-088 string decouple across planned/splits/export (real DB)', () => {
  beforeAll(async () => {
    expect(
  ...
  describe('plannedTransactionRepository', () => {
    it('bankAccount filter matches via accounts.name, not the row string', async () => {
      ...
      const kbc = await plannedTransactionRepository.getAll({ bankAccount: 'kbc' });
      expect(kbc.items.map((r) => r.id)).toEqual([fx.plannedKbc]);
      expect(kbc.total).toBe(1);
      const stale = await plannedTransactionRepository.getAll({ bankAccount: 'stale' });
    ...
    it('search matches the canonical account name, not the stale string', async () => {
      await desync('planned_transactions', fx.plannedKbc);
      const byName = await plannedTransactionRepository.getAll({ search: 'kbc curr' });
      expect(byName.items.map((r) => r.id)).toEqual([fx.plannedKbc]);
      const byStale = await plannedTransactionRepository.getAll({ search: 'stale lab' });
    ...
    it('getAll/getById/getDueSoon project bank_account from accounts.name', async () => {
      ...
      expect((await plannedTransactionRepository.getById(fx.plannedKbc)).bank_account).toBe('KBC Current');
    ...
    it('update() returns the canonical label, and creating against an existing account reuses it', async () => {
      const updated = await plannedTransactionRepository.update(fx.plannedKbc, { memo: 'RENT FEB (EDITED)' });
      ...
      const { rows } = await getTestPool().query(
        'SELECT bank_account FROM planned_transactions WHERE id = $1', [created.id],
      );
  ...
  describe('UPDATE-path FK resolution (ghost-row fix)', () => {
    ...
    it('transaction PATCH to a first-seen label mints the account, moves the FK, and is visible on read', async () => {
      const updated = await transactionRepository.update(fx.txnKbc, { bank_account: 'Brand New Label' });
      ...
      expect((await transactionRepository.getById(fx.txnKbc)).bank_account).toBe('Brand New Label');
      const filtered = await transactionRepository.getAll({ bankAccount: 'brand new' });
    ...
    it('transaction PATCH to an existing label (case variant) reuses the account', async () => {
      const updated = await transactionRepository.update(fx.txnKbc, { bank_account: 'wise usd' });
    ...
    it('transaction PATCH blanking the label detaches the FK', async () => {
      const updated = await transactionRepository.update(fx.txnKbc, { bank_account: null });
    ...
    it('planned PATCH (update and updateWithLoanSchedule) resolves first-seen labels onto the FK', async () => {
      const updated = await plannedTransactionRepository.update(fx.plannedKbc, { bank_account: 'Planned Fresh Label' });
      const { rows } = await getTestPool().query(
      ...
      const filtered = await plannedTransactionRepository.getAll({ bankAccount: 'planned fresh' });
      expect(filtered.items.map((r) => r.id)).toEqual([fx.plannedKbc]);

      const viaSchedule = await plannedTransactionRepository.updateWithLoanSchedule(
        fx.plannedWise, { bank_account: 'Sched Fresh Label' }, [],
      );
    ...
    it('a PATCH label edit keeps the parity invariant (no string-without-FK ghosts)', async () => {
      await transactionRepository.update(fx.txnKbc, { bank_account: 'Parity Probe Label' });
      await plannedTransactionRepository.update(fx.plannedKbc, { bank_account: 'Parity Probe Label' });
      const { rows } = await getTestPool().query(
  ...
  describe('splitRepository owed views', () => {
    it('getOwedByRecipient / export rows read the label via the FK', async () => {
      ...
      const exportRows = await splitRepository.getOwedExportRowsByRecipient(fx.recipientId);
  ...
  describe('transaction CSV export', () => {
    it('streams the canonical account label and filters bank_accounts via the FK', async () => {
      ...
      await streamCsvExport(resAll, {
        whereSql: whereAll.sql, params: whereAll.params, nextParamIdx: whereAll.nextParamIdx,
      });
      ...
      const whereKbc = buildTransactionWhere({ bankAccounts: ['KBC Current'] });
      await streamCsvExport(resKbc, {
        whereSql: whereKbc.sql, params: whereKbc.params, nextParamIdx: whereKbc.nextParamIdx,
      });
