...
function writeTempCSV(prefix, content) {
  const p = path.join(
    os.tmpdir(),
    `${prefix}_${tmpFiles.length}_${process.pid}.csv`,
  );
...
describe("BNP adapter — parseAmountField edge cases", () => {
  ...
  it("normalizes ISO currency cells and nulls malformed free text", async () => {
    const csv = [
      HEADER,
      "1;24/11/2025;24/11/2025;-12,00;usd;BE81;X;;SHOP;;;;",
      "2;23/11/2025;23/11/2025;-13,00;euro;BE81;X;;SHOP;;;;",
    ].join("\n");
    const txns = await parseBnp(writeTempCSV("bnp", csv));
    expect(txns.map((txn) => txn.currency)).toEqual(["USD", null]);
  });
...
describe("ING adapter — parseCommaDecimal", () => {
  ...
  it("normalizes ISO currency cells and nulls malformed free text", async () => {
    const csv = [
      HEADER,
      "BE99;ME;BE12;0001;24/11/2025;24/11/2025;-12,00;usd;BETALING;SHOP;",
      "BE99;ME;BE12;0002;23/11/2025;23/11/2025;-13,00;euro;BETALING;SHOP;",
    ].join("\n");
    const txns = await parseIng(writeTempCSV("ing", csv));
    expect(txns.map((txn) => txn.currency)).toEqual(["USD", null]);
  });
...
describe("generic adapter — configurable mapping", () => {
  ...
  it("throws when no config is supplied (generic requires a mapping)", async () => {
    await expect(
      parseGeneric(writeTempCSV("generic", "Date,Amount\n1/1/2025,1")),
    ).rejects.toThrow(/requires a customConfig/);
  });
  ...
  it("parses %Y-%m-%d %H:%M:%S as a UTC calendar day (no early-morning day-shift)", async () => {
    ...
    const csv = [
      "Date,Amount,Payee,Note,Cur",
      '2024-12-31 00:30:00,"5,00",X,n,EUR',
    ].join("\n");
    const txns = await parseGeneric(writeTempCSV("generic", csv), cfg);
...
describe("wise adapter — cross-currency direction", () => {
  ...
  it("books a cross-currency OUT transfer on the source (your) side", async () => {
    ...
    const csv = [
      HEADER,
      "COMPLETED,2024-12-31,OUT,100,EUR,108,USD,Me,Shop,Payment",
    ].join("\n");
  ...
  it("books a cross-currency IN transfer on the target (your) side", async () => {
    const csv = [
      HEADER,
      "COMPLETED,2024-12-31,IN,100,USD,90,EUR,Sender,Me,Salary",
    ].join("\n");
    const txns = await parseWise(writeTempCSV("wise", csv));
  ...
  it("parses an early-morning timestamp as a UTC calendar day (no day-shift)", async () => {
    const csv = [
      HEADER,
      "COMPLETED,2024-12-31 00:30:00,IN,0,EUR,90,EUR,Sender,Me,Salary",
    ].join("\n");
    const txns = await parseWise(writeTempCSV("wise", csv));
