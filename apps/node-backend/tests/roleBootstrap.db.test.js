...
import { ensureAppRole, renderGrantStatements } from '../src/database/roleBootstrap.js';
...
async function queryAs(user, password, sql) {
  const client = new pg.Client({ connectionString: urlFor(user, password), connectionTimeoutMillis: 5000 });
...
describe.skipIf(!hasTestDatabase())('roleBootstrap (real Postgres)', () => {
  ...
  beforeAll(async () => {
    ...
    authEnforced = await queryAs(decodeURIComponent(migUser), 'definitely-wrong-password', 'SELECT 1')
      .then(() => false)
      .catch(() => true);
  }, 180_000);
  ...
  it('renders the shared grant template with quoted identifiers and no leftover placeholders', () => {
    const stmts = renderGrantStatements({ appRole: 'a"pp', ownerRole: 'own', dbName: 'db' });
    ...
    expect(joined).toContain('ALTER DEFAULT PRIVILEGES FOR ROLE "own" IN SCHEMA public');
  });

  it('skips in single-role setups without touching the database', async () => {
    const log = makeLog();
    const noMig = await ensureAppRole({ databaseUrl: TEST_URL, migrationsUrl: undefined, log });
    expect(noMig).toMatchObject({ status: 'skipped', reason: 'single-role' });

    const sameUrl = await ensureAppRole({ databaseUrl: TEST_URL, migrationsUrl: TEST_URL, log });
  ...
  it('refuses to bootstrap across different databases', async () => {
    ...
    const res = await ensureAppRole({ databaseUrl: urlFor(APP_ROLE, APP_PASS), migrationsUrl: other.toString(), log });
    expect(res).toMatchObject({ status: 'degraded', reason: 'database-mismatch' });
  });
  ...
  it('creates the app role with the full grant set on an already-initialised database', async () => {
    ...
    const read = await queryAs(APP_ROLE, APP_PASS, "SELECT id FROM categories WHERE general = 'vb_rb'");
    expect(read).toHaveLength(1);
    await queryAs(APP_ROLE, APP_PASS, "DELETE FROM categories WHERE general = 'vb_rb'");
  }, 30_000);
  ...
  it('covers tables created later by the migration role (default privileges)', async () => {
    ...
    await pool.query('CREATE TABLE vb_rb_future_table (id serial PRIMARY KEY, v text)');
  ...
  it('never alters a pre-existing role (no password reset)', async () => {
    await pool.query(`CREATE ROLE "${PRE_ROLE}" LOGIN PASSWORD 'original-pw' NOSUPERUSER`);
    const log = makeLog();
    ...
    expect(rows[0].ok).toBe(1);
    if (authEnforced) {
      await expect(queryAs(PRE_ROLE, 'not-the-real-password', 'SELECT 1')).rejects.toThrow();
    }
  }, 30_000);
  ...
  it('degrades gracefully when the migration role lacks CREATEROLE', async () => {
    ...
    const { rows } = await pool.query("SELECT 1 FROM pg_roles WHERE rolname = 'vb_rb_never_created'");
  ...
  it('reports unavailable fast on an authentication failure (no cold-start retry loop)', async (ctx) => {
    ...
    expect(log.lines.warn.join('\n')).toContain('cannot connect as the migration role');
  }, 30_000);
