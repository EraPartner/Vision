...
describe("matchesTolerance", () => {
  ...
  it("rejects sign mismatch", () => {
    expect(
      matchesTolerance(planned({ amount: "-50.00" }), tx({ amount: "50.00" })),
    ).toBe(false);
  });

  it("accepts date within ±5 days and rejects beyond", () => {
    expect(
      matchesTolerance(planned(), tx({ transaction_date: "2026-07-06" })),
    ).toBe(true);
    expect(
      matchesTolerance(planned(), tx({ transaction_date: "2026-06-26" })),
    ).toBe(true);
    expect(
      matchesTolerance(planned(), tx({ transaction_date: "2026-07-07" })),
    ).toBe(false);
  });

  it("rejects a different recipient cluster", () => {
    expect(matchesTolerance(planned(), tx({ recipient_cluster_id: 7 }))).toBe(
      false,
    );
  });
  ...
  it("handles pg Date objects for the date fields", () => {
    ...
    expect(
      matchesTolerance(p, tx({ transaction_date: new Date(2026, 6, 3) })),
    ).toBe(true);
  });

  it("rejects empty and malformed date values instead of treating them as nearby dates", () => {
    expect(matchesTolerance(planned({ planned_date: "" }), tx())).toBe(false);
    expect(matchesTolerance(planned(), tx({ transaction_date: "" }))).toBe(
      false,
    );
    expect(
      matchesTolerance(planned({ planned_date: "2026-02-30" }), tx()),
    ).toBe(false);
  });
});
...
describe("autoLinkTransactions", () => {
  beforeEach(() => {
    ...
    mockGetClusterRootMap.mockResolvedValue(
      new Map([
        [42, 42],
        [7, 7],
      ]),
  ...
  it("auto-executes a single unambiguous match", async () => {
    ...
    expect(res.links).toEqual([
      { plannedTransactionId: 1, transactionId: 1000 },
    ]);
    expect(mockExecutePlanned).toHaveBeenCalledWith({
      id: 1,
      executedTransactionId: 1000,
