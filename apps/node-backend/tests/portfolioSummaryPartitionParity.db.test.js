...
async function seedTxn(
  ...
  type,
  date,
  { units = null, amount = 0, accountId = null } = {},
) {
  const { rows } = await pool.query(
...
async function seed() {
  ...
  await seedTxn(fx.multi, "buy", "2026-01-01", {
    units: 100,
    amount: 1000,
    accountId: A,
  });
  await seedTxn(fx.multi, "buy", "2026-02-01", {
    units: 25,
    amount: 400,
    accountId: B,
  });
  await seedTxn(fx.multi, "buy", "2026-02-15", {
    units: 25,
    amount: 600,
    accountId: B,
  });
  await seedTxn(fx.multi, "buy", "2026-02-20", {
    units: 10,
    amount: 400,
    accountId: A,
  });
  await seedTxn(fx.multi, "sell", "2026-03-01", {
    units: 25,
    amount: 750,
    accountId: B,
  });
  ...
  await seedTxn(fx.split, "buy", "2026-01-01", {
    units: 10,
    amount: 1000,
    accountId: A,
  });
  await seedTxn(fx.split, "buy", "2026-01-02", {
    units: 10,
    amount: 1100,
    accountId: B,
  });
  await seedTxn(fx.split, "split", "2026-02-01", { units: 40 });
  await seedTxn(fx.split, "sell", "2026-03-01", {
    units: 15,
    amount: 900,
    accountId: B,
  });
  ...
  await seedTxn(fx.retag, "buy", "2026-01-01", {
    units: 30,
    amount: 300,
    accountId: A,
  });
  const movedLot = await seedTxn(fx.retag, "buy", "2026-01-10", {
    units: 20,
    amount: 500,
    accountId: A,
  });
  await pool.query(
    `UPDATE portfolio_transactions SET account_id = $1 WHERE id = $2`,
    [B, movedLot],
  );
  await seedTxn(fx.retag, "sell", "2026-02-01", {
    units: 10,
    amount: 400,
    accountId: B,
  });
  ...
  await seedTxn(fx.roc, "buy", "2026-01-01", {
    units: 100,
    amount: 1000,
    accountId: A,
  });
  await seedTxn(fx.roc, "buy", "2026-01-02", {
    units: 50,
    amount: 1000,
    accountId: B,
  });
  ...
  await seedTxn(fx.unassigned, "buy", "2026-01-01", {
    units: 60,
    amount: 600,
    accountId: A,
  });
  await seedTxn(fx.unassigned, "buy", "2026-01-05", { units: 40, amount: 400 });
  await seedTxn(fx.unassigned, "sell", "2026-02-01", {
    units: 10,
    amount: 150,
  });
}
...
describeDb(
  "WP-C4 parity invariant — Σ per-account ≡ global engine (real Postgres)",
  () => {
    ...
    it.each(["weighted_avg", "fifo", "lifo"])(
      ...
      async (method) => {
        ...
        const sum = (field) =>
          result.byAccount.reduce((s, a) => s + a[field], 0);
        const TOL = 0.02; // ≤ one banker's-rounding cent per aggregation side
        expect(
          Math.abs(sum("currentValue") - result.totals.totalPortfolioValue),
        ).toBeLessThanOrEqual(TOL);
        expect(
          Math.abs(sum("totalInvested") - result.totals.totalInvested),
        ).toBeLessThanOrEqual(TOL);
        expect(
          Math.abs(sum("realizedGain") - result.totals.totalRealizedGain),
        ).toBeLessThanOrEqual(TOL);
        expect(
          Math.abs(sum("unrealizedGain") - result.totals.totalUnrealizedGain),
        ).toBeLessThanOrEqual(TOL);
        expect(
          Math.abs(sum("gainLoss") - result.totals.totalGainLoss),
        ).toBeLessThanOrEqual(TOL);
        ...
        expect(result.byAccount.map((a) => a.account_id)).toEqual([
          fx.ibkr,
          fx.degiro,
          null,
        ]);
        ...
        const flagByName = new Map(
          result.summaries.map((s) => [s.name, s.fullyAssigned]),
        );
        ...
        const unassigned = result.summaries.find(
          (s) => s.name === "WPC4P UNASSIGNED",
        );
    ...
    it("re-tagging a lot BACK moves value and basis between account rows with no global change", async () => {
      ...
      expect(
        row(after, fx.ibkr).currentValue - row(before, fx.ibkr).currentValue,
      ).toBe(400);
      expect(
        row(after, fx.degiro).currentValue -
          row(before, fx.degiro).currentValue,
      ).toBe(-200);
      ...
      for (const [field, total] of [
        ...
        expect(
          Math.abs(sum(after, field) - after.totals[total]),
