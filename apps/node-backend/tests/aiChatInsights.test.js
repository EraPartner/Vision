...
describe("getBankBalances", () => {
      ...
      accounts: [
        {
          ...
          first_transaction: "2025-01-02",
          last_transaction: "2025-04-01",
        },
      ],
    ...
    expect(result.data).toEqual([
      {
        ...
        currency: "EUR",
        transactionCount: 10,
        firstTransaction: "2025-01-02",
        lastTransaction: "2025-04-01",
      },
    ]);
    expect(result.meta).toMatchObject({
      totalNetPosition: 1234.5,
      accountCount: 1,
      currency: "EUR",
      renderAs: "table",
    });
  ...
  it("filters balances to explicitly scoped account ids", async () => {
    infoRepository.getBankBalances.mockResolvedValueOnce({
      accounts: [
        { account_id: 1, bank_account: "A", balance: "10" },
        { account_id: 2, bank_account: "B", balance: "20" },
      ],
    });
    const result = await getBankBalances.run(
      {},
      { scope: { accountIds: [2], currency: "EUR" } },
    );
    expect(result.data).toEqual([
      expect.objectContaining({ accountId: 2, account: "B", balance: 20 }),
    ]);
    expect(result.meta.totalNetPosition).toBe(20);
  });
      ...
      accounts: [
        {
          ...
          balance: 0,
          transaction_count: 0,
          first_transaction: new Date(2024, 11, 31),
          last_transaction: new Date(2025, 5, 15),
        },
      ],
...
describe("getSpendingPace", () => {
  function fixturePayload() {
    return {
      past_6_months: {
        avg_daily_spending: 15.5,
        avg_monthly_spending: 465,
        months_counted: 6,
      },
      current_month: {
        total_spending: 200,
      ...
      comparison: {
        projected_monthly_total: 428.57,
        pace: "on_track",
        variance: -7.84,
      },
    };
  ...
  it("defaults to monthly normalisation", async () => {
    ...
    expect(
      r.data.find((d) => d.label.includes("Projected monthly")).value,
    ).toBe(428.57);
  });
  ...
  it("multiplies monthly figures by 12 for yearly normalisation", async () => {
    ...
    expect(r.data.find((d) => d.label.includes("Avg yearly")).value).toBe(
      465 * 12,
    );
    expect(r.data.find((d) => d.label.includes("Projected yearly")).value).toBe(
      428.57 * 12,
    );
  });

  it("throws ToolValidationError on unknown period", async () => {
    await expect(getSpendingPace.run({ period: "weekly" })).rejects.toThrow(
...
describe("getRecipientInsights", () => {
  ...
  it("passes recipientId filter through to the repository", async () => {
    ...
    expect(transactionRepository.getAll).toHaveBeenCalledWith(
      expect.objectContaining({ recipientId: 7, active: true }),
    );
  });
  ...
  it("rejects invalid recipientId", async () => {
    await expect(getRecipientInsights.run({ recipientId: 0 })).rejects.toThrow(
      /recipientId/,
    );
  });
});
...
describe("getWatchlist", () => {
  it("reshapes raw rows and merges live quote prices by symbol", async () => {
    ...
    getQuotes.mockResolvedValueOnce({
      items: [{ symbol: "AAPL", price: 174.99 }],
      total: 1,
    });
  ...
  it("rejects unknown asset class", async () => {
    await expect(
      getWatchlist.run({ assetClass: "magic-beans" }),
...
describe("getCategories", () => {
  ...
  it("passes search trimmed to the repository", async () => {
    ...
    expect(categoryRepository.getAll).toHaveBeenCalledWith(
      expect.objectContaining({ search: "Travel" }),
    );
  });
...
describe("getRecurringDetected", () => {
  ...
  it("reshapes patterns and applies default minOccurrences=3", async () => {
    detectRecurringPatterns.mockResolvedValueOnce({
      patterns: [
        ...
        pattern({ recipientName: "Spotify", occurrences: 2 }),
      ],
    });
  ...
  it("rejects out-of-range minOccurrences", async () => {
    await expect(
      getRecurringDetected.run({ minOccurrences: 1 }),
    ).rejects.toThrow(/minOccurrences/);
    await expect(
      getRecurringDetected.run({ minOccurrences: 999 }),
    ).rejects.toThrow(/minOccurrences/);
  });
});
