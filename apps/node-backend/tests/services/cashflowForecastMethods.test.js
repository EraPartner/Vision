...
function syntheticHistory({ startIso = '2024-01-01', days = 365 * 2, amplitude = 10, noise = 0 } = {}) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    const base = amplitude * Math.sin((2 * Math.PI * i) / 7) + (dow === 0 ? -5 : 2);
...
describe('prng', () => {
  ...
  it('gaussian returns finite samples', () => {
    const rng = makeRng('g');
    for (let i = 0; i < 50; i++) expect(Number.isFinite(gaussian(rng))).toBe(true);
  });
});
...
describe('seasonality', () => {
  it('dayOfWeek / dayOfMonth parse ISO dates', () => {
    ...
    expect(dayOfWeek('2026-04-24')).toBe(new Date(Date.UTC(2026, 3, 24)).getUTCDay());
  });
...
describe('simpleAverage', () => {
  it('returns mean-per-DOM and correct length', () => {
    ...
    const out = simpleAverage.forecast({ history: hist, forecastDates: ['2026-04-01', '2026-04-02'] });
...
describe('weightedAverage', () => {
  it('weights recent months heavier than older ones', () => {
    ...
    const out = weightedAverage.forecast({ history: hist, forecastDates: ['2026-04-01'] });
...
describe('monteCarloParametric', () => {
  it('is deterministic per seed', () => {
    ...
    const a = monteCarloParametric.forecast({ history: hist, forecastDates: dates, paths: 200, seed: 'abc' });
    const b = monteCarloParametric.forecast({ history: hist, forecastDates: dates, paths: 200, seed: 'abc' });
  ...
  it('returns monotone percentiles', () => {
    const hist = syntheticHistory({ days: 120 });
    const dates = futureDates({ days: 4 });
    const out = monteCarloParametric.forecast({ history: hist, forecastDates: dates, paths: 500, seed: 's' });
    for (let i = 0; i < dates.length; i++) {
      expect(out.bands.p10[i].value).toBeLessThanOrEqual(out.bands.p50[i].value);
      expect(out.bands.p50[i].value).toBeLessThanOrEqual(out.bands.p90[i].value);
    }
  });

  it('handles empty forecast horizon', () => {
    const out = monteCarloParametric.forecast({ history: [], forecastDates: [], paths: 10 });
...
describe('monteCarloBlockBootstrap', () => {
  it('is deterministic per seed', () => {
    const hist = syntheticHistory({ days: 120 });
    const dates = futureDates({ days: 4 });
    const a = monteCarloBlockBootstrap.forecast({ history: hist, forecastDates: dates, paths: 200, seed: 'xyz' });
    const b = monteCarloBlockBootstrap.forecast({ history: hist, forecastDates: dates, paths: 200, seed: 'xyz' });
    expect(a.series).toEqual(b.series);
  });
  ...
  it('returns monotone percentiles on realistic history', () => {
    const hist = syntheticHistory({ days: 120 });
    const dates = futureDates({ days: 4 });
    const out = monteCarloBlockBootstrap.forecast({ history: hist, forecastDates: dates, paths: 500, seed: 's' });
    for (let i = 0; i < dates.length; i++) {
      expect(out.bands.p10[i].value).toBeLessThanOrEqual(out.bands.p50[i].value);
      expect(out.bands.p50[i].value).toBeLessThanOrEqual(out.bands.p90[i].value);
    }
  ...
  it('handles empty history gracefully', () => {
    ...
    const out = monteCarloBlockBootstrap.forecast({ history: [], forecastDates: dates, paths: 10 });
...
describe('walkForwardBacktest', () => {
  it('computes per-method aggregate metrics', () => {
    const hist = syntheticHistory({ days: 400 });
    const methods = [
      { id: simpleAverage.id, label: simpleAverage.label, forecast: simpleAverage.forecast },
    ];
    const result = walkForwardBacktest({ history: hist, methods, asOfMonth: '2025-02', windowMonths: 3 });
  ...
  it('short-circuits methods with no training history', () => {
    ...
    const result = walkForwardBacktest({ history: [], methods, asOfMonth: '2025-02', windowMonths: 2 });
    expect(result[0].aggregate).toEqual({ mae: 0, rmse: 0, mape: 0, months: 0 });
  });
});
...
describe('ensemble', () => {
  ...
  it('forecast applies explicit weights correctly', () => {
    ...
    const weights = new Map([['simple_avg', 0.25], ['ewma', 0.75]]);
    const result = ensemble.forecast({ forecastDates: dates, methodOutputs: outputs, weights });
  ...
  it('forecast skips errored methods', () => {
    const outputs = [
      ...
      { id: 'ewma', series: dates.map((date) => ({ date, value: 20 })), error: 'forecast_failed' },
    ];
    const result = ensemble.forecast({ forecastDates: dates, methodOutputs: outputs, weights: new Map() });
  ...
  it('forecast returns zeros when all methods errored', () => {
    const outputs = [
      { id: 'simple_avg', series: dates.map((date) => ({ date, value: 10 })), error: 'forecast_failed' },
    ];
    const result = ensemble.forecast({ forecastDates: dates, methodOutputs: outputs, weights: new Map() });
...
describe('accuracyStore', () => {
  ...
  it('records and retrieves latest by method', async () => {
    await recordAccuracy({ userId: 'u1', methodId: 'simple_avg', asOfMonth: '2026-03', mae: 1, rmse: 2, mape: 0.1, sampleDays: 30 });
    await recordAccuracy({ userId: 'u1', methodId: 'simple_avg', asOfMonth: '2026-04', mae: 0.5, rmse: 1, mape: 0.05, sampleDays: 30 });
  ...
  it('returns history sorted newest-first', async () => {
    await recordAccuracy({ userId: 'u1', methodId: 'ewma', asOfMonth: '2026-01', mae: 1, rmse: 1, mape: 0, sampleDays: 30 });
    await recordAccuracy({ userId: 'u1', methodId: 'ewma', asOfMonth: '2026-03', mae: 2, rmse: 2, mape: 0, sampleDays: 30 });
...
describe('orchestrator computeCashflowForecast', () => {
  ...
  it('returns envelope with all methods and backtest diagnostics', async () => {
    ...
    const { computeCashflowForecast } = await import('../../src/services/calculations/forecast/index.js');
    const env = await computeCashflowForecast({ mcPaths: 100, includeBacktest: true, userId: 'u1' });
  ...
  it('ensemble_imse present in methods output', async () => {
    ...
    const { computeCashflowForecast } = await import('../../src/services/calculations/forecast/index.js');
    const env = await computeCashflowForecast({ mcPaths: 50, includeBacktest: false });
...
describe('reconcileCategoryForecasts', () => {
  ...
  it('scales categories so their sum matches reference', () => {
    ...
    const categoryForecasts = [
      { cat: { key: 'a', category_id: 1, general: 'G', detail: 'D' }, series: [{ date: '2026-04-26', value: 2 }, { date: '2026-04-27', value: 4 }] },
      { cat: { key: 'b', category_id: 2, general: 'H', detail: 'E' }, series: [{ date: '2026-04-26', value: 3 }, { date: '2026-04-27', value: 6 }] },
    ];

    const result = reconcileCategoryForecasts(categoryForecasts, future, refByDate);
  ...
  it('preserves cat metadata', () => {
    ...
    const cat = { key: 'x', category_id: 99, general: 'Food', detail: 'Groceries' };
    const categoryForecasts = [
...
describe('buildCategoryBreakdown', () => {
  ...
  const historyByCategory = [
    { date: '2025-04-03', category_id: 1, general: 'Income', detail: 'Salary', net: 100 },
    { date: '2025-04-03', category_id: 2, general: 'Expense', detail: 'Food', net: -30 },
  ];

  const currentActualByCategory = [
    { date: '2026-04-01', category_id: 1, general: 'Income', detail: 'Salary', net: 120 },
    { date: '2026-04-02', category_id: 1, general: 'Income', detail: 'Salary', net: 50 },
    { date: '2026-04-01', category_id: 2, general: 'Expense', detail: 'Food', net: -20 },
  ];

  const referenceDaily = [{ date: '2026-04-03', value: 70 }]; // simple_avg aggregate

  it('returns one entry per category', () => {
    const result = buildCategoryBreakdown({ historyByCategory, currentActualByCategory, future, all, todayDay, referenceDaily });
