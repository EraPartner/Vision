...
describe("computeCashflowForecastRolling — MC cache", () => {
  ...
  it("cache hit with fresh entry → cached payload returned, DB not called", async () => {
    ...
    expect(
      infoRepository.getCashflowForecastDataRolling,
  ...
  it("effective app date changes the monthly cache identity at midnight", () => {
    ...
    const beforeMidnight = filterHash({
      ...common,
      effectiveDate: "2026-04-15",
    });
    const afterMidnight = filterHash({
      ...common,
      effectiveDate: "2026-04-16",
    });
    expect(beforeMidnight).not.toBe(afterMidnight);
    expect(beforeMidnight).toContain("|d2026-04-15|");
    expect(afterMidnight).toContain("|d2026-04-16|");
  });
});
