...
const REJECTED = {
  ...
  // parseAmountFilter returns undefined for anything unparseable, so the bound was
...
describe("normalizeBulkFilter — rejects instead of silently ignoring", () => {
  ...
  it("rejects one field spelled both ways rather than silently preferring one", () => {
    expect(() =>
      normalizeBulkFilter({ categoryId: 7, category_id: 9 }),
    ).toThrow(/not both/i);
    expect(() => normalizeBulkFilter({ tagSlugs: ["a"], tags: ["b"] })).toThrow(
      /not both/i,
    );
  });
  ...
  it("rejects an over-long search instead of truncating it", () => {
    ...
    expect(() => normalizeBulkFilter({ search: "x".repeat(201) })).toThrow(
      ValidationError,
    );
    expect(
      normalizeBulkFilter({ search: "x".repeat(200) }).search,
    ).toHaveLength(200);
  });
});
...
describe("normalizeBulkFilter — accepts what real callers send", () => {
  ...
  it('still resolves the whole-table selection the "select all N matching" flow sends', async () => {
    ...
    expect(await resolveBulkSelection({ filter: { active: true } })).toEqual([
      11, 12,
    ]);
  });
  ...
  it("keeps amount magnitude vs signed semantics", () => {
    ...
    expect(
      normalizeBulkFilter({ amount_min: -50, amount_signed: true }).amountMin,
    ).toBe(-50);
  });
