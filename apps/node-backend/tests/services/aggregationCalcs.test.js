...
function expectEnvelope(envelope, { source }) {
  ...
  expect(new Date(envelope.meta.computedAt).toString()).not.toBe(
    "Invalid Date",
  );
}
...
describe("computeRecipientInsights", () => {
  ...
  it("forwards the statistics date range", async () => {
    ...
    await computeRecipientInsights({
      targetCurrency: "USD",
      startDate: "2024-10-01",
      endDate: "2026-09-07",
    });
    expect(infoRepository.getRecipientInsights).toHaveBeenCalledWith("USD", {
      excludedCategoryIds: [],
      excludedRecipientIds: [],
      startDate: "2024-10-01",
      endDate: "2026-09-07",
...
describe("computeCashflowComparison", () => {
  it("tags source=mv when both exclusion lists are empty", async () => {
    ...
    expect(infoRepository.getCashflowComparison).toHaveBeenCalledWith(
      [],
      [],
      "EUR",
    );
...
describe("computeAverageVsCurrent", () => {
  it("always tags source=live (no MV backing in Phase 2)", async () => {
    ...
    expect(infoRepository.getAverageVsCurrentSpending).toHaveBeenCalledWith(
      "EUR",
    );
    expectEnvelope(env, { source: "live" });
  });
