...
import {
  __parseRecurrenceStep as parseRecurrenceStep,
  nextOccurrenceYmd,
  fastForwardYmd,
  calculateNextDate,
  isValidPattern,
 ...
 * the equivalence blocks pin its relationship to the Date-returning stepper:
 * both advance the APP_TIMEZONE calendar exactly, including across daylight-
 * saving transitions. Default APP_TIMEZONE is Europe/Brussels.
 */
...
describe("string-space vs Date-space steppers", () => {
  ...
  it("month-based patterns produce identical sequences from ANY anchor (incl. month-end and leap day)", () => {
    for (const pattern of ["monthly", "quarterly", "yearly"]) {
      for (const anchor of [
        ...
        expect(stringWalk(anchor, pattern, 30)).toEqual(
          dateWalk(anchor, pattern, 30),
        );
      }
    }
  });
  ...
  it("keeps summer-anchored day cadences calendar-exact across fall-back", () => {
    // 2026-10-25 is Europe/Brussels' fall-back Sunday. Both APIs advance on
    // the local calendar rather than preserving a fixed UTC duration.
    ...
    expect(stringWalk("2026-10-24", "daily", 2)).toEqual([
      "2026-10-25",
      "2026-10-26",
    ]);
    expect(dateWalk("2026-10-24", "daily", 2)).toEqual([
      "2026-10-25",
      "2026-10-26",
    ]);
  });
...
describe("fastForwardYmd (optional fast-forward)", () => {
  ...
  it("jumps a stale daily anchor to the day before the target", () => {
    ...
    expect(fastForwardYmd("2025-11-01", "daily", "2026-07-01")).toBe(
      "2026-06-30",
    );
  });
  ...
  it("is a no-op within one step of the target, past the target, for month-based patterns, and for unadvanceable patterns", () => {
    expect(fastForwardYmd("2026-06-25", "weekly", "2026-07-01")).toBe(
      "2026-06-25",
    ); // deficit 6 < 7
    expect(fastForwardYmd("2026-06-24", "weekly", "2026-07-01")).toBe(
      "2026-06-24",
    ); // deficit == step
    expect(fastForwardYmd("2026-08-15", "weekly", "2026-07-01")).toBe(
      "2026-08-15",
    ); // negative deficit
    ...
    expect(fastForwardYmd("2020-01-31", "monthly", "2026-07-01")).toBe(
      "2020-01-31",
    );
    expect(fastForwardYmd("2020-01-31", "yearly", "2026-07-01")).toBe(
      "2020-01-31",
    );
    expect(fastForwardYmd("2020-01-31", "fortnightly", "2026-07-01")).toBe(
      "2020-01-31",
    );
  });
