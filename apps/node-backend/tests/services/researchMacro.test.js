...
import { trimToRange, periodToMs } from '../../src/services/research/adapters/macroRange.js';
import {
  searchCatalog,
  isValidSeriesId,
  MACRO_CATALOG,
} from '../../src/services/research/adapters/macroCatalog.js';
import { parseJsonStat } from '../../src/services/research/adapters/eurostatAdapter.js';
...
describe('macroCatalog', () => {
  it('searchCatalog AND-matches terms over title/region/keywords', () => {
    expect(searchCatalog('eurostat', 'belgium inflation').map((i) => i.region)).toEqual(['BE']);
    expect(searchCatalog('eurostat', 'unemployment').length).toBeGreaterThanOrEqual(2);
  ...
  it('isValidSeriesId enforces per-provider shapes', () => {
    ...
    expect(isValidSeriesId('eurostat', 'prc_hicp_midx?geo=BE&coicop=CP00&unit=I15')).toBe(true);
...
describe('eurostat parseJsonStat', () => {
  it('zips the time index with values, skips missing, and sorts ascending', () => {
    const payload = {
      dimension: { time: { category: { index: { '2020-02': 1, '2020-01': 0, '2020-03': 2 } } } },
...
const macroAdapters = (over = {}) => ({
  fred: { macroSearch: vi.fn(async () => ({ items: [{ provider: 'fred', seriesId: 'CPIAUCSL', title: 'US CPI' }] })), macroSeries: vi.fn(async () => ({ provider: 'fred', seriesId: 'CPIAUCSL', points: [{ time: 1, close: 1 }] })) },
  eurostat: { macroSearch: vi.fn(async () => ({ items: [{ provider: 'eurostat', seriesId: 'x?geo=BE', title: 'BE HICP' }] })), macroSeries: vi.fn(async () => ({ provider: 'eurostat', seriesId: 'x?geo=BE', points: [{ time: 2, close: 2 }] })) },
  dbnomics: { macroSearch: vi.fn(async () => ({ items: [] })), macroSeries: vi.fn(async () => ({ provider: 'dbnomics', seriesId: 'A/B/C', points: [] })) },
  ...over,
});
...
describe('researchAggregator.searchMacro', () => {
  it('unions items across all usable macro providers', async () => {
    ...
    expect(out.items.map((i) => i.provider).sort()).toEqual(['eurostat', 'fred']);
  });

  it('excludes providers without a key (e.g. no FRED key)', async () => {
    const adapters = macroAdapters();
    const agg = build({ adapters, governor: makeGovernor(), isKeyed: (p) => p !== 'fred' });
    const out = await agg.searchMacro('cpi');
...
describe('researchAggregator.fetchMacroSeries', () => {
  it('routes to exactly the named provider (no fallback) and caches', async () => {
    ...
    const out = await agg.fetchMacroSeries({ provider: 'eurostat', seriesId: 'x?geo=BE', range: '1y' });
    expect(out.source).toBe('live');
    expect(out.provider).toBe('eurostat');
    expect(out.data.points).toEqual([{ time: 2, close: 2 }]);
    expect(adapters.eurostat.macroSeries).toHaveBeenCalledWith('x?geo=BE', { range: '1y' });
    expect(adapters.fred.macroSeries).not.toHaveBeenCalled();

    const cached = await agg.fetchMacroSeries({ provider: 'eurostat', seriesId: 'x?geo=BE', range: '1y' });
  ...
  it('reports unavailable (no_key) when the provider is unkeyed', async () => {
    const agg = build({ adapters: macroAdapters(), governor: makeGovernor(), isKeyed: () => false });
    const out = await agg.fetchMacroSeries({ provider: 'fred', seriesId: 'CPIAUCSL', range: '1y' });
    expect(out.source).toBe('unavailable');
    expect(out.attempted[0]).toMatchObject({ provider: 'fred', skipped: 'no_key' });
  });

  it('reports unavailable (quota) when the governor blocks the spend', async () => {
    const agg = build({ adapters: macroAdapters(), governor: makeGovernor(() => false) });
    const out = await agg.fetchMacroSeries({ provider: 'fred', seriesId: 'CPIAUCSL', range: '1y' });
    expect(out.source).toBe('unavailable');
    expect(out.attempted[0]).toMatchObject({ provider: 'fred', skipped: 'quota' });
  });

  it('reports unavailable with the error message when the adapter throws', async () => {
    const adapters = macroAdapters({
      eurostat: { macroSeries: vi.fn(async () => { throw new Error('boom'); }) },
    });
    const agg = build({ adapters, governor: makeGovernor() });
    const out = await agg.fetchMacroSeries({ provider: 'eurostat', seriesId: 'x?geo=BE', range: '1y' });
    expect(out.source).toBe('unavailable');
    expect(out.attempted[0]).toMatchObject({ provider: 'eurostat', error: 'boom' });
  });
});
