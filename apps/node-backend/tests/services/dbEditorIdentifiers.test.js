...
beforeEach(() => {
  ...
  query.mockImplementation(async (sql) => {
    ...
    if (sql.includes('information_schema.columns')) {
      return { rows: CATALOG_COLUMNS.map((c, i) => catalogColumnRow(c, i + 1)) };
    }
...
describe('table identifiers', () => {
  it('rejects a table that is not in the catalog', async () => {
    await expect(readRows('transactions; DROP TABLE users')).rejects.toThrow(/Unknown table/);
...
describe('filter identifiers', () => {
  it('rejects a filter column that is not in the catalog', async () => {
    await expect(
      readRows(CATALOG_TABLE, { filters: [{ column: 'amount) OR pg_sleep(5)--', op: 'eq', value: 1 }] }),
  ...
  it('rejects an operator outside the allowlist', async () => {
    await expect(
      readRows(CATALOG_TABLE, { filters: [{ column: 'amount', op: 'raw', value: 1 }] }),
    ).rejects.toThrow(/Unknown filter operator/);
  });

  it('parameterizes the value and interpolates only the catalog column', async () => {
    await readRows(CATALOG_TABLE, { filters: [{ column: 'memo', op: 'eq', value: "'; DROP TABLE x--" }] });
  ...
  it('still rejects the removed raw WHERE escape hatch', async () => {
    await expect(readRows(CATALOG_TABLE, { where: '1=1' })).rejects.toThrow(/raw WHERE parameter has been removed/);
    expect(clientSql).toEqual([]);
  });
});
...
describe('sort identifiers', () => {
  it('rejects a sort column that is not in the catalog', async () => {
    await expect(readRows(CATALOG_TABLE, { orderBy: 'id; DROP TABLE users' })).rejects.toThrow(/Unknown sort column/);
    expect(clientSql).toEqual([]);
  });

  it('constrains the sort direction to ASC or DESC', async () => {
    await readRows(CATALOG_TABLE, { orderBy: 'amount', dir: 'DESC; DROP TABLE users' });
    const dataSql = clientSql.find((s) => s.startsWith('SELECT *'));
...
describe('pagination', () => {
  it('coerces limit and offset to clamped integers', async () => {
    await readRows(CATALOG_TABLE, { limit: '10; DROP TABLE users', offset: '-5' });
    const dataSql = clientSql.find((s) => s.startsWith('SELECT *'));
...
describe('mutation identifiers', () => {
  it('drops write columns that are not in the catalog instead of quoting them', async () => {
    ...
    expect(res.statements[0].preview).toContain('INSERT INTO "transactions" ("memo")');
    expect(res.statements[0].preview).not.toContain('VALUES (1)--');
  });

  it('rejects a batch whose columns are all unknown', async () => {
    await expect(
      applyMutations(CATALOG_TABLE, [{ op: 'insert', values: { 'nope--': 1 } }], { dryRun: true }),
  ...
  it('builds the UPDATE predicate from the catalog primary key', async () => {
    ...
    expect(res.statements[0].preview).toContain('UPDATE "transactions" SET "memo"');
