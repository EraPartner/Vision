...
describe("expandOccurrences", () => {
  ...
  it("returns the single date for a non-recurring row within the horizon", () => {
    expect(
      expandOccurrences(
        { planned_date: "2026-02-10", is_recurring: false },
        "2026-06-01",
      ),
    ).toEqual(["2026-02-10"]);
  });
  ...
  it("returns an empty list for a non-recurring row past the horizon", () => {
    expect(
      expandOccurrences(
        { planned_date: "2026-07-10", is_recurring: false },
        "2026-06-01",
      ),
    ).toEqual([]);
  });
  ...
  it("keeps a summer-anchored weekly row on its calendar cadence across fall-back", () => {
    const out = expandOccurrences(
      {
  ...
  it("emits each fall-back calendar day once and includes the horizon", () => {
    const out = expandOccurrences(
      {
