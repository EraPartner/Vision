...
const buildHistory = ({ days = 400 } = {}) => {
  ...
  for (let i = 0; i < days; i++) {
    ...
    out.push({
      date: iso,
      net: 5 * Math.sin((2 * Math.PI * i) / 7) + (dow === 0 ? -3 : 1),
    });
  }
  return out;
};
  ...
  infoRepository: {
    ...
    getCashflowForecastDataRolling: vi.fn(
      async (historyMonths, daysBack, daysForward) => ({
        ...
        scheduledActual: [{ date: isoOffsetFromToday(3), net: -75 }],
        plannedCurrent: [
...
describe("computeCashflowForecastRolling", () => {
  ...
  it("returns shape with actual length === daysBack + 1 and forecast length === daysForward", async () => {
    ...
    expect(env.data.scheduled_actual).toEqual([
      { date: isoOffsetFromToday(3), net: -75 },
    ]);
