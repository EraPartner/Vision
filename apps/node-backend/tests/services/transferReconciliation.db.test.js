...
describe.skipIf(!hasTestDatabase())(
  "services/transferReconciliationService (real DB)",
  () => {
    beforeAll(async () => {
    ...
    afterEach(async () => {
      ...
      await pool.query(
        `DELETE FROM user_settings WHERE key = 'transfers_backfilled'`,
      );
    });
    ...
    describe("reconcileTransfers — auto-pairing", () => {
      it("pairs an unambiguous equal-and-opposite match across two accounts", async () => {
        ...
        await expect(reconcileTransfers()).resolves.toEqual({
          pairsCreated: 1,
        });
      ...
      it("leaves an ambiguous match open instead of guessing", async () => {
        ...
        const inA = await insertTxn({
          account: "Savings",
          amount: 200,
          date: daysAfter(1),
        });
        const inB = await insertTxn({
          account: "Brokerage",
          amount: 200,
          date: daysAfter(1),
        });
    ...
    describe("reconcileTransfers — release paths", () => {
      ...
      it("does not release current system rows that are transfers with no peer", async () => {
        await seedFixtures();
        // Opening anchors and adjustments are is_transfer=true with a NULL peer
        // but are NOT reconciler-owned pairs — they must survive.
        ...
        const adjustmentId = await insertTxn({
          account: "Brokerage",
          amount: -500,
          isTransfer: true,
          source: "adjustment",
        ...
        expect(await readTxn(adjustmentId)).toMatchObject({
          is_transfer: true,
    ...
    describe("markTransfer", () => {
      it("marks a cross-currency pair that auto-detection would reject", async () => {
        await seedFixtures();
        const aId = await insertTxn({
          account: "Checking",
          amount: -100,
          currency: "EUR",
        });
        const bId = await insertTxn({
          account: "Savings",
          amount: 108.4,
          currency: "USD",
        });
      ...
      it("rejects two legs on the same account", async () => {
        ...
        await expect(markTransfer(aId, bId)).rejects.toThrow(
      ...
      it("rejects a missing leg", async () => {
        await seedFixtures();
        const aId = await insertTxn({ account: "Checking", amount: -30 });
        await expect(markTransfer(aId, aId + 100_000)).rejects.toThrow(
          /must exist/i,
        );
        expect((await readTxn(aId)).is_transfer).toBe(false);
      });
