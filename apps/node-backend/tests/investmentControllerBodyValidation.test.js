...
describe('portfolio transaction request body validation', () => {
  it.each([
    ...
    [{ recurrence_end_date: '2026\/12\/31' }, /recurrence_end_date must be in YYYY-MM-DD format/],
    [{ note: 123 }, /note/],
  ...
  it('normalizes valid fields while preserving field-specific clear values', () => {
    expect(parsePortfolioTransactionBody({
      type: 'buy',
      date: '2026-01-15',
      amount: '1200.5',
      fx_rate_to_eur: null,
  ...
  it('accepts compatible numeric strings within the safe money boundary', () => {
    ...
    expect(parsePortfolioTransactionBody({ amount: '1e3' })).toEqual({ amount: 1000 });
    expect(parsePortfolioTransactionBody({ price_per_unit: '999999999999' })).toEqual({
...
describe('portfolio transaction controller validation boundary', () => {
  it('rejects a malformed create body before repository create', async () => {
    vi.spyOn(investmentRepository, 'getById').mockResolvedValue({ id: 1, currency: 'EUR' });
    const createSpy = vi.spyOn(portfolioTransactionRepository, 'create');

    await expect(createTransaction({
      params: { id: 1 },
      body: { type: 'buy', date: '2026-01-15', amount: 1000, is_recurring: 'false' },
    }, {})).rejects.toMatchObject({ status: 400, code: 'VALIDATION_ERROR' });
    expect(createSpy).not.toHaveBeenCalled();
  });

  it('rejects a malformed patch body before repository update', async () => {
    const updateSpy = vi.spyOn(portfolioTransactionRepository, 'update');
