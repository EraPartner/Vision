...
describe("detectSubscriptionCreep", () => {
  ...
  it("a 'new' dismissal removes the new finding but keeps the priceChange for the same recipient", async () => {
    ...
    expect(digest.priceChanges[0]).toMatchObject({
      recipientId: 1,
      findingType: "priceChange",
    });
  });
  ...
  it("caps each list to the top 5 by confidence descending", async () => {
    ...
    expect(digest.priceChanges.map((f) => f.confidence)).toEqual([
      90, 85, 80, 75, 70,
    ]);
  });

  it("refills the top five after a higher-ranked finding is dismissed", async () => {
    detectRecurringPatterns.mockResolvedValue({
      patterns: [95, 90, 85, 80, 75, 70].map((confidence, index) =>
        pattern({
          recipientId: index + 1,
          recipientName: `Service ${index + 1}`,
          confidence,
        }),
      ),
      total: 6,
    ...
    expect(digest.new.map((finding) => finding.confidence)).toEqual([
      90, 85, 80, 75, 70,
    ]);
  });
...
describe("buildSubscriptionCreep", () => {
  ...
  it("tolerates a missing patterns array and defaults dismissRecords to none", () => {
    ...
    expect(buildSubscriptionCreep(undefined)).toEqual({
      new: [],
      priceChanges: [],
    });
  });
