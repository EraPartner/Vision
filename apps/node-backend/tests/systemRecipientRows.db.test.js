...
describe.skipIf(!hasTestDatabase())(
  "system-generated ledger rows + their recipient (real DB)",
  () => {
    beforeAll(async () => {
      expect(
    ...
    describe("reconcile 'adjustment'", () => {
      ...
      it("clears the drift of a multi-currency account in the base partition currency", async () => {
        ...
        await seedRecipient();
        const id = await addAccount("WISE MULTI", {
    ...
    describe("merge vs. opening anchors", () => {
      it("refuses the merge with a 400 instead of a raw 23505, leaving both anchors intact", async () => {
        ...
        await setOpeningBalance(survivor, {
          balance: 1000,
          date: "2020-01-01",
        });
        ...
        expect(err.message).not.toMatch(
          /duplicate key|uq_transactions_opening_anchor/,
        );
        ...
        expect(accounts.map((r) => r.id)).toEqual(
          [survivor, source].sort((a, b) => a - b),
        );
        const anchors = await systemRows();
        expect(anchors).toHaveLength(2);
        expect(new Set(anchors.map((r) => r.account_id))).toEqual(
          new Set([survivor, source]),
        );
      });
      ...
      it("allows the merge when the two anchors are in different currencies", async () => {
        ...
        await setOpeningBalance(source, {
          balance: 400,
          date: "2020-01-01",
          currency: "USD",
        });
        ...
        expect(
          (await previewMerge(source, survivor)).openingAnchorCollision,
        ).toBe(false);
    ...
    describe("system recipient", () => {
      it("is created once, inactive, and shared by both services", async () => {
        ...
        expect(new Set(generated.map((r) => r.recipient_id))).toEqual(
          new Set([rows[0].id]),
        );
      });
      ...
      it("does not write the row on the resolve path (no xmin/updated_at churn)", async () => {
        ...
        const version = async () =>
          (
            await getTestPool().query(
              `SELECT xmin::text AS xmin, updated_at FROM recipients WHERE id = $1`,
              [id],
            )
          ).rows[0];
      ...
      it("resolves idempotently and stays out of the active-only recipient list", async () => {
        ...
        const active = await recipientRepository.getAll({
          active: true,
          limit: 100,
        });
        ...
        expect((await recipientRepository.getById(first)).name).toBe(
          SYSTEM_RECIPIENT_NAME,
        );
      });
      ...
      it("adopts an existing recipient of the same name rather than deactivating it", async () => {
        ...
        expect(await recipientRepository.getOrCreateSystemId()).toBe(
          rows[0].id,
        );
        const after = await getTestPool().query(
          `SELECT is_active FROM recipients WHERE id = $1`,
          [rows[0].id],
        );
