...
async function monthKey(monthsBack) {
  const { rows } = await getTestPool().query(
    anchored(`SELECT to_char(date_trunc('month', CURRENT_DATE) - interval '${monthsBack} months', 'YYYY-MM') AS k`),
  );
  return rows[0].k;
}
...
const byMonth = (result) =>
  Object.fromEntries(result.months.map((m) => [`${m.year}-${String(m.month).padStart(2, '0')}`, m]));
...
describe.skipIf(!hasTestDatabase())('repositories/infoRepositoryMonthly (real DB)', () => {
  beforeAll(async () => {
    expect(
  ...
  describe('live path', () => {
    it('returns exactly the 6-month window, zero-filling months with no rows', async () => {
      ...
      expect(r.months.map((m) => `${m.year}-${String(m.month).padStart(2, '0')}`)).toEqual(expectedKeys);

      const months = byMonth(r);
      expect(months[await monthKey(2)]).toMatchObject({
        total_spending: -25, total_income: 0, net_amount: -25, transaction_count: 1,
      });
    ...
    it('emits period_start/period_end as calendar-day strings covering the whole month', async () => {
      ...
      expect(current.period_start).toBe(await ymd(`date_trunc('month', CURRENT_DATE)`));
      expect(current.period_end).toBe(
        await ymd(`date_trunc('month', CURRENT_DATE) + interval '1 month' - interval '1 day'`),
      );
    ...
    it('splits income from spending per month and rolls the window into summary', async () => {
      ...
      await insertTxn({ dateExpr: monthDay(0, 10), amount: '-9999.00', isActive: false });
      ...
      expect(months[await monthKey(1)]).toMatchObject({
        total_income: 2000, total_spending: -500, net_amount: 1500, transaction_count: 3,
      });
      expect(months[await monthKey(0)]).toMatchObject({
        total_income: 0, total_spending: -50, net_amount: -50, transaction_count: 1,
      });
      expect(r.summary).toMatchObject({
        total_income: 2000, total_spending: -550, net_amount: 1450, transaction_count: 4,
      });
      expect(r.summary.period_start).toBe(await ymd(`date_trunc('month', CURRENT_DATE) - interval '5 months'`));
    });
    ...
    it('excludes internal transfers by default and includes them when the setting is on', async () => {
      ...
      await insertTxn({ dateExpr: monthDay(1, 5), amount: '-900.00', isTransfer: true });

      const off = await getMonthlyFinancialSummary([], 'EUR', [], false);
      expect(byMonth(off)[await monthKey(1)]).toMatchObject({ total_spending: -100, transaction_count: 1 });
      ...
      expect(byMonth(on)[await monthKey(1)]).toMatchObject({ total_spending: -1000, transaction_count: 2 });
    });
    ...
    it('applies alias-aware category and recipient exclusions to real rows', async () => {
      ...
      await insertTxn({ dateExpr: monthDay(1, 5), amount: '-7.00', recipientId: rec.misc });

      const all = await getMonthlyFinancialSummary([], 'EUR', [], false);
      expect(byMonth(all)[await monthKey(1)].total_spending).toBe(-307);

      const exclBills = await getMonthlyFinancialSummary([cat.Bills], 'EUR', [], false);
      expect(byMonth(exclBills)[await monthKey(1)]).toMatchObject({ total_spending: -107, transaction_count: 2 });

      const exclPrimary = await getMonthlyFinancialSummary([], 'EUR', [rec.electrabel], false);
      expect(byMonth(exclPrimary)[await monthKey(1)]).toMatchObject({ total_spending: -107, transaction_count: 2 });
    });
    ...
    it('extends the series back to the earliest active transaction when allTime is set', async () => {
      ...
      expect(byMonth(r)[await monthKey(8)]).toMatchObject({ total_spending: -10 });
      expect(byMonth(r)[await monthKey(7)]).toMatchObject({ total_spending: 0, transaction_count: 0 });
    });
  ...
  describe('materialized-view fast path', () => {
    ...
    async function seedCorpus() {
      await seedBase();
      await insertTxn({ dateExpr: monthDay(1, 5), amount: '2000.00', recipientId: rec.misc });
      await insertTxn({ dateExpr: monthDay(1, 5), amount: '-300.00', recipientId: rec.misc, categoryId: cat.Food });
      await insertTxn({ dateExpr: monthDay(1, 15), amount: '-200.00', recipientId: rec.electrabelAlias });
      await insertTxn({ dateExpr: monthDay(3, 10), amount: '-40.00', recipientId: rec.misc });
      await insertTxn({ dateExpr: monthDay(0, 10), amount: '-50.00', recipientId: rec.misc });
      await insertTxn({ dateExpr: monthDay(0, 11), amount: '-900.00', recipientId: rec.misc, isTransfer: true });
    }
    ...
    it('really serves from the MV: a post-creation insert stays invisible until REFRESH', async () => {
      ...
      await insertTxn({ dateExpr: monthDay(0, 12), amount: '-77.00', recipientId: rec.misc });
      ...
      await getTestPool().query('REFRESH MATERIALIZED VIEW mv_monthly_summary');
      const refreshed = await getMonthlyFinancialSummary([], 'EUR', [], false);
      expect(byMonth(refreshed)[await monthKey(0)].total_spending)
        .toBe(byMonth(before)[await monthKey(0)].total_spending - 77);
    });
    ...
    it('falls back to the live path when the MV holds a non-target currency', async () => {
      ...
      await insertTxn({ dateExpr: monthDay(1, 5), amount: '-100.00', currency: 'USD' });
      await insertTxn({ dateExpr: monthDay(1, 6), amount: '-10.00', currency: 'EUR' });
    ...
    it('never takes the MV path when an exclusion or allTime is requested', async () => {
      ...
      await insertTxn({ dateExpr: monthDay(8, 10), amount: '-11.00', recipientId: rec.misc });
      ...
      const excluded = await getMonthlyFinancialSummary([cat.Food], 'EUR', [], false);
  ...
  describe('one clock: APP_TIMEZONE anchor across a month rollover', () => {
    ...
    const oldestDbWindowMonthDay5 = "date_trunc('month', now()) - interval '5 months' + interval '4 days'";
    ...
    it('live path: the 6-month set and its window both follow the app month', async () => {
      ...
      const keys = r.months.map((m) => `${m.year}-${String(m.month).padStart(2, '0')}`);
      expect(r.months).toHaveLength(6);
      ...
      const months = byMonth(r);
      expect(months[facts.app_month]).toMatchObject({ total_spending: -10, transaction_count: 1 });
      expect(months[facts.db_month]).toMatchObject({ total_spending: -40, transaction_count: 1 });
    });
    ...
    it('MV fast path: SQL filter and JS zero-fill agree on ONE 6-month set', async () => {
      ...
      const months = byMonth(r);
      expect(months[facts.db_month]).toMatchObject({ total_spending: -40, transaction_count: 1 });
      expect(months[facts.app_month]).toMatchObject({ total_spending: 0, transaction_count: 0 });
    });
  });
});
