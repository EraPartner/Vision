...
describe('timezone helpers', () => {
  ...
  it('toUtc round-trips zoned components (winter)', () => {
    const utc = toUtc({ year: 2026, month: 1, day: 31, hour: 0 }, 'Europe/Brussels');
    expect(utc.toISOString()).toBe('2026-01-30T23:00:00.000Z');
  });

  it('toUtc round-trips zoned components (summer/DST)', () => {
    const utc = toUtc({ year: 2026, month: 7, day: 1, hour: 0 }, 'Europe/Brussels');
    expect(utc.toISOString()).toBe('2026-06-30T22:00:00.000Z');
  });

  it('toUtc handles DST spring-forward boundary', () => {
    const before = toUtc({ year: 2026, month: 3, day: 29, hour: 1, minute: 30 }, 'Europe/Brussels');
    const after = toUtc({ year: 2026, month: 3, day: 29, hour: 3, minute: 30 }, 'Europe/Brussels');
