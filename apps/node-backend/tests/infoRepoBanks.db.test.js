 ...
 * (BALANCE_PROVENANCE_LATERAL) resolving against real NUMERIC/DATE columns, the
...
describe.skipIf(!hasTestDatabase())(
  "repositories/infoRepositoryBanks (real DB)",
  () => {
    beforeAll(async () => {
      expect(
    ...
    describe("current balances", () => {
      it("computes anchor+delta for stamped accounts and Σ(amount) for manual-only ones", async () => {
        ...
        await insertTxn({
          dateExpr: "CURRENT_DATE - interval '20 days'",
          amount: "-10.00",
          bank: "AAA MANUAL",
        });
        await insertTxn({
          dateExpr: "CURRENT_DATE - interval '15 days'",
          amount: "-20.00",
          bank: "AAA MANUAL",
        });
        await insertTxn({
        ...
        expect(r.accounts.map((a) => a.bank_account)).toEqual([
          "AAA MANUAL",
          "BBB STAMPED",
        ]); // ORDER BY a.name
    ...
    describe("history", () => {
      it("emits one point per day from the first activity to today, anchoring each day on the latest stamp ≤ that day", async () => {
        ...
        await insertTxn({
          dateExpr: "CURRENT_DATE - interval '4 days'",
          amount: "-50.00",
          bank: "HIST BANK",
          balance: "1000.00",
        });
      ...
      it("fills the whole 12-month window from a stamp that predates it", async () => {
        ...
        await insertTxn({
          dateExpr: "CURRENT_DATE - interval '18 months'",
          amount: "-1.00",
          bank: "OLD STAMP",
          balance: "500.00",
        });
      ...
      it("opens the window at the balance carried in from BEFORE it, stamps and unstamped rows alike", async () => {
        ...
        await insertTxn({
          dateExpr: "CURRENT_DATE - interval '15 months'",
          amount: "-100.00",
          bank: "OLD ACTIVITY",
        });
    ...
    describe("wire-date convention", () => {
      ...
      it("emits first_transaction/last_transaction as calendar-day strings, not raw pg Dates", async () => {
        ...
        expect(account.last_transaction).toBe(
          await ymdFromToday("- interval '1 day'"),
        );
        ...
        expect(
          typeof account.anchor_date === "undefined" ||
            typeof account.anchor_date === "string",
        ).toBe(true);
      });
    });
    ...
    describe("multi-currency accounts", () => {
      ...
      it("revalues the headline at TODAY rate, not at the last statement date, over a moving curve", async () => {
        ...
        expect(points.find((p) => p.date === statementDay)).toEqual({
          date: statementDay,
          balance: 500,
        });
      });
    ...
    describe("history/headline agreement", () => {
      ...
      it("includes manual-only accounts in history, so today equals total_net_position (mixed ledger)", async () => {
        ...
        expect(Object.keys(r.history).sort()).toEqual([
          "MANUAL ONLY",
          "STAMPED",
        ]);
      ...
      it("keeps a future-dated row out of the headline and chart until effective", async () => {
        await seedRecipient();
