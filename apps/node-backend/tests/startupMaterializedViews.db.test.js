 ...
 * migrations, three full aggregation scans of `transactions` sat inside the pre-`/health`
...
describe.skipIf(!hasTestDatabase())('startup: materialized views are built after listen', () => {
  /** @type {pg.Client} */
  let admin;
  /** @type {pg.Pool} */
  let scratch;
  ...
  afterAll(async () => {
    ...
    try { await locker?.query('ROLLBACK'); } catch { /* already gone */ }
  ...
  it('listens (and answers /health) while MV creation is still blocked', async () => {
    ...
    const detailed = await fetch(`${baseUrl}/health/detailed`).then((r) => r.json());
    ...
    // Release immediately: the blocked CREATE is on the app pool, whose 30s
    // statement_timeout covers the lock wait too.
  ...
  it('completes creation, indexing and refresh post-listen, then flips the health flag', async () => {
    ...
    expect(views.map((v) => v.matviewname)).toEqual([...MANAGED_VIEWS].sort());
