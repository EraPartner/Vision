...
describe("normalizeDateLikeToYmd", () => {
  it("normalizes pg DATE objects and valid ISO-prefixed strings", () => {
    process.env.TZ = "Europe/Brussels";
    expect(normalizeDateLikeToYmd(new Date(2026, 5, 1))).toBe("2026-06-01");
    expect(normalizeDateLikeToYmd("2026-06-01T12:34:56Z")).toBe("2026-06-01");
  });

  it("fails closed for nullish, malformed, invalid, and rolled dates", () => {
    expect(normalizeDateLikeToYmd(null)).toBeUndefined();
    expect(normalizeDateLikeToYmd(new Date(Number.NaN))).toBeUndefined();
    expect(normalizeDateLikeToYmd("not-a-date")).toBeUndefined();
    expect(normalizeDateLikeToYmd("2026-02-30")).toBeUndefined();
    expect(normalizeDateLikeToYmd(0)).toBeUndefined();
    expect(normalizeDateLikeToYmd(1)).toBeUndefined();
    expect(normalizeDateLikeToYmd("0")).toBeUndefined();
    expect(normalizeDateLikeToYmd("1")).toBeUndefined();
    expect(normalizeDateLikeToYmd(".5")).toBeUndefined();
    expect(normalizeDateLikeToYmd("1.")).toBeUndefined();
  });
});
