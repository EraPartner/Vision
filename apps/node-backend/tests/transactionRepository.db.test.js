...
async function seedCorpus() {
  ...
  rec.delhaizeAlias = await addRecipient("Delhaize BXL", {
  ...
  rec.employer = await addRecipient("Acme Payroll", {
  ...
  T.t1 = await insertTxn({
    ...
    memo: "CARD PAYMENT DELHAIZE",
  });
  T.t2 = await insertTxn({
    date: "2024-02-29",
    amount: "-18.75",
  ...
  T.t3 = await insertTxn({
    date: "2024-03-01",
    amount: "-120.00",
    recipientId: rec.electrabelAlias,
    memo: "ELECTRABEL INVOICE 2024",
  });
  T.t4 = await insertTxn({
    date: "2024-03-01",
    amount: "2500.00",
    recipientId: rec.employer,
    memo: "SALARY FEBRUARY",
  });
  T.t5 = await insertTxn({
    date: "2024-03-02",
    amount: "-45.10",
    currency: "USD",
    recipientId: rec.delhaize,
...
describe.skipIf(!hasTestDatabase())(
  "repositories/transactionRepository (real DB)",
  () => {
    beforeAll(async () => {
      expect(
    ...
    describe("getAll", () => {
      ...
      it("applies inclusive date bounds across the month boundary", async () => {
        const march = await transactionRepository.getAll({
          startDate: "2024-03-01",
        });
        expect(march.map((r) => r.id)).toEqual([T.t5, T.t4, T.t3]);
        const feb = await transactionRepository.getAll({
          endDate: "2024-02-29",
        });
      ...
      it("recipientGroupId resolves the same group from the primary, the alias, and a sibling", async () => {
        const fromPrimary = await transactionRepository.getAll({
          recipientGroupId: rec.delhaize,
        });
        const fromAlias = await transactionRepository.getAll({
          recipientGroupId: rec.delhaizeAlias,
        });
        ...
        expect(
          (
            await transactionRepository.getAll({
              recipientGroupId: rec.electrabel,
            })
          ).map((r) => r.id),
        ).toEqual([T.t3, T.t7]);
        expect(
          (
            await transactionRepository.getAll({
              recipientGroupId: rec.electrabelAlias,
            })
          ).map((r) => r.id),
        ).toEqual([T.t3, T.t7]);
      ...
      it("recipientGroupId excludes non-members and yields nothing for an unknown id", async () => {
        ...
        expect(
          await transactionRepository.getAll({ recipientGroupId: 9_999_999 }),
        ).toEqual([]);
      });

      it("recipientGroupId honours inactive rows and composes with other filters", async () => {
        const withInactive = await transactionRepository.getAll({
          recipientGroupId: rec.delhaize,
          active: false,
        });
      ...
      it("computes running_balance as a per-account ledger (partitioned, date ASC over the filtered set)", async () => {
        ...
        const rb = Object.fromEntries(
          rows.map((r) => [r.id, Number(r.running_balance)]),
        );
    ...
    describe("getCount", () => {
      ...
      it("counts by effective category across all three levels", async () => {
        ...
        expect(
          await transactionRepository.getCount({ categoryId: cat.Bills }),
        ).toBe(2);
        expect(
          await transactionRepository.getCount({ categoryId: cat.Food }),
        ).toBe(2);
      });
      ...
      it("counts category lists, transaction type, and signed or magnitude amount bounds", async () => {
        ...
        expect(
          await transactionRepository.getCount({ transactionType: "income" }),
        ).toBe(1);
    ...
    describe("getAllWithCount", () => {
      ...
      it("splits income from expenses by amount sign", async () => {
        ...
        const expense = await transactionRepository.getAllWithCount({
      ...
      it("amount bounds compare magnitude by default and the signed amount when amountSigned", async () => {
        const magnitude = await transactionRepository.getAllWithCount({
          amountMin: 50,
        });
        expect(magnitude.rows.map((r) => r.id).sort((a, b) => a - b)).toEqual(
          [T.t1, T.t3, T.t4].sort((a, b) => a - b), // |−52.30|, |−120|, |2500|
        );
        const signed = await transactionRepository.getAllWithCount({
          amountMax: -100,
          amountSigned: true,
        });
      ...
      it("tagSlugs matches rows via ACTIVE tags only", async () => {
        const groceries = await transactionRepository.getAllWithCount({
          tagSlugs: ["groceries"],
        });
        ...
        const archived = await transactionRepository.getAllWithCount({
          tagSlugs: ["archived"],
        });
      ...
      it("free-text search spans memo, recipient (incl. primary), category and amount text", async () => {
        const byName = await transactionRepository.getAllWithCount({
          search: "delhaize",
        });
        expect(byName.rows.map((r) => r.id)).toEqual([T.t5, T.t2, T.t1]); // t6 inactive stays out
        const byCategory = await transactionRepository.getAllWithCount({
          search: "utilities",
        });
        expect(byCategory.rows.map((r) => r.id)).toEqual([T.t3, T.t7]);
        const byAmount = await transactionRepository.getAllWithCount({
          search: "2500",
        });
    ...
    describe("getUncategorised / getUncategorisedWithCount", () => {
      ...
      it("supports recipientName / bankAccount narrowing", async () => {
        const hit = await transactionRepository.getUncategorised({
          recipientName: "delh",
        });
        expect(hit.map((r) => r.id)).toEqual([T.t1]);
        const miss = await transactionRepository.getUncategorised({
          bankAccount: "wise",
        });
        expect(miss).toEqual([]);
      });
      ...
      describe("the route-supplied filters narrow rows AND total", () => {
        ...
        beforeEach(async () => {
          ...
          U.small = await insertTxn({
            date: "2024-03-05",
            amount: "-5.00",
            recipientId: bakery,
            memo: "BAKERY SMALL",
          });
          U.big = await insertTxn({
            date: "2024-03-06",
            amount: "-300.00",
            recipientId: bakery,
            memo: "BAKERY BIG",
          });
          U.income = await insertTxn({
            date: "2024-03-07",
            amount: "900.00",
            recipientId: bakery,
            memo: "BAKERY REFUND",
        ...
        it("search and transactionId narrow both the uncategorised rows and total", async () => {
          ...
          const exact = await transactionRepository.getUncategorisedWithCount({
            transactionId: U.small,
          });
        ...
        it("amountMin+amountMax bracket a range, and amountSigned flips to the signed amount", async () => {
          const bracket = await transactionRepository.getUncategorisedWithCount(
        ...
        it("transactionType narrows both halves", async () => {
          const income = await transactionRepository.getUncategorisedWithCount({
            transactionType: "income",
          });
        ...
        it("recipientGroupId narrows both halves and resolves the whole primary group", async () => {
          ...
          expect(outside.rows.map((r) => r.id)).toEqual([
            U.income,
            U.big,
            U.small,
          ]);
        ...
        it("plain and counted uncategorised queries share the same filter semantics", async () => {
          ...
          const counted =
            await transactionRepository.getUncategorisedWithCount(filters);
          expect(plain.map((row) => row.id)).toEqual([U.big]);
          expect(plain.map((row) => row.id)).toEqual(
            counted.rows.map((row) => row.id),
          );
        });
        ...
        it("honours custom sorting and includeBalance on the uncategorised path", async () => {
          ...
          expect(rows.map((row) => Number(row.running_balance))).toEqual([
            -357.3, -52.3, -57.3, 542.7,
          ]);
        });
      });
    ...
    describe("update", () => {
      ...
      it("returns null for a missing id, with fields and tags-only alike", async () => {
        expect(
          await transactionRepository.update(T.t7 + 100_000, {
            amount: "1.00",
          }),
        ).toBeNull();
        expect(
        ...
        const { rows } = await getTestPool().query(
          "SELECT 1 FROM transaction_tags WHERE transaction_id = $1",
          [T.t7 + 100_000],
        );
        expect(rows).toEqual([]);
      });
      ...
      it("update() response resolves the full 3-level category — identical to getById on alias rows", async () => {
        const updated = await transactionRepository.update(T.t3, {
          amount: "-121.00",
        });
        ...
        const withTags = await transactionRepository.update(T.t3, {
          amount: "-122.00",
          tags: [],
        });
    ...
    describe("alias with its own default under a differently-defaulted primary", () => {
      ...
      it("getById / getAll / getAllWithCount label the row with the category the id names", async () => {
        const expected = {
          effective_category_id: cat.Zzz,
          category_name: "Zzz:Last",
        };

        expect(await transactionRepository.getById(aliasTxn)).toMatchObject(
          expected,
        );
        ...
        expect(paged.rows.find((r) => r.id === aliasTxn)).toMatchObject(
          expected,
        );
      });
      ...
      it("create / update return the same pairing as an immediate GET", async () => {
        ...
        expect(await transactionRepository.getById(created.id)).toEqual(
          created,
        );

        const updated = await transactionRepository.update(aliasTxn, {
          amount: "-33.33",
        });
        ...
        // The tags-path fetch (fields + tags in one PATCH) resolves identically.
        const withTags = await transactionRepository.update(aliasTxn, {
          amount: "-34.00",
          tags: [],
        });
      ...
      it("an own category_id still wins over both defaults", async () => {
        const row = await transactionRepository.update(aliasTxn, {
