...
function mockSnapshotQueries({
  ...
  query.mockImplementation(async (sql) => {
    ...
    if (
      sql.includes("FROM portfolio_performance_snapshots") &&
      sql.includes("snapshot_date >=")
    ) {
      return {
        rows: [
          {
...
describe("portfolioPerformanceSnapshotService", () => {
  ...
  it("computes snapshots, deletes existing currency rows, and stores batches", async () => {
    ...
    const deleteCall = query.mock.calls.find(
      ([sql]) =>
        typeof sql === "string" &&
        sql.includes("DELETE FROM portfolio_performance_snapshots"),
    );
    ...
    const insertCall = query.mock.calls.find(
      ([sql]) =>
        typeof sql === "string" &&
        sql.includes("INSERT INTO portfolio_performance_snapshots"),
    );
  ...
  it("applies stock splits to historical units so value tracks the live summary", async () => {
    ...
    mockSnapshotQueries({
      investments: [
        { id: 1, currency: "EUR", current_price: 5, asset_class: "stock" },
      ],
      transactions: [
        {
          investment_id: 1,
  ...
  it("orders same-day buys before sells so an oversell cannot mint phantom units", async () => {
    // Regression: an earlier-day buy was deleted, leaving a same-day sell that
    // (by raw id order) precedes its own-day buy. Replaying the sell first left
    // the buy's units un-clamped → phantom units. The builder must replay
    // buys/gifts/splits before sells within a day (both in SQL ORDER BY and a
    // defensive JS sort). Buy 5 then sell 8 → oversell clamps to 0 units.
    mockSnapshotQueries({
      investments: [
        { id: 1, currency: "EUR", current_price: 10, asset_class: "stock" },
      ],
      transactions: [
        // Deliberately sell-before-buy in the raw row order.
        {
          investment_id: 1,
          day: "2026-01-01",
          type: "sell",
    ...
    const txCall = query.mock.calls.find(
      ([sql]) =>
        String(sql).includes("FROM portfolio_transactions pt") &&
  ...
  it("converts a foreign-currency holding with no stored fx rate at each day's historical rate", async () => {
    ...
    mockSnapshotQueries({
      investments: [
        { id: 1, currency: "USD", current_price: 10, asset_class: "stock" },
      ],
      transactions: [
        {
          investment_id: 1,
  ...
  it("computes the FX-neutral series locked at the cost-weighted purchase rate", async () => {
    ...
    const insertCall = query.mock.calls.find(([sql]) =>
      String(sql).includes("INSERT INTO portfolio_performance_snapshots"),
    );
  ...
  it("falls back from historical price to last known transaction price and current price", async () => {
    ...
    mockSnapshotQueries({
      ...
      prices: [{ investment_id: 1, day: "2026-01-01", close_price: 12 }],
    });
  ...
  it("sanitizes isolated one-day spikes in computed values", async () => {
    ...
    mockSnapshotQueries({
      investments: [
        { id: 1, currency: "EUR", current_price: 101, asset_class: "stock" },
      ],
      transactions: [
        {
          investment_id: 1,
