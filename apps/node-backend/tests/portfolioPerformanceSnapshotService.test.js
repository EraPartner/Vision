...
function mockSnapshotQueries({
  ...
  query.mockImplementation(async (sql) => {
    ...
    if (sql.includes('FROM portfolio_transactions pt') && sql.includes('ORDER BY pt.date::date')) {
  ...
  it('orders same-day buys before sells so an oversell cannot mint phantom units', async () => {
    ...
    mockSnapshotQueries({
      ...
      transactions: [
        ...
        { investment_id: 1, day: '2026-01-01', type: 'sell', amount: 50, units: 8, currency: 'EUR', fx_rate_to_eur: null },
        { investment_id: 1, day: '2026-01-01', type: 'buy', amount: 50, units: 5, currency: 'EUR', fx_rate_to_eur: null },
      ],
      prices: [
        { investment_id: 1, day: '2026-01-01', close_price: 10 },
      ],
    ...
    const txCall = query.mock.calls.find(
      ([sql]) => String(sql).includes('FROM portfolio_transactions pt') && String(sql).includes('ORDER BY'),
    );
