...
async function seedBase() {
  ...
  rec.aldiAlias = await addRecipient("Aldi Anderlecht", {
    primaryId: rec.aldi,
  });
...
describe.skipIf(!hasTestDatabase())(
  "repositories/infoRepositoryStatistics (real DB)",
  () => {
    beforeAll(async () => {
      expect(
    ...
    describe("getCategoryBreakdown", () => {
      it("aggregates per category across currencies, excluding transfers and inactive rows", async () => {
        ...
        await insertTxn({
          date: "2024-02-10",
          amount: "-10.00",
          recipientId: rec.aldi,
        }); // Food via recipient default
        await insertTxn({
          date: "2024-02-15",
          amount: "-20.50",
      ...
      it("rolls direct and mixed-depth assignments into each requested ancestor exactly once", async () => {
        await seedBase();
        const pool = getTestPool();
        ...
        const rootId = root.rows[0].parent_id;
        const legacy = await pool.query(
          "SELECT id FROM categories WHERE general = 'Food' AND detail = 'Groceries'",
        );
        expect(legacy.rows[0].id).toBe(cat.Food);
        const depth3 = await pool.query(
          `INSERT INTO categories (general, detail, name, parent_id, legacy_compatible)
         VALUES ('__HIERARCHY_TEST_3', 'Fruit', 'Fruit', $1, false) RETURNING id`,
          [cat.Food],
        );
        const depth4 = await pool.query(
          `INSERT INTO categories (general, detail, name, parent_id, legacy_compatible)
         VALUES ('__HIERARCHY_TEST_4', 'Apples', 'Apples', $1, false) RETURNING id`,
          [depth3.rows[0].id],
        );
        await insertTxn({
          date: "2024-03-01",
        ...
        await insertTxn({
          ...
          amount: "-20.00",
          recipientId: rec.misc,
          categoryId: depth3.rows[0].id,
        });
        await insertTxn({
          date: "2024-03-03",
          amount: "-30.00",
          recipientId: rec.misc,
          categoryId: depth4.rows[0].id,
        });
        await insertTxn({
          date: "2024-03-04",
          amount: "-40.00",
        ...
        await expect(
          pool.query("UPDATE categories SET parent_id = $1 WHERE id = $2", [
            depth4.rows[0].id,
            depth3.rows[0].id,
          ]),
        ).rejects.toThrow("Category hierarchy cycle is not allowed");

        const leafRollup = await statisticsRepository.getCategoryBreakdown(
          "EUR",
          cat.Food,
        );
        const rootRollup = await statisticsRepository.getCategoryBreakdown(
          "EUR",
          rootId,
        );
        const otherBranch = await statisticsRepository.getCategoryBreakdown(
          "EUR",
          cat.Bills,
        );
        expect(leafRollup).toEqual([
          { id: cat.Food, name: "Food:Groceries", count: 3, total: -60 },
        ]);
        expect(rootRollup).toEqual([
          { id: rootId, name: "Food", count: 4, total: -65 },
        ]);
        expect(otherBranch).toEqual([
          { id: cat.Bills, name: "Bills:Utilities", count: 1, total: -40 },
        ]);
        ...
        expect(rows.find((row) => row.categoryId === rootId)).toMatchObject({
          categoryPathIds: [rootId],
          categoryPathSegments: ["Food"],
          total: -5,
        });
        expect(rows.find((row) => row.categoryId === cat.Food)).toMatchObject({
          categoryPathIds: [rootId, cat.Food],
          categoryPathSegments: ["Food", "Groceries"],
          total: -10,
        });
        expect(
          rows.find((row) => row.categoryId === depth3.rows[0].id),
        ).toMatchObject({
          categoryPathIds: [rootId, cat.Food, depth3.rows[0].id],
          categoryPathSegments: ["Food", "Groceries", "Fruit"],
          total: -20,
        });
        expect(
          rows.find((row) => row.categoryId === depth4.rows[0].id),
        ).toMatchObject({
          categoryPathIds: [
            rootId,
            cat.Food,
          ...
          categoryPathSegments: ["Food", "Groceries", "Fruit", "Apples"],
          total: -30,
        });

        const exclusion = buildExclusionClauses({
          excludedCategoryIds: [rootId],
        });
        const remaining = await pool.query(
          `SELECT COUNT(*)::int AS count FROM transactions t
           ${exclusion.joinSql}
           WHERE t.is_active=true AND ${exclusion.whereSql}`,
          exclusion.params,
        );
        expect(remaining.rows[0].count).toBe(1); // Bills, no Food descendant
      });
    });
    ...
    describe("getBanks", () => {
      it("lists only accounts with ACTIVE transactions, ordered by name", async () => {
        ...
        expect(await statisticsRepository.getBanks()).toEqual([
          "AAA BANK",
          "ZZZ BANK",
        ]);
      });
    ...
    describe("getTransactionCount", () => {
      it("counts active rows, optionally scoped to one account", async () => {
        ...
        expect(
          await statisticsRepository.getTransactionCount({
            accountId: rows[0].id,
          }),
        ).toBe(2);
      });
    ...
    describe("getCategoryPivot", () => {
      it("splits income/expense within a mixed-sign category-month", async () => {
        ...
        expect(r.categoryPivot["2024-02"]).toMatchObject([
          {
            categoryId: cat.Food,
      ...
      it("converts each month at its own historical rate (per-date FX from exchange_rates)", async () => {
        ...
        expect(r.categoryPivot["2024-03"]).toMatchObject([
          {
            categoryId: cat.Food,
            categoryName: "Food: Groceries",
            total: -12,
      ...
      it("alias row categorised only via its primary: Bills in breakdown and pivot, agreeing with the transactions list", async () => {
        ...
        const txnId = await insertTxn({
          date: "2024-02-10",
          amount: "-120.00",
          recipientId: aliasRes.rows[0].id,
        });

        // The transactions list categorises the row via the primary's default…
        const listed = (await transactionRepository.getAll({})).find(
          (t) => t.id === txnId,
        );
        ...
        const pivot = await statisticsRepository.getCategoryPivot();
        expect(pivot.categoryPivot).toMatchObject({
          "2024-02": [
            {
              categoryId: cat.Bills,
