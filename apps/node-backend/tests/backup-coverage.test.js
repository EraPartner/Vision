...
import { BACKUP_COVERED_TABLES, BACKUP_EXCLUDED_TABLES } from '../src/backup/coverage.js';
...
function deriveMigrationTableSet() {
  ...
  for (const file of files) {
    ...
    const upgradeEnd = downgradeIdx > upgradeStart ? downgradeIdx : content.length;
...
describe('BACKUP_COVERED_TABLES registry', () => {
  ...
  it('matches the net table set derived from alembic migrations', () => {
    ...
    const missingFromRegistry = [...migrationTables].filter(
      (t) => !BACKUP_COVERED_TABLES.includes(t) && !(t in BACKUP_EXCLUDED_TABLES),
    );
...
describe('localStorage keys registry', () => {
  it('LOCAL_STORAGE_KEYS can be imported and is non-empty', async () => {
    ...
    const keysPath = join(REPO_ROOT, 'apps/frontend/src/lib/localStorage-keys.ts');
    ...
    for (const key of requiredKeys) {
      expect(content, `localStorage key "${key}" missing from localStorage-keys.ts`).toContain(key);
    }
  });
  ...
  it('every localStorage.setItem/getItem call in frontend uses a registered key', () => {
    ...
    const keysContent = readFileSync(join(frontendSrc, 'lib/localStorage-keys.ts'), 'utf8');
    ...
    const excludedContent = keysContent.match(/LOCAL_STORAGE_EXCLUDED_KEYS[\s\S]*?\]/)?.[0] ?? '';
