...
vi.mock("../src/services/aiChat/tools/index.js", () => ({
  ...
  getToolSchemas: vi.fn().mockReturnValue([
    {
      type: "function",
      function: { name: "getSpendByCategory", parameters: {} },
    },
  ]),
...
beforeEach(() => {
  ...
  aiChatRepository.createConversation.mockImplementation(
    async ({ title, model }) => makeConversation({ title, model }),
  );
...
describe("runChatTurn — conversation lifecycle", () => {
  it("regenerates an interrupted turn without duplicating its user message", async () => {
    const priorUser = makeMessage({
      id: "user-1",
      role: "user",
      content: "original question",
    });
    const danglingTool = makeMessage({
      id: "tool-1",
      role: "tool",
      content: "",
      tool_name: "oldTool",
    });
    aiChatRepository.getMessages.mockResolvedValueOnce([
      priorUser,
      danglingTool,
    ]);
    const ollamaClient = makeMockOllama();
    ...
    const result = await runChatTurn({
      ...
      message: "client copy is not authoritative",
      retryLastTurn: true,
      ollamaClient,
      onEvent: (event) => events.push(event),
    });

    const persistedRoles = aiChatRepository.appendMessage.mock.calls.map(
      ([input]) => input.role,
    );
  ...
  it("refuses to regenerate a turn that already has an assistant response", async () => {
    aiChatRepository.getMessages.mockResolvedValueOnce([
      makeMessage({ id: "user-1", role: "user", content: "complete question" }),
      makeMessage({
        id: "assistant-1",
        role: "assistant",
    ...
    await expect(
      runChatTurn({
        conversationId: "conv-1",
        message: "complete question",
        retryLastTurn: true,
        ollamaClient: makeMockOllama(),
      }),
    ).rejects.toMatchObject({ code: "TURN_ALREADY_COMPLETE", status: 409 });
    expect(aiChatRepository.appendMessage).not.toHaveBeenCalled();
  });

  it("serializes a retry behind an in-flight turn and prevents two answers", async () => {
    const persisted = [];
    aiChatRepository.getMessages.mockImplementation(async () => [...persisted]);
    aiChatRepository.appendMessage.mockImplementation(async (input) => {
    ...
    let resolveOriginal;
    const ollamaClient = makeMockOllama();
    ollamaClient.chat.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
    ...
    expect(aiChatRepository.getMessages).toHaveBeenCalledTimes(1);
    ...
    await expect(original).resolves.toMatchObject({
      assistantMessage: { content: "only answer" },
    });
  ...
  it("cancels a queued turn without persisting or calling the model", async () => {
    ...
    await vi.waitFor(() => {
      expect(persisted.map((row) => row.content)).toEqual(["first"]);
    });

    const controller = new AbortController();
    const queued = runChatTurn({
      conversationId: "conv-1",
    ...
    await original;
    expect(persisted.map((row) => row.content)).toEqual([
      "first",
      "first answer",
    ]);
    expect(ollamaClient.chat).toHaveBeenCalledTimes(1);
  ...
  it("updates conversation model when caller passes a different model", async () => {
    ...
    expect(aiChatRepository.updateConversationModel).toHaveBeenCalledWith(
      "c-1",
      "qwen2.5:7b",
    );
  ...
  it("bad-JSON tool args: persists the raw string next to the error result, streams an object args frame, and feeds the model the exact error payload", async () => {
    ...
    const validationResult = {
      ...
      error: {
        ...
        message: "arguments is not valid JSON: …",
      },
    };
    const ollamaClient = makeMockOllama();
    ollamaClient.chat
...
describe("runChatTurn — server-side pre-call (ADR-110 §4)", () => {
  ...
  it("dispatches the pre-call tool, persists/emits it before the assistant turn, injects it into the model context, and returns the narration", async () => {
    ...
    expect(result.assistantMessage.content).toBe(
      "One new subscription: Netflix at €12.99.",
    );
    expect(result.iterations).toBe(1);
  });
...
describe("runChatTurn — iteration cap", () => {
  it("returns fallback error assistant message when cap reached", async () => {
    ...
    expect(ollamaClient.chat).toHaveBeenCalledTimes(
      __constants.MAX_TOOL_ITERATIONS,
    );
...
describe("runChatTurn — Ollama error mapping", () => {
  it("maps OllamaError (non-abort) to AiChatServiceError 502", async () => {
    ...
    await expect(
      runChatTurn({ message: "x", ollamaClient }),
    ).rejects.toMatchObject({
      name: "AiChatServiceError",
      code: "HTTP_ERROR",
      status: 502,
  ...
  it("maps a coded abort from another injected provider to 499", async () => {
    const ollamaClient = makeMockOllama();
    const providerError = Object.assign(new Error("aborted"), {
      code: "ABORTED",
    });
    ollamaClient.chat.mockRejectedValueOnce(providerError);
...
describe("AiChatServiceError", () => {
  ...
  it("attaches cause when provided", () => {
    ...
    const err = new AiChatServiceError("outer", {
      code: "X",
      status: 502,
      cause,
    });
...
describe("runChatTurn — streaming", () => {
  ...
  it("emits tool_call before dispatch (progress affordance) and tool_message after", async () => {
    ...
    const result = await runChatTurn({
      ...
      onEvent: (e) => {
        if (e.type === "tool_call")
          dispatchCountAtToolCall = dispatchTool.mock.calls.length;
        events.push(e);
      },
    });
  ...
  it("does not call fetch on the insights pre-call path (ADR-110 §4)", async () => {
    const ollamaClient = {
      chat: vi.fn().mockResolvedValue({
        message: {
          role: "assistant",
          content: "Here is what stands out this month.",
        },
        tool_calls: [],
      }),
    };
