...
describe("createInvestment — numeric field guards", () => {
  it("rejects a non-numeric current_price with a 400, not a pg cast error", async () => {
    await expect(
      createInvestment(createReq({ current_price: "banana" }), mockRes()),
  ...
  it("rejects negative cadastral_income", async () => {
    await expect(
      createInvestment(createReq({ cadastral_income: -500 }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it("rejects a 5000% municipality_tax_rate", async () => {
    await expect(
      createInvestment(createReq({ municipality_tax_rate: 5000 }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it('rejects absurd magnitudes and JSON "Infinity"', async () => {
    await expect(
      createInvestment(createReq({ current_price: "1e15" }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
    await expect(
      createInvestment(createReq({ interest_rate: "Infinity" }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
  });
  ...
  it("coerces numeric strings and accepts negative interest rates", async () => {
    ...
    await createInvestment(
      createReq({ current_price: "12.5", interest_rate: "-0.5" }),
      res,
    );
...
describe("updateInvestment — numeric field guards", () => {
  ...
  it("applies the same guards as create", async () => {
    await expect(
      updateInvestment(req({ municipality_tax_rate: "high" }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
    await expect(
      updateInvestment(req({ current_price: -1 }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
  ...
  it("passes valid coerced values and explicit nulls through", async () => {
    ...
    await updateInvestment(
      req({ current_price: "99.99", interest_rate: null }),
      res,
    );
...
describe("createInvestment — numeric boundary pins", () => {
  ...
  it("rejects values just past the bounds", async () => {
    await expect(
      createInvestment(createReq({ current_price: 1e12 + 1 }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
    await expect(
      createInvestment(createReq({ interest_rate: 100.01 }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
    await expect(
      createInvestment(createReq({ interest_rate: -100.01 }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
    await expect(
      createInvestment(createReq({ municipality_tax_rate: -0.01 }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
    await expect(
      createInvestment(createReq({ cadastral_income: -0.01 }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
    expect(investmentRepository.create).not.toHaveBeenCalled();
  });
});
...
describe("createInvestment — string width and currency pins", () => {
  it("accepts strings exactly at the column width and rejects one char over", async () => {
    const widths = [
      ...
      ["price_provider_url", 500],
      ["price_provider_latest_url", 500],
      ["price_provider_latest_path", 300],
      ["price_provider_history_url", 500],
      ["price_provider_history_path", 300],
      ["price_provider_history_ts_path", 300],
      ["price_provider_history_price_path", 300],
    ];
    for (const [field, max] of widths) {
      await createInvestment(
        createReq({ [field]: "x".repeat(max) }),
        mockRes(),
      );
      await expect(
        createInvestment(
          createReq({ [field]: "x".repeat(max + 1) }),
          mockRes(),
        ),
      ).rejects.toBeInstanceOf(ValidationError);
    }
  ...
  it("uppercases a valid currency and rejects non-ISO shapes", async () => {
    ...
    await expect(
      createInvestment(createReq({ currency: "euro" }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
    await expect(
      createInvestment(createReq({ currency: "€" }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it("rejects explicit empty/null currency and lets an absent key use the repository default", async () => {
    await expect(
      createInvestment(createReq({ currency: "" }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
    await expect(
...
describe("updateInvestment — string field pins", () => {
  ...
  it("rejects clearing the NOT NULL currency on update", async () => {
    await expect(
      updateInvestment(req({ currency: "" }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
    await expect(
      updateInvestment(req({ currency: null }), mockRes()),
    ).rejects.toBeInstanceOf(ValidationError);
    expect(investmentRepository.update).not.toHaveBeenCalled();
  });
});
