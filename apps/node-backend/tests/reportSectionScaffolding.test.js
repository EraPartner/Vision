...
import { normalizeBreakdownRow } from '../src/services/reports/dataFetcherPortfolio.js';
...
const EMPTY_RENDERER_NAMES = [
  'executiveSummary', 'cashflowTrend', 'categoryBreakdown', 'topRecipients', 'bankBalances',
  'rollingAverages', 'plannedOutlook', 'portfolioExecutiveSummary', 'portfolioAllocation',
  'topHoldings', 'performanceTrend', 'assetClassDetail', 'dividendIncome', 'taxExecutiveSummary',
  'taxTypeBreakdown', 'taxByAssetClass', 'taxMonthlyTrend', 'topInvestmentsByCost', 'feeBreakdown',
  'belgianRulesSummary',
];
const EMPTY_PAGE_BREAKS = new Set([
  'cashflowTrend', 'categoryBreakdown', 'topRecipients', 'bankBalances', 'rollingAverages', 'plannedOutlook',
]);

const month = { year: 2026, month: 1, total_income: 100, total_spending: -40, net_amount: 60, transaction_count: 2 };
const breakdown = [{
  id: 1,
  name: 'Index Fund',
  symbol: 'IDX',
...
const taxData = {
  ...
  byMonth: [{ year: 2026, month: 1, tob: 3, wht: 2, sell: 1, other: 0.5, fees: 4 }],
  byAssetClass: [{ assetClass: 'etf', taxes: 6.5, fees: 4 }],
  byInvestment: [{ investmentId: 1, name: 'Index Fund', symbol: 'IDX', tob: 3, wht: 2, sell: 1, other: 0.5, fees: 4, total: 10.5 }],
  taxTables: { tob: {}, dividendExemption: 859, dividendWHTRate: 0.3 },
};

const POPULATED_RENDERERS = [
  { name: 'executiveSummary', render: () => renderExecutiveSummary({ monthly: { months: [month] } }, { currency, period }), pageBreak: false },
  { name: 'cashflowTrend', render: () => renderCashflowTrend({ monthly: { months: [month] } }, { currency, period }), pageBreak: true },
  { name: 'categoryBreakdown', render: () => renderCategoryBreakdown({ categories: { categories: [{ id: 1, name: 'Food', total: -40, count: 2 }] }, exclusions: { categoryIds: [] } }, { currency, period }), pageBreak: true, continuation: true },
  { name: 'topRecipients', render: () => renderTopRecipients({ recipients: { topMerchants: [{ recipientId: 1, name: 'Shop', totalSpend: 40, transactionCount: 2, avgAmount: 20, lastSeen: '2026-01-31' }], monthOverMonth: [] }, exclusions: { recipientIds: [] } }, { currency }), pageBreak: true, continuation: true },
  { name: 'bankBalances', render: () => renderBankBalances({ banks: { accounts: [{ bank_account: 'Checking', balance: 60, transaction_count: 2, last_transaction: '2026-01-31' }], total_net_position: 60 } }, { currency }), pageBreak: true },
  { name: 'rollingAverages', render: () => renderRollingAverages({ averages: { past_6_months: { avg_daily_spending: 2, avg_monthly_spending: 40, months_counted: 6 }, current_month: { total_spending: 20, days_elapsed: 15, days_in_month: 31 }, comparison: { pace: 1, variance: 0, projected_monthly_total: 40, avg_monthly_spending: 40 } } }, { currency }), pageBreak: true },
  { name: 'plannedOutlook', render: () => renderPlannedOutlook({ planned: { summary: { total_income: 100, total_expenses: -40, net_amount: 60, transaction_count: 1 }, daily_data: [{ date: '2026-02-01', transactions: [{ recipient_name: 'Landlord', category_name: 'Housing:Rent', amount: -40, is_recurring: true }] }], period_start: '2026-02-01', period_end: '2026-02-28' } }, { currency }), pageBreak: true },
  { name: 'portfolioExecutiveSummary', render: () => renderPortfolioExecutiveSummary({ breakdown, snapshots: [], dividends: null }, { currency, period }), pageBreak: false },
  { name: 'portfolioAllocation', render: () => renderPortfolioAllocation({ breakdown, snapshots: [] }, { currency }), pageBreak: false },
  { name: 'topHoldings', render: () => renderTopHoldings({ breakdown }, { currency }), pageBreak: false },
  { name: 'performanceTrend', render: () => renderPerformanceTrend({ snapshots: [snapshot] }, { currency }), pageBreak: false },
  { name: 'assetClassDetail', render: () => renderAssetClassDetail({ breakdown, snapshots: [] }, { currency }), pageBreak: false },
  { name: 'dividendIncome', render: () => renderDividendIncome({ dividends: { byMonth: [{ year: 2026, month: 1, amount: 10 }], byInvestment: [{ name: 'Index Fund', symbol: 'IDX', assetClass: 'etf', total: 10 }] } }, { currency }), pageBreak: false },
  { name: 'taxExecutiveSummary', render: () => renderTaxExecutiveSummary(taxData, { currency }), pageBreak: false },
  { name: 'taxTypeBreakdown', render: () => renderTaxTypeBreakdown(taxData, { currency }), pageBreak: false },
  { name: 'taxByAssetClass', render: () => renderTaxByAssetClass(taxData, { currency }), pageBreak: false },
  { name: 'taxMonthlyTrend', render: () => renderTaxMonthlyTrend(taxData, { currency }), pageBreak: false },
  { name: 'topInvestmentsByCost', render: () => renderTopInvestmentsByCost(taxData, { currency }), pageBreak: false },
  { name: 'feeBreakdown', render: () => renderFeeBreakdown(taxData, { currency }), pageBreak: false },
  { name: 'belgianRulesSummary', render: () => renderBelgianRulesSummary(taxData, { currency }), pageBreak: false },
];
...
describe('report section scaffolding', () => {
  ...
  it('keeps the established plain and headed empty-state styles', () => {
    expect(emptySection({ title: 'Plain', subtitle: 'Sub', message: '<none>' }))
      .toContain('<div class="empty-notice">&lt;none&gt;</div>');
    expect(emptySection({ title: 'Headed', subtitle: 'Sub', heading: '<None>', message: 'Try & retry' }))
      .toContain('<div class="placeholder-notice"><strong>&lt;None&gt;</strong>Try &amp; retry</div>');
  });
  ...
  it('gives every empty renderer exactly one canonical title, subtitle, and divider', () => {
    ...
    for (const [index, render] of EMPTY_RENDERERS.entries()) {
      ...
      expect(html.includes('class="page page-break"'), name).toBe(EMPTY_PAGE_BREAKS.has(name));
    }
  });
  ...
  it('pins populated shell, page-break, and continuation behavior for all 20 renderers', () => {
    ...
    for (const { name, render, pageBreak, continuation = false } of POPULATED_RENDERERS) {
      const html = render();
      expect(html.match(/class="section-title"/g), name).toHaveLength(1);
      expect(html.match(/class="section-subtitle"/g), name).toHaveLength(1);
      expect(html.match(/class="section-divider"/g), name).toHaveLength(1);
      expect(html.includes('class="page page-break"'), name).toBe(pageBreak);
      expect(html.includes('class="page-continuation"'), name).toBe(continuation);
  ...
  it('pins filtered/all chart order, escaping, trusted charts, and notice grammar', () => {
    ...
    expect(filterNotice({ filteredCount: 1, excludedCount: 2, singular: 'category', plural: 'categories' }))
      .toBe('<div class="filter-notice">Table shows 1 category matching active filters. 2 categories excluded — see "All data" chart above.</div>');
    expect(filterNotice({ filteredCount: 2, excludedCount: 1, singular: 'recipient', plural: 'recipients' }))
  ...
  it('keeps raw page scaffolding out of every section renderer', async () => {
    const sectionDirectory = fileURLToPath(new URL('../src/services/reports/sections/', import.meta.url));
    const files = (await readdir(sectionDirectory)).filter((file) => file.endsWith('.js'));

    expect(files).toHaveLength(20);
    for (const file of files) {
      const source = await readFile(new URL(`../src/services/reports/sections/${file}`, import.meta.url), 'utf8');
      expect(source.match(/<div class="page(?: page-break)?">/), file).toBeNull();
...
describe('portfolio breakdown normalization', () => {
  ...
  it('is wired into the portfolio fetch boundary', async () => {
    const source = await readFile(new URL('../src/services/reports/dataFetcherPortfolio.js', import.meta.url), 'utf8');
    expect(source).toContain('breakdown: breakdown?.map(normalizeBreakdownRow) ?? breakdown');
  });
});
