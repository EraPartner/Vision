...
describe("literal CSV records", () => {
  it("preserves quoting, escaped quotes and embedded newlines while removing only CRLF", () => {
    const records = parseCsvText('A;B\r\n1;"hello; ""friend""\nnext"\r\n', {
      columns: true,
      delimiter: ";",
...
describe("splitDelimitedRecord", () => {
  ...
  it("unescapes doubled quotes and drops the wrapping quotes", () => {
    expect(splitDelimitedRecord('x;"he said ""hi""";y')).toEqual([
      "x",
      'he said "hi"',
      "y",
    ]);
...
describe("parseDateWithFormat round-trip validation", () => {
  it("parses a valid date for the chosen format", () => {
    expect(parseDateWithFormat("25/12/2024", "%d/%m/%Y")?.toISOString()).toBe(
  ...
  it("validates the %m/%d/%Y and ISO branches too", () => {
    expect(parseDateWithFormat("12/25/2024", "%m/%d/%Y")?.toISOString()).toBe(
      "2024-12-25T00:00:00.000Z",
    );
    expect(parseDateWithFormat("25/12/2024", "%m/%d/%Y")).toBeNull();
    expect(parseDateWithFormat("2024-12-25", "%Y-%m-%d")?.toISOString()).toBe(
      "2024-12-25T00:00:00.000Z",
    );
...
describe("readTextWithEncodingFallback", () => {
  ...
  const writeBytes = (buffer) => {
    const p = path.join(
      os.tmpdir(),
      `test_encfallback_${Date.now()}_${files.length}.csv`,
    );
