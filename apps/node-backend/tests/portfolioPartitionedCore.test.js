...
const stock = (current_price) => ({ asset_class: 'stock', current_price, interest_rate: 0 });
const n = (d) => toNumber(toDecimal(d));

const buy = (accountId, units, amount, date, extra = {}) =>
  ({ type: 'buy', units, amount, fees: 0, taxes: 0, date, account_id: accountId, ...extra });
const sell = (accountId, units, amount, date, extra = {}) =>
  ({ type: 'sell', units, amount, fees: 0, taxes: 0, date, account_id: accountId, ...extra });
...
describe('areLotsFullyAssigned (transition predicate)', () => {
  it('is false when any buy/gift/sell lacks an account, regardless of other rows', () => {
    ...
    expect(areLotsFullyAssigned([buy(null, 10, 100, '2026-01-01')])).toBe(false);
    expect(areLotsFullyAssigned([
      buy(1, 10, 100, '2026-01-01'),
      sell(null, 5, 60, '2026-02-01'), // unassigned SELL blocks too
    ])).toBe(false);
    expect(areLotsFullyAssigned([
      buy(1, 10, 100, '2026-01-01'),
      { type: 'gift', units: 2, amount: 0, date: '2026-02-01', account_id: null },
    ])).toBe(false);
...
describe('partitionTxnsByAccount — corporate actions apply across partitions', () => {
  it('rewrites a split into per-partition absolute totals that sum exactly to the global total', () => {
    ...
    const partitions = partitionTxnsByAccount([
      ...
      { type: 'split', units: 80, amount: 0, fees: 0, taxes: 0, date: '2026-02-01', account_id: null },
    ]);
  ...
  it('allocates return_of_capital proportional to units held at that date', () => {
    ...
    const partitions = partitionTxnsByAccount([
      ...
      { type: 'return_of_capital', amount: 300, units: 0, fees: 0, taxes: 0, date: '2026-03-01', account_id: null },
    ]);

    expect(n(partitions.get(1).find((t) => t.type === 'return_of_capital').amount)).toBe(200);
    expect(n(partitions.get(2).find((t) => t.type === 'return_of_capital').amount)).toBe(100);
  });
  ...
  it('keeps a corporate action row\'s own fees exactly once (no-op residual in its home partition)', () => {
    const partitions = partitionTxnsByAccount([
      ...
      { type: 'split', units: 40, amount: 0, fees: 7, taxes: 3, date: '2026-02-01', account_id: null },
    ]);
...
describe('buildInvestmentSummaryCorePartitioned — sells consume SAME-account lots', () => {
  ...
  it.each([
  ...
  ])('%s: realized gain comes from B\'s own lots; flat replay would differ', (method, expectedRealized, expectedRemainingCostB) => {
    const { core, partitions, fullyAssigned } = buildInvestmentSummaryCorePartitioned(
      inv, txns, { ...OPTS, costBasisMethod: method },
    );
    ...
    expect(toNumber(core.unrealizedGain)).toBeCloseTo(
      toNumber(partA.core.unrealizedGain) + toNumber(partB.core.unrealizedGain), 10,
    );

    // And it genuinely differs from the flat cross-account replay.
    const flat = buildInvestmentSummaryCore(inv, txns, { ...OPTS, costBasisMethod: method });
  ...
  it('split-then-sell: the split rescales BOTH partitions before the sell consumes B\'s lots', () => {
    ...
    const rows = [
      ...
      { type: 'split', units: 40, amount: 0, fees: 0, taxes: 0, date: '2026-02-01', account_id: null },
      sell(2, 15, 900, '2026-03-01'),
    ];
    const { core, partitions } = buildInvestmentSummaryCorePartitioned(
      stock(55), rows, { ...OPTS, costBasisMethod: 'fifo' },
    );
    ...
    const flat = buildInvestmentSummaryCore(stock(55), rows, { ...OPTS, costBasisMethod: 'fifo' });
  ...
  it('return_of_capital reduces each partition\'s basis by its own unit share', () => {
    ...
    const { core, partitions } = buildInvestmentSummaryCorePartitioned(
      stock(12), rows, { ...OPTS, costBasisMethod: 'weighted_avg' },
    );

    expect(toNumber(partitions.find((p) => p.accountId === 1).core.totalInvested)).toBe(800);  // 1000 − 200
    expect(toNumber(partitions.find((p) => p.accountId === 2).core.totalInvested)).toBe(900);  // 1000 − 100
    ...
    const flat = buildInvestmentSummaryCore(stock(12), rows, { ...OPTS, costBasisMethod: 'weighted_avg' });
    expect(toNumber(flat.totalInvested)).toBe(1700);
  });

  it('re-tag = the math is a pure function of the rows\' CURRENT account_id', () => {
    const before = buildInvestmentSummaryCorePartitioned(stock(12), [
      buy(1, 100, 1000, '2026-01-01'),
      buy(2, 50, 1000, '2026-01-02'),
    ], { ...OPTS, costBasisMethod: 'fifo' });
    // Whole-lot re-tag of B's lot onto A (basis travels with the lot).
    const after = buildInvestmentSummaryCorePartitioned(stock(12), [
      buy(1, 100, 1000, '2026-01-01'),
      buy(1, 50, 1000, '2026-01-02'),
    ], { ...OPTS, costBasisMethod: 'fifo' });
    ...
    expect(toNumber(after.core.totalInvested)).toBe(toNumber(before.core.totalInvested));
    expect(toNumber(after.core.currentValue)).toBe(toNumber(before.core.currentValue));
  });
});
...
describe('buildInvestmentSummaryCorePartitioned — transition & degenerate cases', () => {
  it('any unassigned lot → flat global core, whole investment on the null partition', () => {
    ...
    const { core, partitions, fullyAssigned } = buildInvestmentSummaryCorePartitioned(
      stock(12), rows, { ...OPTS, costBasisMethod: 'fifo' },
    );
    const flat = buildInvestmentSummaryCore(stock(12), rows, { ...OPTS, costBasisMethod: 'fifo' });

    expect(fullyAssigned).toBe(false);
    expect(partitions).toHaveLength(1);
    expect(partitions[0].accountId).toBe(null);
    for (const field of ['totalUnits', 'totalInvested', 'realizedGain', 'unrealizedGain', 'currentValue', 'gainLoss']) {
  ...
  it('fully assigned to ONE broker: identical to the flat replay, attributed to that broker', () => {
    const rows = [
      ...
      { type: 'split', units: 20, amount: 0, fees: 0, taxes: 0, date: '2026-02-01', account_id: null },
      sell(7, 5, 600, '2026-03-01'),
      { type: 'return_of_capital', amount: 100, units: 0, fees: 0, taxes: 0, date: '2026-04-01', account_id: null },
    ];
    const { core, partitions, fullyAssigned } = buildInvestmentSummaryCorePartitioned(
      stock(120), rows, { ...OPTS, costBasisMethod: 'lifo' },
    );
    const flat = buildInvestmentSummaryCore(stock(120), rows, { ...OPTS, costBasisMethod: 'lifo' });
  ...
  it('non-unit-based investments attribute whole-investment only (accrual is non-linear)', () => {
    const savings = { asset_class: 'savings', current_price: 0, interest_rate: 3 };
    const rows = [
      { type: 'buy', amount: 10000, units: 0, date: '2026-01-01', account_id: 4 },
      { type: 'interest', amount: 100, units: 0, date: '2026-06-01', account_id: 4 },
    ];
    ...
    const mixed = buildInvestmentSummaryCorePartitioned(savings, [
      rows[0],
      { ...rows[1], account_id: 9 },
    ], OPTS);
