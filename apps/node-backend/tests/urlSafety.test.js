...
import {
  __isBlockedIpv4 as isBlockedIpv4,
  __isBlockedIpv6 as isBlockedIpv6,
...
describe("assertPublicHttpUrl", () => {
  ...
  it("rejects non-http(s) schemes", async () => {
    for (const u of [
      ...
      await expect(
        assertPublicHttpUrl(u, { lookup: publicLookup }),
        u,
  ...
  it("rejects invalid URLs", async () => {
    await expect(
      assertPublicHttpUrl("http://", { lookup: publicLookup }),
    ).rejects.toBeInstanceOf(BlockedUrlError);
    await expect(
      assertPublicHttpUrl("not a url", { lookup: publicLookup }),
    ).rejects.toBeInstanceOf(BlockedUrlError);
  });
  ...
  it("rejects private IP-literal hosts without needing DNS", async () => {
    for (const u of [
      ...
      await expect(
        assertPublicHttpUrl(u, { resolveDns: false }),
        u,
      ).rejects.toBeInstanceOf(BlockedUrlError);
    }
  });
  ...
  it("allows public IP literals", async () => {
    const url = await assertPublicHttpUrl("https://8.8.8.8/data", {
      resolveDns: false,
    });
  ...
  it("blocks when ANY resolved address is private (mixed result set)", async () => {
    ...
    await expect(
      assertPublicHttpUrl("https://mixed.example.com/x", {
        lookup: mixedLookup,
      }),
    ).rejects.toBeInstanceOf(BlockedUrlError);
  });
  ...
  it("allows a DNS name that resolves only to public addresses", async () => {
    const url = await assertPublicHttpUrl("https://api.example.com/price", {
      lookup: publicLookup,
    });
  ...
  it("rejects when DNS resolution fails or returns nothing", async () => {
    await expect(
      assertPublicHttpUrl("https://nxdomain.example/x", {
        lookup: async () => {
    ...
    await expect(
      assertPublicHttpUrl("https://empty.example/x", {
        lookup: async () => [],
      }),
    ).rejects.toBeInstanceOf(BlockedUrlError);
  });
