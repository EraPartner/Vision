...
describe("plannedTransactionRepository.getAll", () => {
  ...
  it("returns empty items with total=0 by running fallback count query when page is empty", async () => {
    ...
    const result = await plannedTransactionRepository.getAll({
      limit: 25,
      offset: 1000,
    });
  ...
  it("does not run execution or loan schedule follow-up queries when there are no planned rows", async () => {
    ...
    expect(
      sqlCalls.some((sql) =>
        sql.includes("FROM planned_transaction_executions"),
      ),
    ).toBe(false);
    expect(
      sqlCalls.some((sql) =>
        sql.includes("FROM planned_transaction_loan_schedule"),
      ),
    ).toBe(false);
  });
...
describe("plannedTransactionService.create", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("creates loan transaction and inserts schedule entries in a transaction", async () => {
    ...
    const result = await plannedTransactionRepository.create({
      ...
      url: null,
      is_recurring: true,
      recurrence_pattern: null, // service must force 'monthly' for loans
      is_loan: true,
      loan_type: "mortgage",
...
describe("plannedTransactionService.update", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns getById(id) when sanitized update fields are empty", async () => {
    ...
    const result = await plannedTransactionRepository.update(33, {
      unsafe_field: undefined,
    });
  ...
  it("returns updated loan row with executions and schedule", async () => {
    ...
    const result = await plannedTransactionRepository.update(70, {
      memo: "updated memo",
    });
...
describe("plannedTransactionRepository.small mutations", () => {
  ...
  it("rolls back loan schedule replacement when delete fails", async () => {
    ...
    await expect(
      plannedTransactionRepository.replaceLoanSchedule(13, []),
...
describe("plannedTransactionService.updateWithLoanSchedule", () => {
  ...
  it("applies the field update and the schedule replace in ONE transaction, then re-fetches", async () => {
    ...
    const result = await plannedTransactionRepository.updateWithLoanSchedule(
      70,
      {
        loan_regular_payment_amount: 300,
        loan_first_payment_date: "2026-10-01",
      },
      [
        {
          installment_number: 1,
    ...
    expect(clientQuery).toHaveBeenNthCalledWith(
      2,
      expect.stringContaining("UPDATE planned_transactions SET"),
      expect.any(Array),
    );
    expect(clientQuery).toHaveBeenNthCalledWith(
      3,
    ...
    expect(release).toHaveBeenCalledTimes(1);
    expect(result).toEqual(
      expect.objectContaining({ id: 70, loan_schedule: expect.any(Array) }),
    );
  });
...
describe("plannedTransactionService.executeAndAdvance", () => {
  beforeEach(() => vi.clearAllMocks());
  ...
  it("returns { duplicate: false } and commits on fresh execution", async () => {
    ...
    const result = await plannedTransactionRepository.executeAndAdvance(
      1,
      10,
      "2025-01-15",
    );
  ...
  it("inherits tags into transaction_tags with ON CONFLICT DO NOTHING", async () => {
    ...
    await plannedTransactionRepository.executeAndAdvance(
      1,
      10,
      "2025-01-15",
      {},
    ...
    const tagInsertCall = clientQuery.mock.calls.find(
      ([sql]) => typeof sql === "string" && sql.includes("transaction_tags"),
    );
