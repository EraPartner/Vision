...
describe("Validation Middleware", () => {
  describe("validateId", () => {
    it("should accept valid positive integers", () => {
      ...
      expect(validateId("2147483647")).toEqual({
        valid: true,
        value: 2147483647,
      });
    });
    ...
    it("accepts only a plain base-10 positive integer within int32", () => {
      for (const ok of ["1", "42", "00005", "2147483647"]) {
        expect(
          validateId(ok).valid,
          `expected ${JSON.stringify(ok)} to be accepted`,
        ).toBe(true);
      }
    ...
    it("names the offending field in the error", () => {
      expect(validateId("12abc", "patternId").error).toBe(
  ...
  describe("assertOptionalId", () => {
    it("returns undefined for absent/empty values", () => {
      expect(assertOptionalId(undefined, "account_id")).toBeUndefined();
      expect(assertOptionalId(null, "account_id")).toBeUndefined();
      expect(assertOptionalId("", "account_id")).toBeUndefined();
    });
    ...
    it("throws ValidationError for malformed input (would otherwise reach pg as NaN → 500)", () => {
      expect(() => assertOptionalId("abc", "account_id")).toThrow(
        ValidationError,
      );
      expect(() => assertOptionalId("0", "account_id")).toThrow(
        ValidationError,
      );
      expect(() => assertOptionalId("-3", "account_id")).toThrow(
        ValidationError,
      );
    });
  });
  ...
  describe("assertIdParam", () => {
    it("returns a validated id for raw strings and middleware-normalised numbers", () => {
      ...
      expect(assertIdParam(/** @type {any} */ ({ params: { id: 42 } }))).toBe(
        42,
      );
      expect(
        assertIdParam(
          /** @type {any} */ ({ params: { accountId: "7" } }),
          "accountId",
        ),
      ).toBe(7);
      expect(
        assertIdParam(
          /** @type {any} */ ({ params: { id: String(MAX_INT32_ID) } }),
        ),
      ).toBe(MAX_INT32_ID);
    });
    ...
    it("throws before a handler can retarget or use an out-of-range id", () => {
      for (const id of [
        ...
        "0",
        "-1",
        String(MAX_INT32_ID + 1),
      ]) {
        expect(() =>
          assertIdParam(/** @type {any} */ ({ params: { id } })),
        ).toThrow(ValidationError);
      }
      expect(() => assertIdParam(/** @type {any} */ ({ params: {} }))).toThrow(
        ValidationError,
      );
    });
  });
  ...
  describe("sanitizeUpdateFields", () => {
    ...
    it("should throw for unknown resource type", () => {
      expect(() =>
        sanitizeUpdateFields("unknown_table", { field: "value" }),
  ...
  describe("validateIntArray", () => {
    it("should accept valid integer arrays", () => {
      expect(validateIntArray([1, 2, 3])).toEqual({
        valid: true,
        value: [1, 2, 3],
      });
    });
    ...
    it("accepts only plain base-10 positive integers per element", () => {
      expect(validateIntArray(["1", "00005", 2147483647]).value).toEqual([
        1, 5, 2147483647,
      ]);
    ...
    it("rejects the whole array on one bad element and names the value", () => {
      ...
      expect(result.error).toBe(
        "excludedCategoryIds contains invalid value: 12abc",
      );
  ...
  describe("coercedIdSchema agrees with validateId", () => {
    ...
    it("agrees with validateId on every shape", () => {
      ...
      for (const v of shapes) {
        ...
        expect(coerced.success, `disagreed on ${JSON.stringify(v)}`).toBe(
          strict.valid,
        );
  ...
  describe("parseOverrideId", () => {
    it("agrees with validateId on every non-null shape, at the int4 bound", () => {
      ...
      for (const v of shapes) {
        ...
        if (strict.valid) {
          expect(
            parseOverrideId(v, "field"),
            `disagreed on ${JSON.stringify(v)}`,
          ).toBe(strict.value);
        } else {
          expect(
            () => parseOverrideId(v, "field"),
            `expected ${JSON.stringify(v)} to throw`,
          ).toThrow(ValidationError);
        }
      }
    });
    ...
    it("names the field in the error", () => {
      expect(() => parseOverrideId("1e3", "investment_id")).toThrow(
