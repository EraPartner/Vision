...
describe("Transaction Routes", () => {
  ...
  describe("GET /", () => {
    ...
    it("should return transactions with data", async () => {
      ...
      expect(res.body.data.items[0].transaction_date).toBe("2026-01-15");
      expect(res.body.data.items[0]).not.toHaveProperty("date");
    });
    ...
    it("should respect pagination", async () => {
      ...
      const res = await api
        .get("/api/transactions/?limit=2&offset=3")
        .expect(200);
    ...
    it("should handle uncategorised filter", async () => {
      transactionRepository.getUncategorisedWithCount.mockResolvedValue({
        rows: [],
        total: 0,
      });
      ...
      expect(
        transactionRepository.getUncategorisedWithCount,
      ).toHaveBeenCalled();
    });
    ...
    it("should support filtering by transaction_id", async () => {
      ...
      const res = await api
        .get("/api/transactions/?transaction_id=42")
        .expect(200);
    ...
    it("rejects a malformed account_id through the real validation guard (400 envelope)", async () => {
      const res = await api
        .get("/api/transactions/?account_id=abc")
        .expect(400);
  ...
  describe("GET /:id", () => {
    it("should return transaction by id", async () => {
      ...
      expect(res.body.data.transaction_date).toBe("2026-01-15");
      expect(res.body.data).not.toHaveProperty("date");
    });
  ...
  describe("GET /export/json", () => {
    ...
    it("should stream NDJSON with correct Content-Type", async () => {
      ...
      expect(res.headers["content-disposition"]).toMatch(
        /filename=transactions_export.*\.ndjson/,
      );
    });
  ...
  describe("POST /", () => {
    ...
    it("should return 400 for missing fields", async () => {
      const res = await api
        .post("/api/transactions/")
        .send({ bank_account: "Chase", amount: -50.0 })
        .expect(400);
  ...
  describe("PATCH /:id", () => {
    ...
    it("should sanitize server error detail when patch fails", async () => {
      transactionRepository.update.mockRejectedValue(
        new Error("constraint: internal detail"),
      );
  ...
  describe("DELETE /:id", () => {
    ...
    it("removes attachment files from disk after the delete", async () => {
      ...
      expect(
        attachmentRepository.listPathsByTransactionIds,
      ).toHaveBeenCalledWith([1]);
      expect(removeAttachmentFile).toHaveBeenCalledWith(
        "attachments/1/receipt-a.png",
      );
      expect(removeAttachmentFile).toHaveBeenCalledWith(
        "attachments/1/receipt-b.pdf",
  ...
  describe("DELETE /transfers/:id", () => {
    it("clears the transfer mark and returns 204 with no body", async () => {
      const res = await api
        .delete("/api/transactions/transfers/10")
        .expect(204);
