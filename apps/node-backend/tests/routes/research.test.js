...
describe("Research route parameter guards", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    researchAggregator.fetch.mockResolvedValue({
      provider: "yahoo",
      source: "live",
      data: { x: 1 },
    });
    researchAggregator.fetchMacroSeries.mockResolvedValue({
      provider: "fred",
      source: "live",
      data: { points: [] },
    });
    researchMappingService.list.mockResolvedValue([]);
    researchMappingService.resolve.mockResolvedValue({
      instrumentKey: "X",
      keyType: "isin",
      proposals: [],
      existing: [],
  ...
  describe("symbol requireds", () => {
    ...
    it("uses the first entry of an array-valued symbol and trims scalars", async () => {
      ...
      expect(researchAggregator.fetch).toHaveBeenCalledWith(
        "quote",
        expect.objectContaining({ symbol: "AAPL" }),
      );

      await api.get(`${BASE}/news`).query({ symbol: "  TSLA  " }).expect(200);
      expect(researchAggregator.fetch).toHaveBeenLastCalledWith("news", {
        symbol: "TSLA",
      });
    });
  });
  ...
  describe("macro series guards", () => {
    it("rejects an unknown provider with the provider list", async () => {
      const res = await api
        .get(`${BASE}/macro/series`)
        .query({ provider: "nope", series_id: "CPIAUCSL" })
        .expect(400);
      expect(res.body.error.message).toBe(
        "provider must be one of: fred, eurostat, dbnomics",
      );
    });
    ...
    it("rejects a series_id that fails the provider shape", async () => {
      const res1 = await api
        .get(`${BASE}/macro/series`)
        .query({ provider: "fred", series_id: "a/b/c" })
        .expect(400);
    ...
    it("fetches with defaulted range for a valid provider/series pair", async () => {
      await api
        .get(`${BASE}/macro/series`)
        .query({ provider: "fred", series_id: "CPIAUCSL" })
        .expect(200);
  ...
  describe("mapping guards", () => {
    it("GET /mappings requires instrument_key and defaults key_type to isin", async () => {
      ...
      await api
        .get(`${BASE}/mappings`)
        .query({ instrument_key: "US0378331005" })
        .expect(200);
      expect(researchMappingService.list).toHaveBeenCalledWith(
        "US0378331005",
        "isin",
      );
    });
    ...
    it("rejects an unknown key_type", async () => {
      const res = await api
        .get(`${BASE}/mappings`)
        .query({ instrument_key: "X", key_type: "weird" })
        .expect(400);
      expect(res.body.error.message).toBe(
        "key_type must be 'isin' or 'internal'",
      );
    });
    ...
    it("POST /mappings/resolve requires query and validates investment_id strictly", async () => {
      const res = await api
        .post(`${BASE}/mappings/resolve`)
        .send({ instrument_key: "X", query: "" })
        .expect(400);
      ...
      await api
        ...
        .expect(200);
      expect(researchMappingService.resolve).toHaveBeenCalledWith(
    ...
    it("POST /mappings/resolve translates service keys to the snake_case wire contract", async () => {
      researchMappingService.resolve.mockResolvedValue({
        instrumentKey: "US0378331005",
        keyType: "isin",
        proposals: [
          { provider: "yahoo", status: "auto", providerSymbol: "AAPL" },
        ],
        existing: [],
      });
      ...
      expect(res.body.data).not.toHaveProperty("instrumentKey");
      expect(res.body.data).not.toHaveProperty("keyType");
    });
    ...
    it("POST /mappings rejects a missing or empty mappings array", async () => {
      for (const mappings of [undefined, "x", []]) {
        const res = await api
          .post(`${BASE}/mappings`)
          .send({ instrument_key: "X", mappings })
          .expect(400);
        expect(res.body.error.message).toBe(
  ...
  it("returns explicit null data when scorecard inputs are unavailable", async () => {
    researchAggregator.fetchFundamentals.mockResolvedValue({
      source: "unavailable",
    });
