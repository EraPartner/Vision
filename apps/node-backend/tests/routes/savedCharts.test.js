...
const { default: savedChartsRouter } = await import('../../src/routes/savedCharts.js');
...
describe('Saved Charts Routes', () => {
  ...
  describe('GET /', () => {
    ...
    it('pages and reports the full total when limit/offset are supplied', async () => {
      ...
      const res = await api.get(BASE).query({ limit: '1', offset: '1' }).expect(200);

      expect(savedChartsRepository.getAll).toHaveBeenCalledWith({ limit: 1, offset: 1 });
      expect(res.body).toEqual(okEnvelope({ items: [{ id: 2 }], total: 5, limit: 1, offset: 1 }));
    });
  ...
  describe('POST /', () => {
    it('throws ValidationError when name is missing', async () => {
      const res = await api.post(BASE).send({ chartType: 'line', categoryIds: [1] }).expect(400);
      expect(res.body).toEqual(errEnvelope({ code: 'VALIDATION_ERROR' }));
    });

    it('throws ValidationError when chartType is invalid', async () => {
      await api.post(BASE).send({ name: 'Main', chartType: 'pie', categoryIds: [1] }).expect(400);
    });

    it('throws ValidationError when categoryIds has invalid entries', async () => {
      await api.post(BASE).send({ name: 'Main', chartType: 'line', categoryIds: [1, 'nope'] }).expect(400);
    });
    ...
    it('creates chart with trimmed name and default chart type', async () => {
      ...
      const res = await api.post(BASE).send({ name: '  Main  ', categoryIds: ['1', 2] }).expect(201);
      ...
      expect(res.body).toEqual(okEnvelope({ id: 4, name: 'Main', chart_type: 'line', category_ids: [1, 2] }));
    });

    it('normalizes tagIds when provided', async () => {
      savedChartsRepository.create.mockResolvedValue({ id: 5, name: 'Tagged', tag_ids: [5, 6] });

      await api.post(BASE).send({ name: 'Tagged', categoryIds: [], tagIds: ['5', 6] }).expect(201);
    ...
    it('throws ValidationError when tagIds has invalid entries', async () => {
      await api.post(BASE).send({ name: 'Main', categoryIds: [], tagIds: [1, 'nope'] }).expect(400);
    });
    ...
    it('passes all-source flags through to the repository', async () => {
      ...
      await api.post(BASE).send({ name: 'AllTags', categoryIds: [], allTags: true }).expect(201);
    ...
    it('throws ValidationError when an all-source flag is not a boolean', async () => {
      await api.post(BASE).send({ name: 'Main', categoryIds: [], allTags: 'yes' }).expect(400);
    });

    it('accepts the ranked variant on a bar chart', async () => {
      savedChartsRepository.create.mockResolvedValue({ id: 7, name: 'Ranked' });

      await api.post(BASE).send({ name: 'Ranked', categoryIds: [1], chartType: 'bar', chartVariant: 'ranked' }).expect(201);
    ...
    it('rejects the ranked variant on a line chart', async () => {
      await api.post(BASE).send({ name: 'Bad', categoryIds: [1], chartType: 'line', chartVariant: 'ranked' }).expect(400);
    });

    it('rejects a whitespace-only name', async () => {
      await api.post(BASE).send({ name: '   ', categoryIds: [1] }).expect(400);
      expect(savedChartsRepository.create).not.toHaveBeenCalled();
    });

    it('rejects a non-string name', async () => {
      await api.post(BASE).send({ name: 123, categoryIds: [1] }).expect(400);
    });
    ...
    it('wraps a scalar categoryIds into a one-element int array', async () => {
      ...
      await api.post(BASE).send({ name: 'Scalar', categoryIds: '5' }).expect(201);
    ...
    it('normalizes recipientIds and rejects invalid entries', async () => {
      ...
      await api.post(BASE).send({ name: 'R', categoryIds: [], recipientIds: ['2', 3] }).expect(201);
      expect(savedChartsRepository.create).toHaveBeenCalledWith(
        expect.objectContaining({ recipientIds: [2, 3] }),
      );

      await api.post(BASE).send({ name: 'R', categoryIds: [], recipientIds: [0] }).expect(400);
    });

    it('rejects an unknown chartVariant', async () => {
      await api.post(BASE).send({ name: 'Main', categoryIds: [1], chartVariant: 'wavy' }).expect(400);
    });

    it('rejects an unknown timeBucket', async () => {
      await api.post(BASE).send({ name: 'Main', categoryIds: [1], timeBucket: 'weekly' }).expect(400);
    });

    it('rejects a null chartType (only undefined may fall back to the default)', async () => {
      await api.post(BASE).send({ name: 'Main', categoryIds: [1], chartType: null }).expect(400);
    });

    it('passes a yearly timeBucket through', async () => {
      savedChartsRepository.create.mockResolvedValue({ id: 12 });

      await api.post(BASE).send({ name: 'Yearly', categoryIds: [1], timeBucket: 'yearly' }).expect(201);
    ...
    it.each([
      ...
      await api.post(BASE).send({ name: 'Combo', categoryIds: [1], chartType, chartVariant }).expect(400);
      expect(savedChartsRepository.create).not.toHaveBeenCalled();
    });
    ...
    it('rejects a variant that is illegal against the DEFAULT line chartType', async () => {
      ...
      await api.post(BASE).send({ name: 'Combo', categoryIds: [1], chartVariant: 'stacked' }).expect(400);
    });
    ...
    it.each([
      ...
      await api.post(BASE).send({ name: 'Combo', categoryIds: [1], chartType, chartVariant }).expect(201);
    ...
    it('rejects an unparsable dateRangeStart', async () => {
      await api.post(BASE).send({ name: 'Dated', categoryIds: [1], dateRangeStart: 'not-a-date' }).expect(400);
    });
    ...
    it('passes a valid dateRangeStart through unchanged and maps empty string to null', async () => {
      ...
      await api.post(BASE).send({ name: 'Dated', categoryIds: [1], dateRangeStart: '2025-01-01', dateRangeEnd: '' }).expect(201);
  ...
  describe('PATCH /:id', () => {
    ...
    it('throws ValidationError when categoryIds is invalid', async () => {
      await api.patch(`${BASE}/1`).send({ categoryIds: [1, 'bad-id'] }).expect(400);
    });

    it('throws NotFoundError when chart does not exist', async () => {
      savedChartsRepository.update.mockResolvedValue(null);

      const res = await api.patch(`${BASE}/9`).send({ name: 'Updated' }).expect(404);
    ...
    it('updates chart and normalizes categoryIds when provided', async () => {
      ...
      const res = await api.patch(`${BASE}/9`).send({ categoryIds: ['3', 4], chartType: 'bar' }).expect(200);
    ...
    it('normalizes tagIds when provided', async () => {
      ...
      await api.patch(`${BASE}/9`).send({ tagIds: ['7', 8] }).expect(200);
    ...
    it('throws ValidationError when tagIds is invalid', async () => {
      await api.patch(`${BASE}/1`).send({ tagIds: [1, 'bad-id'] }).expect(400);
    });
    ...
    it('rejects an illegal chartType/chartVariant pair when both are provided', async () => {
      await api.patch(`${BASE}/1`).send({ chartType: 'line', chartVariant: 'stacked' }).expect(400);
    ...
    it('rejects an unparsable dateRangeEnd', async () => {
      await api.patch(`${BASE}/1`).send({ dateRangeEnd: 'yesterdayish' }).expect(400);
    });
