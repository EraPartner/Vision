...
describe("Investment Routes", () => {
  ...
  describe("GET /transactions", () => {
    it("should cache and differentiate entries by limit parameter", async () => {
      ...
      const resA1 = await api
        .get(`${BASE}/transactions`)
        .query(query10)
        .expect(200);
      expect(resA1.body).toEqual(
        okEnvelope(expect.objectContaining({ items: repoResultA, limit: 10 })),
      );

      const resA2 = await api
        .get(`${BASE}/transactions`)
        .query(query10)
        .expect(200);
      expect(resA2.body).toEqual(
        okEnvelope(expect.objectContaining({ items: repoResultA, limit: 10 })),
      );
      ...
      expect(resB.body).toEqual(
        okEnvelope(expect.objectContaining({ items: repoResultB, limit: 20 })),
      );
      ...
      expect(
        portfolioTransactionRepository.getAllByInvestmentIds,
      ).toHaveBeenNthCalledWith(1, expect.objectContaining({ limit: 10 }));
      expect(
        portfolioTransactionRepository.getAllByInvestmentIds,
      ).toHaveBeenNthCalledWith(2, expect.objectContaining({ limit: 20 }));
    });
  });
  ...
  describe("GET /", () => {
    ...
    it("should respect pagination and filters", async () => {
      ...
      expect(investmentRepository.getAllWithCount).toHaveBeenCalledWith(
    ...
    it("should handle errors", async () => {
      ...
      expect(res.body).toEqual(
        errEnvelope({ code: "INTERNAL_SERVER_ERROR", message: "DB error" }),
      );
    });
  });
  ...
  describe("POST /", () => {
    it("should create investment with 201", async () => {
      ...
      await api
        .post(`${BASE}/`)
        .send({ name: "Bitcoin", asset_class: "crypto", currency: "EUR" })
        .expect(201);
    });
    ...
    it("should handle errors", async () => {
      ...
      const res = await api
        .post(`${BASE}/`)
        .send({ name: "Bitcoin", asset_class: "crypto" })
        .expect(500);
      expect(res.body).toEqual(
        errEnvelope({ code: "INTERNAL_SERVER_ERROR", message: "DB error" }),
      );
  ...
  describe("GET /:id/transactions", () => {
    ...
    it("should filter by type", async () => {
      ...
      expect(
        portfolioTransactionRepository.getAllWithCount,
      ).toHaveBeenCalledWith(expect.objectContaining({ type: "buy" }));
    });
  ...
  describe("POST /:id/transactions", () => {
    it("should create portfolio transaction with 201", async () => {
      ...
      await api
        ...
        .send({ type: "buy", date: "2026-01-15", amount: 1000 })
        .expect(201);
    });
  ...
  describe("DELETE /transactions/:txnId", () => {
    ...
    it("should throw ValidationError for invalid ID", async () => {
      ...
      expect(portfolioTransactionService.remove).not.toHaveBeenCalled();
    });
  ...
  describe("PATCH /transactions/:txnId", () => {
    ...
    it("should handle errors", async () => {
      portfolioTransactionRepository.update.mockRejectedValue(
        new Error("DB error"),
      );
    ...
    it("rejects a free-text or cleared currency and uppercases a valid one", async () => {
      ...
      await api
        .patch(`${BASE}/transactions/1`)
        .send({ currency: "usd", fx_rate_to_eur: 0.9 })
        .expect(200);
  ...
  describe("GET /:id/summary", () => {
    ...
    it("should handle errors", async () => {
      portfolioTransactionRepository.getSummary.mockRejectedValue(
        new Error("DB error"),
      );
