...
describe("Info Routes", () => {
  ...
  describe("GET /transaction-count", () => {
    ...
    it("should handle errors", async () => {
      infoRepository.getTransactionCount.mockRejectedValue(
        new Error("DB error"),
      );
  ...
  describe("GET /net-worth", () => {
    ...
    it("should handle errors", async () => {
      infoRepository.getNetWorthFromSnapshots.mockRejectedValue(
        new Error("DB error"),
      );
    ...
    it("should paginate snapshots newest-first when limit/offset supplied", async () => {
      ...
      const res = await api
        .get(`${BASE}/net-worth`)
        .query({ currency: "AUD", limit: "2", offset: "0" })
        .expect(200);
    ...
    it("should honor offset for pagination", async () => {
      ...
      const res = await api
        .get(`${BASE}/net-worth`)
        .query({ currency: "CAD", limit: "2", offset: "2" })
        .expect(200);
  ...
  describe("GET /recurring-patterns", () => {
    ...
    it("should return empty recurring payload when detector fails", async () => {
      mockDetectRecurringPatterns.mockRejectedValue(
  ...
  describe("POST /exchange-rates/refresh", () => {
    it("should clear cache and refresh exchange rates", async () => {
      const res = await api
        .post(`${BASE}/exchange-rates/refresh`)
        .send({})
        .expect(200);
      ...
      expect(mockWarmCache).toHaveBeenCalledTimes(1);
      expect(res.body).toEqual(
        okEnvelope({ message: "Exchange rates refreshed from ECB" }),
      );
    });
  ...
  describe("POST /refresh-views", () => {
    ...
    it("should handle refresh-view failures", async () => {
      mockRefreshMaterializedViews.mockRejectedValueOnce(
  ...
  describe("GET /portfolio-performance", () => {
    ...
    it("should default invalid currency input to EUR", async () => {
      ...
      try {
        ...
        await api
          .get(`${BASE}/portfolio-performance`)
          .query({ currency: "invalid-currency" })
          .expect(200);
  ...
  describe("warmInfoCaches", () => {
    ...
    it("should prewarm both caches and serve warmed values without extra repository calls", async () => {
      ...
      try {
        ...
        const netWorthRes = await api
          .get(`${BASE}/net-worth`)
          .query({ currency: "JPY" })
          .expect(200);
        const perfRes = await api
          .get(`${BASE}/portfolio-performance`)
          .query({ currency: "JPY" })
          .expect(200);

        expect(infoRepository.getNetWorthFromSnapshots).toHaveBeenCalledTimes(
          1,
        );
        expect(mockGetSnapshots).toHaveBeenCalledTimes(1);
        expect(netWorthRes.body).toEqual(okEnvelope(netWorthPayload));
        expect(perfRes.body).toEqual(
          okEnvelope(
            expect.objectContaining({
              currency: "JPY",
              start_date: "2000-01-01",
    ...
    it("should continue warming portfolio cache when net-worth warm fails", async () => {
      ...
      try {
        vi.setSystemTime(new Date("2026-04-11T10:00:00.000Z"));
        infoRepository.getNetWorthFromSnapshots.mockRejectedValueOnce(
          new Error("net-worth warm failed"),
        );
        mockGetSnapshots.mockResolvedValue([]);
  ...
  describe("GET /inflation-rates", () => {
    it("should return Belgian inflation rates", async () => {
      ...
      const res = await api
        .get(`${BASE}/inflation-rates`)
        .query({ start_month: "2024-01", end_month: "2024-12" })
        .expect(200);
    ...
    it("should ignore invalid month params", async () => {
      ...
      await api
        .get(`${BASE}/inflation-rates`)
        .query({ start_month: "invalid", end_month: "2024/01" })
        .expect(200);
    ...
    it("should pass db_only flag and enable background refresh scheduling", async () => {
      ...
      await api
        .get(`${BASE}/inflation-rates`)
        .query({ db_only: "true", start_month: "2024-01" })
        .expect(200);
    ...
    it("keeps the offline-safe db-only default for an unsupported value", async () => {
      mockInflationService.getInflationRates.mockResolvedValue({
        source: "db",
        rates: [],
      });
    ...
    it("should handle inflation route errors", async () => {
      mockInflationService.getInflationRates.mockRejectedValue(
        new Error("boom"),
      );
  ...
  describe("POST /inflation-rates/refresh", () => {
    it("should refresh Belgian inflation rates", async () => {
      ...
      const res = await api
        .post(`${BASE}/inflation-rates/refresh`)
        .send({})
        .expect(200);
