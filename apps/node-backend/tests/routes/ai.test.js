...
describe("POST /api/ai/chat/stream", () => {
  ...
  it("answers a 400 VALIDATION_ERROR envelope when conversationId is not a UUID", async () => {
    const res = await api
      .post(`${BASE}/chat/stream`)
      .send({ conversationId: "not-a-uuid", message: "hi" })
      .expect(400);
  ...
  it("streams SSE events in order: user_message → token → tool_call → tool_result → done", async () => {
    ...
    const assistantMsg = {
      id: "a1",
      role: "assistant",
      content: "Your top category was food.",
    };
    ...
    const res = await api
      .post(`${BASE}/chat/stream`)
      .send({ message: "hi", conversationId: UUID })
      .expect(200);
  ...
  it("emits error SSE event on AiChatServiceError and ends response", async () => {
    runChatTurn.mockRejectedValue(
      new AiChatServiceError("Model unavailable", {
...
describe("POST /api/ai/chat", () => {
  ...
  it("answers with the AiChatServiceError status/code when the service rejects", async () => {
    runChatTurn.mockRejectedValue(
      new AiChatServiceError("bad", { code: "INVALID_INPUT", status: 400 }),
    );
  ...
  it("passes a 5xx AiChatServiceError through the generic handler untranslated (same wire shape as the old shim)", async () => {
    ...
    expect(res.body).toEqual(
      errEnvelope({
        code: "OLLAMA_ERROR",
        message: "Ollama call failed: connect ECONNREFUSED",
      }),
    );
  });
...
describe("POST /api/ai/chat body validation", () => {
  ...
  it("accepts an UPPERCASE conversation UUID (regex is case-insensitive) and forwards it unchanged", async () => {
    ...
    await api
      .post(`${BASE}/chat`)
      .send({ message: "hi", conversationId: upper })
      .expect(200);

    expect(runChatTurn).toHaveBeenCalledWith(
      expect.objectContaining({ conversationId: upper }),
    );
  });
  ...
  it("maps a null conversationId to null (new conversation)", async () => {
    ...
    await api
      .post(`${BASE}/chat`)
      .send({ message: "hi", conversationId: null })
      .expect(200);
  ...
  it("rejects a non-string conversationId", async () => {
    const res = await api
      .post(`${BASE}/chat`)
      .send({ message: "hi", conversationId: 42 })
      .expect(400);
    expect(res.body).toEqual(errEnvelope({ code: "VALIDATION_ERROR" }));
    expect(runChatTurn).not.toHaveBeenCalled();
  });
  ...
  it("defaults useTools to true when omitted and honours an explicit false", async () => {
    ...
    await api
      .post(`${BASE}/chat`)
      .send({ message: "hi", useTools: false })
      .expect(200);
    expect(runChatTurn).toHaveBeenLastCalledWith(
      expect.objectContaining({ useTools: false }),
    );
  });
  ...
  it("rejects a non-boolean useTools", async () => {
    await api
      .post(`${BASE}/chat`)
      .send({ message: "hi", useTools: "yes" })
      .expect(400);
  });

  it("passes retryLastTurn through only as a boolean", async () => {
    runChatTurn.mockResolvedValue(okTurn);
    ...
    await api
      ...
      .expect(200);
    expect(runChatTurn).toHaveBeenLastCalledWith(
      expect.objectContaining({ retryLastTurn: true }),
    );
...
describe("AI conversation routes validation", () => {
  ...
  describe("POST /conversations", () => {
    it("creates with optional title/model absent (even without a body)", async () => {
      ...
      expect(createEmptyConversation).toHaveBeenCalledWith({
        title: undefined,
        model: undefined,
      });
    });
