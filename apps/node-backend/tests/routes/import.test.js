...
const apiProd = routeAgent(importRouter, {
  mountPath: BASE,
  isProduction: () => true,
});
...
describe("Import Routes", () => {
  ...
  describe("POST /csv", () => {
    ...
    it("should return 400 for invalid bank config", async () => {
      runImportPipeline.mockRejectedValue(
        new Error("No configuration found for bank"),
      );
  ...
  describe("POST /recipients", () => {
    it("should return 201 with completed status on successful import", async () => {
      ...
      expect(res.body).toEqual(
        okEnvelope(expect.objectContaining({ status: "completed" })),
      );
    });
  ...
  describe("POST /categories", () => {
    it("should return 201 with completed status on successful import", async () => {
      importCategoriesCSV.mockResolvedValue({
        total_processed: 2,
        imported: 2,
        skipped: 0,
        errors: 0,
  ...
  describe("DELETE /batches/:id rollback callback parity (listener-free)", () => {
    ...
    it("refreshes aggregations when recipient cleanup is the only side effect", async () => {
      ...
      expect(clearForecastMcCaches).toHaveBeenCalledOnce();
      expect(scheduleMaterializedViewRefresh).toHaveBeenCalledOnce();
    ...
    it("skips refresh when both side-effect counts are zero", async () => {
      ...
      expect(clearForecastMcCaches).not.toHaveBeenCalled();
      expect(scheduleMaterializedViewRefresh).not.toHaveBeenCalled();
    ...
    it("still returns the rollback body when aggregation refresh fails", async () => {
      ...
      clearForecastMcCaches.mockRejectedValue(new Error("cache clear failed"));
      const res = { ok: vi.fn() };
  ...
  describe("multer error middleware", () => {
    ...
    it("maps other multer errors to a 400 VALIDATION_ERROR envelope", async () => {
      ...
      expect(res.body).toEqual(
        errEnvelope({
          code: "VALIDATION_ERROR",
          message: "Upload error: unexpected file",
        }),
      );
    });
  ...
  describe("POST /batches/:id/rows/:rowId/category-override", () => {
    ...
    it("clears the override when category_id is null", async () => {
      ...
      expect(overrideCategory).toHaveBeenCalledWith({
        batchId: 1,
        rowId: 5,
        categoryId: null,
      });
...
describe("Saved custom parser routes", () => {
  ...
  describe("POST /parsers", () => {
    it("creates a parser and returns 201", async () => {
      ...
      const res = await api
        .post(`${BASE}/parsers`)
        .send({ name: "My Bank", config: validConfig })
        .expect(201);
    ...
    it("rejects a config missing required columns", async () => {
      const res = await api
        .post(`${BASE}/parsers`)
        .send({ name: "X", config: { dateColumn: "Date" } })
        .expect(400);
      expect(res.body).toEqual(errEnvelope({ code: "VALIDATION_ERROR" }));
    });
    ...
    it("maps a unique-violation to ConflictError", async () => {
      const err = Object.assign(new Error("dup"), {
        code: "23505",
        constraint: "uq_custom_parser_configs_name_kind",
      });
