...
describe("per-currency statement balance routes (ADR-089 D2)", () => {
  beforeEach(() => vi.clearAllMocks());

  it("stores a currency reading and invalidates account-derived caches", async () => {
    accountService.setStatementBalance.mockResolvedValue({
      account_id: 7,
      currency: "USD",
      balance: "120.0000",
    ...
    await api
      .put(`${BASE}/7/statement-balances/USD`)
      .send({ balance: 120, date: "2026-09-04" })
      .expect(200);
    expect(accountService.setStatementBalance).toHaveBeenCalledWith(7, "USD", {
      balance: 120,
      date: "2026-09-04",
    });
  ...
  it("deletes only the named currency reading", async () => {
    accountService.removeStatementBalance.mockResolvedValue({
      account_id: 7,
      currency: "USD",
    });
    await api.delete(`${BASE}/7/statement-balances/USD`).expect(200);
    expect(accountService.removeStatementBalance).toHaveBeenCalledWith(
      7,
      "USD",
    );
    expect(invalidatePortfolioCaches).toHaveBeenCalledOnce();
...
describe("POST /:id/merge — source_ids are rejected, not filtered", () => {
  ...
  it("merges a fully-valid body unchanged (parseInt coercion preserved)", async () => {
    ...
    expect(res.body).toEqual(
      okEnvelope({ into: 1, merged: [2, 3], links: [] }),
    );
  });
  ...
  it("names each offending entry (index and raw value) in the 400", async () => {
    const res = await api
      .post(`${BASE}/1/merge`)
      .send({ source_ids: [2, "abc", null] })
      .expect(400);
    expect(res.body.error.message).toMatch(
      /source_ids\[1\] \("abc"\).*source_ids\[2\] \(null\)/s,
    );
    expect(mergeAccounts).not.toHaveBeenCalled();
  });
...
describe("GET /:id/merge-preview — ?into= is a strict id", () => {
  ...
  it("rejects a coercible non-id rather than previewing a different account", async () => {
    for (const raw of [
      ...
      const res = await api
        .get(`${BASE}/1/merge-preview?into=${encodeURIComponent(raw)}`)
        .expect(400);
      expect(res.body.error.message).toBe(
        "into must be a positive integer account id",
      );
    }
...
describe("Account Routes — GET / pagination is opt-in", () => {
  ...
  it("returns the full list and no limit/offset when neither param is sent", async () => {
    ...
    expect(res.body).toEqual(
      okEnvelope({ items: [{ id: 1 }, { id: 2 }], total: 2, links: [] }),
    );
  });
  ...
  it("pages and reports the full total when limit/offset are supplied", async () => {
    ...
    expect(accountService.list).toHaveBeenCalledWith({
      active: undefined,
      limit: 1,
      offset: 2,
    });
