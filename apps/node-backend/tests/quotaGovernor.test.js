...
describe("createQuotaGovernor — dayMirror eviction", () => {
  it("reserves a persistent daily slot atomically and fails closed without a reservation store", async () => {
    const tryReserveDay = async (_provider, _day, limit, delta) =>
      delta <= limit ? delta : null;
    const governed = createQuotaGovernor({
      limits: { fmp: { perDay: 1 } },
      store: {
        getDayCount: async () => 0,
        addDayCount: async () => {},
    ...
    expect(await governed.reserve("fmp")).toBe(true);
    const unpersisted = createQuotaGovernor({ limits: { fmp: { perDay: 1 } } });
    expect(await unpersisted.reserve("fmp")).toBe(false);
  });
