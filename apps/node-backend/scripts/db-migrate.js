 ...
 * order is $ALEMBIC_BIN, then the repo venv (`venv/bin/alembic`) when present,
 * then PATH lookup.
 *
...
// Parity with the retired venv-based npm scripts (`venv/bin/alembic ...`):
// prefer the repo venv's alembic when ALEMBIC_BIN is not set and the venv
// exists; otherwise migrate.js falls back to PATH lookup. Must happen before
// migrate.js is imported — it captures ALEMBIC_BIN at module load.
if (!process.env.ALEMBIC_BIN) {
  const venvAlembic = path.join(REPO_ROOT, 'venv', 'bin', 'alembic');
  if (existsSync(venvAlembic)) {
    process.env.ALEMBIC_BIN = venvAlembic;
  }
}
...
const { runMigrations, runAlembicCommand } = await import('../src/database/migrate.js');
