...
aliases:
  [route-service-boundary, no-repo-direct-from-route, service-seam enforcement]
---
...
The remaining investments handler moved from `controllers/investmentController.js` to
`services/investmentService.js`. `routes/investments.js` now depends directly on that service, so
the repository has no controller-to-repository exception to this boundary.
