...
**Partially supersedes ADR-020.** The premium layer returns, but with the GPU budget ADR-020 fought for built in — the difference from the original Phase 9 implementation is _bounded scope_, not lower ambition:
...
- Build and on-stack visual verification were deferred at the user's request.
