...
aliases:
  [
    ...
    trades as transfers,
    trade cash leg,
    single source of truth cash,
  ]
---

# ADR-090: Cash Sleeve & Trades = Transfers

> [!update] Retired by ADR-108
> Synthetic trade cash legs were removed in favour of real imported brokerage cash rows. Migration
> `0102_retire_adr090_transaction_schema` removes the unused
> `transactions.portfolio_transaction_id` column and narrows `transfer_source` after fail-closed
> zero-row guards. The live Vision and synthetic Demo databases both passed the zero-row preflight
> on 2026-09-08.
...
income/spending aggregates _for free_. It is marked `transfer_source = 'trade'` (not `'auto'`) and
...
- _Double-counting_ (the flagged danger): the cash leg is `is_transfer=true` → excluded from
...
- _Reconciler collision_: isolated by `transfer_source='trade'` (see above); add a regression test
  that a single-sided `'trade'` leg survives `reconcileTransfers`.
- _Orphaned legs_: `ON DELETE CASCADE` removes the cash leg when the trade is deleted.
