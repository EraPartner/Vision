...
`splitRepository.addPayment` remains the authoritative exact four-decimal cap under a row lock.
...
legacy sequence high-water mark so omitted IDs are not reused. This supersedes only ADR-109's
...
Phase E), and the side-table idiom. **Supersedes ADR-004** (status updated).
