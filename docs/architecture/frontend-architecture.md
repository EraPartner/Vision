...
**Tier semantics**: `reduced` = no backdrop-filter glass + no liquid canvas; `standard` = animated CSS aurora blobs + glass; `enhanced` = adds `ShaderAurora` (WebGL) + Electron vibrancy. A successfully built shader marks `fx-webgl-live` and freezes the CSS blobs as a static fallback, leaving one live animator; failed initialization or context loss removes that marker and restores CSS drift. **Effective tier**: `reduced` when `autoAdaptDisplay` is on and the window is on a large display (`screen.width × screen.height × devicePixelRatio² > 6,000,000` px), otherwise the chosen tier.
