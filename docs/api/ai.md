...
```json
{
  ...
  "error": {
    ...
    "message": "Ollama not reachable: connect ECONNREFUSED"
  }
}
```
...
- `retryLastTurn` optional boolean (default `false`). When true, `conversationId` is required and the latest incomplete persisted user turn is regenerated without appending that user message again. The server uses the persisted row as the source of truth and serializes turns per conversation so a retry cannot race a late original completion.
...
| Event          | Payload                                                 | When                                                                                                                                   |
| -------------- | ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `user_message` | `{ message }`                                           | Newly persisted user row (camelCase fields) — first event on a normal send; omitted for `retryLastTurn` because the row already exists |
