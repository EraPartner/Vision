...
Canonical transaction and planned-transaction writes require `account_id`; their response
`bank_account` label is projected from `accounts.name`. The orthogonal flag columns
...
Statement readings are created through the currency-scoped endpoint below, not account creation.
Legacy scalar statement fields are rejected.
...
Explicit `null` **clears** a nullable metadata field (`display_name`, `institution`, or
`funding_account_id`); an omitted key leaves the field untouched. Legacy scalar statement fields
are rejected. Use the per-currency statement-balance endpoints below.
...
reading is stored only in `account_statement_balances`.
...
> [!info] Account rename is identity-safe
> Transaction and planned-transaction responses project the label from `accounts.name` through
> `account_id`. Renaming an account therefore updates its displayed label without rewriting owned
> ledger rows. See [[docs/adr/088-account-entity|ADR-088 addendum]].
...
source is repointed to the survivor — `transactions.account_id`, `planned_transactions.account_id`, portfolio lots
...
response) and clears the survivor's now-invalidated statement-reading collection (reversible —
re-reconcile with a fresh statement). Historical
...
The list response exposes `statement_balances` as the authoritative per-currency series and
derives `drift` from the selected reading. When that statement is set and `drift` is non-zero,
...
The API defines `drift` as `selected statement reading − reconcilable_balance`, not against the
