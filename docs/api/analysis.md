...
Saved inputs contain `name`, `workspace`, `querySpec`, `parameters`, `charts`, `sourceReferences`,
and `refreshMode`. `PUT` never edits a definition row. It inserts the next version and advances the
library pointer. Refresh failures are stored without clearing `lastSuccessfulRunId` or its result.
