...
- **`uncategorised=true` honours the full filter set (fixed 2026-08-13).** `recipient_group_id`, `tags`, `transaction_type`, `amount_min`, `amount_max` (and `amount_signed`) were parsed by the route but dropped by `getUncategorisedWithCount`, so both the listed rows and `total` were computed over a wider set than the request named — a tag- or amount-filtered queue answered with every uncategorised row and an unfiltered count. All of them now narrow **both** halves, built by the same `buildTransactionWhere` the main list uses. Two consequences worth knowing: `recipient_id` now resolves aliases on the rows as it always did on the total (the queue and its count can no longer disagree), and `category_ids` joins `category_id`/`search`/`transaction_id` as **total-only** — a category filter cannot narrow a set defined by having no effective category. The queue itself is unchanged: active rows whose 3-level effective category is NULL, regardless of `active=false` ([[apps/node-backend/src/repositories/transactionRepository.js]]).
