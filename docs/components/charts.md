...
### Period Selector Semantics (2026-08-23)

`ChartPeriodSelector` renders each period as a native button with `aria-pressed` on the active
choice. It deliberately does not expose `tablist` or `tab` roles: the control switches one chart's
data window and does not own discrete `tabpanel` elements or implement the tabs arrow-key pattern.
