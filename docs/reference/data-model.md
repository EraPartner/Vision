...
| `bank_account`              | TEXT          | OUT-OF-BAND COMPATIBILITY ONLY                                       | Absent from the maintained live database after the 2026-09-08 ADR-088 contract operation. Fresh Alembic `head` retains it until that manual operation; contract-phase application code does not read or write it.                                                                                                                                                          |
| `account_id`                | INTEGER       | FK → accounts ON DELETE RESTRICT, NULLABLE                           | Canonical owning account (ADR-088, migration 0050). API responses derive their compatibility `bank_account` label from `accounts.name`.                                                                                                                                                                                                                                    |
...
| `source_record_hash`        | CHAR(64)      | NULLABLE                                                             | Byte-sensitive SHA-256 provenance for the retained logical CSV record; deliberately non-unique (migration 0103, ADR-134)                                                                                                                                                                                                                                                   |
| `dedup_fingerprint`         | CHAR(64)      | UNIQUE with version when non-null                                    | Provider-neutral occurrence fingerprint for new imports (migration 0103, ADR-134)                                                                                                                                                                                                                                                                                          |
...
> - **ADR-088 manual contract** (APPLIED TO THE MAINTAINED LIVE DATABASE 2026-09-08): `alembic/manual/contract_drop_bank_account/up.sql` removes both canonical-table `bank_account` columns and the dual-write trigger under a race-free parity guard. The coupled source accepts the legacy API field only as account-resolution input and persists `account_id`. Fresh Alembic-only databases retain the compatibility schema until this operation is run. `down.sql` restores it; use both only in the documented stopped-writer maintenance workflow.
...
`account_liquidity_class`, `account_tax_wrapper`, `account_owner`.
...
> Every install has **one flat `investments` table** holding every asset class — there is no base/child inheritance and no `stock_investments`/`etf_investments`/… child tables. This flat shape is the **canonical** schema per [[docs/adr/109-flat-investments-schema-canonical|ADR-109]], which **supersedes** [[docs/adr/004-postgresql-table-inheritance|ADR-004]]. Fresh installs get it from the `0001` baseline; legacy installs that carried the ADR-004 table-inheritance shape are converted by the guarded migration `0087_flat_investments_conversion`. Converted installs retain renamed `legacy_inh_*` rollback copies until migration 0087 passes the PostgreSQL 18 fixture gate, that exact installation completes a 30-day incident-free soak, and an operator verifies a restorable logical backup before running `alembic/manual/drop_adr109_legacy_relations/`. Asset-class-specific columns are NULL when not applicable.
...
| `source_record_hash`         | CHAR(64)           | NULLABLE                                                   | Byte-sensitive, non-unique source-record provenance (migration 0103, ADR-134)                                                                                                                                                                                                                   |
| `dedup_fingerprint`          | CHAR(64)           | UNIQUE with version when non-null                          | Occurrence-aware duplicate fingerprint for imported lots                                                                                                                                                                                                                                        |
| `dedup_fingerprint_version`  | SMALLINT           | NULLABLE, paired with fingerprint                          | Fingerprint algorithm version                                                                                                                                                                                                                                                                   |
| `is_recurring`               | BOOLEAN            | NOT NULL, DEFAULT false                                    | Whether the transaction repeats.                                                                                                                                                                                                                                                                |
...
| `tx_hash`                    | TEXT          | NULLABLE                                                   | Legacy compatibility hash; new rows copy `dedup_fingerprint`              |
| `source_transaction_id`      | TEXT          | NULLABLE                                                   | Immutable provider transaction ID when the adapter supplies one           |
| `source_account_identity`    | TEXT          | NULLABLE                                                   | Provider account identity used before destination-account fallback        |
...
| `config_json` | JSONB       | NOT NULL                          | Column mapping JSONB; shape differs by kind. Portfolio configs use the frontend camelCase mapping fields and may include optional `accountId`, the file-level broker destination; no schema migration is needed for this JSON addition. |
| `created_at`  | TIMESTAMPTZ | NOT NULL, DEFAULT NOW()           | Creation timestamp                                                                                                                                                                                                                      |
...
## Smart Insights persistence (migration 0101)

`insight_dismissals` is the authoritative suppression store. Subscription rows are unique by
finding kind and recipient. Category-outlier rows are unique by category and month and store the
server-derived deviation used by the 14-day worsening rule. Recipient and category foreign keys
cascade so obsolete identities do not leave permanent preferences.

`insight_digest_state` is a singleton versioned count projection. Statement triggers on
transactions, categories, recipients, planned transactions, and dismissals increment its dirty
version. A refresh updates the count only when its captured version still matches.

`insight_cash_projections` stores the latest month-end net-cashflow observation per month,
currency, and forecast method. The service compares like with like, upserts after successful
detection, and retains the current and previous two calendar months. All three tables are included
in PostgreSQL backups automatically.
