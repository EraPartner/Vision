...
## Research (11 endpoints) — ADR-079

Provider-agnostic research surface mounted at `/api/research` under `marketRateLimiter`. Each endpoint maps to one data type in the research aggregation layer (capability map → quota governor → cache → race-to-first provider). All responses carry `meta.provider` (which provider answered, or `null`) and `meta.source` (`'cache'` | `'live'` | `'unavailable'`). Yahoo Finance is the only adapter wired today; Twelve Data / Finnhub / FMP / Alpha Vantage are defined in the capability map and light up when their API key is provisioned in `.env.local`. See [[docs/api/research|Research API]] and [[docs/features/research|Research Feature]].
...
| GET | `/api/research/chart` | Historical chart points; `range` default `1mo` (1d/5d/1mo/3mo/6mo/1y/2y/5y/max) | — | [[docs/api/research\|Research]] |
| GET | `/api/research/fundamentals` | Fundamentals snapshot (P/E, EPS, market cap, …); 24 h cache | — | [[docs/api/research\|Research]] |
...
## Aggregations (14 endpoints) — Phase 2 / Phase 6 / Phase 7 / Phase 10 / Phase D / Phase E / Phase 9 / Phase C / Phase G
