...
>
> **2026-08-22 — import batch/row OpenAPI path types corrected (BREAKING schema correction, no runtime or route change; operation count unchanged at 212):** all `:id` / `:rowId` parameters on `/api/import/batches/*` and `/api/portfolio/import/batches/*` are now published as `integer` / `int64`, range 1..`Number.MAX_SAFE_INTEGER`, instead of `string`. This matches the shared runtime validator and shipped frontend callers. Generated TypeScript path parameters therefore change from `string` to `number`. External clients generated from the former schema may need a source-type update, but accepted HTTP requests and response payloads are unchanged. The `ImportBatch.id` and `PortfolioImportBatch.id` fields in batch list/detail responses remain strings because those repositories return raw node-postgres `BIGINT` values.
>
> **2026-08-26 — report exclusion IDs bounded at the route edge (BREAKING only for out-of-range input; operation count unchanged at 213):** `excludedCategoryIds` and `excludedRecipientIds` on all three report POST bodies now reject values above PostgreSQL `int4` maximum with `400 VALIDATION_ERROR`, before report generation reaches the SQL exclusion builder. `openapi.yaml` now publishes both arrays as positive `int32` IDs. Shipped callers already send database IDs in this range; valid report requests are unchanged.
>
> **2026-08-11 — path-param id validation tightened (BREAKING, no route changes; operation count unchanged at 212):** every integer `:id` / `:patternId` / `:accountId` / `:txnId` path param now accepts **only** a plain base-10 integer in 1..2³¹−1. Previously `validateId` used `parseInt`, so `"12abc"` and `"12.5"` resolved to id **12** and `"1e3"` to id **1** — a malformed id silently addressed a record the client never named (e.g. `DELETE /api/research/mappings/12abc` deleted mapping 12). Such requests now return **400 `VALIDATION_ERROR`** (`"<field> must be a positive integer"`) instead of acting on a coerced id. Affects the 13 routers on `validateIdParam`/`validateIntParam`: Accounts, Attachments, Categories, Investments, Planned Transactions, Recipients (+ Recipient Bank Accounts), Research (`DELETE /mappings/:id`), Saved Charts, Splits, Tags, Transactions, Watchlist. Breaking **only** for clients that were sending malformed ids — a plain integer behaves exactly as before, and `openapi.yaml` already typed these params `integer` (now annotated `format: int32, minimum: 1`), so the implementation moved _onto_ the published contract rather than away from it. Import batch/row ids (`/api/import/batches/*`, `/api/portfolio/import/batches/*`) were unaffected by _this_ change — they used a separate validator that already rejected `"12abc"` — but see the follow-up below, which converged them.
>
...
>
> **2026-08-11 — id-array _query_ params and the AI-chat tool parser converged too (BREAKING for malformed ids, no route changes; operation count unchanged at 212):** the last two id parsers, closing the class.
>
...
>
> **2026-08-11 — the last six operations with no `:id` middleware, plus two unvalidated body FKs (BREAKING for malformed ids, no route changes; operation count unchanged at 212):** closes the id-parser class.
>
> 1. **`PATCH`/`DELETE /api/import/parsers/:id` and `/api/portfolio/import/parsers/:id`** carried **no `validateIdParam`** and a `parseInt` guarded only by `Number.isNaN`. `DELETE /api/import/parsers/12abc` returned **`204` having deleted parser 12** — an irreversible delete of a record the caller never named, reported as success. `12.5` hit 12, `1e3` hit 1, `-1`/`0` reached the repository.
> 2. **`PATCH`/`DELETE /api/investments/transactions/:txnId`** were the same story with `requireTxnId`: `DELETE /transactions/12abc` returned **`204` having hard-deleted transaction 12**. These six were the only numeric-id operations in the app without the middleware; a sweep of every `:param` route found no others (`/api/ai/conversations/:id` is a UUID, `:table`/`:provider`/`:key` are strings, the import batch/row ids go through `coercedIdSchema`).
> 3. **`account_id`** on `POST /api/investments/:id/transactions` and `PATCH /api/investments/transactions/:txnId` was a bare `Number()` with no integer check at all — `'1e3'` booked the lot against account **1000**, `'0x10'` against 16, `true` against 1, `[7]` against 7, all `201`. The PATCH forwarded it raw. **`category_id`** on `POST /api/transactions` had no guard whatsoever and 500'd on a cast error instead.
> 4. **`investment_ids`** on `GET /api/investments/transactions` was `parseInt` + `filter(Number.isInteger)` at both the controller and the repository: `?investment_ids=5,12abc` read investments 5 **and** 12 with a `200`. Read-only, hence lowest severity of the set.
>
> `openapi.yaml` changes are documentation-only: this operation's filter was published as `investment_id` (singular, optional) though the route has only ever read `investment_ids` and 400s without it — now corrected, along with `account_id` on the portfolio-transaction create body, which was accepted but undocumented. No operation, path or method added or removed. No shipped caller breaks: every affected id is typed `number` in the API clients and taken from server-supplied rows. See [[docs/security/input-validation#validateIntParam|Input Validation]].
>
...
>
> 1. **Comma-separated id lists** — `category_ids` on `GET /api/transactions` and both export endpoints, `account_ids` on both export endpoints — were `.split(',').map(parseInt).filter(isFinite && > 0)` and had _both_ failure modes. **Retarget:** `?category_ids=5,12abc` filtered by categories 5 **and 12**; `?account_ids=12abc` exported account **12**. **Widen:** an all-bad list parsed to `[]`, which the caller mapped back to "no filter", so `?account_ids=abc` emitted no account predicate at all and `GET /export/csv` streamed **every account's transactions** into the downloaded file, 200 and all. They now go through `parseIdListQueryParam` → `validateIntArray` → `validateId`; a bad element is **400 `VALIDATION_ERROR`** (`"<field> contains invalid value: <value>"`). `account_ids` is validated in full _before_ `EXPORT_MAX_LIST_SIZE` caps it.
...
> 3. **`POST /api/transactions/transfers`** (`aId`/`bId`) was bare `parseInt` guarded only by `Number.isInteger`, so `"12abc"` stamped transaction **12** as one leg of a transfer pair — a wrong-record _write_ — and an id past `int4` passed the guard and 500'd at the column. Both now go through `validateId`.
>
...
| GET, POST, DELETE | `/api/ai-research/disclosures/*`             | Preview exact/tokenized payloads, run optional AgentCloak preflight, and govern digest-bound egress | 600 req/min         | [[docs/api/ai-research\|AI Research]] |
...
| POST   | `/api/transactions/bulk-delete`          | Hard-delete by `ids` (≤500) or `filter` plus required confirmed `expected_count` (≤5000); response distinguishes requested, matched, and deleted counts (breaking filter request change 2026-08-27)                                                                                                                                                                                                                                                                                        | 30 req/min | [[docs/features/bulk-actions\|Bulk Actions]] |
| POST   | `/api/transactions/bulk-update`          | Apply category/recipient/active update; filter mode requires confirmed `expected_count`, and response distinguishes requested, matched, and updated counts (breaking filter request change 2026-08-27)                                                                                                                                                                                                                                                                                     | 30 req/min | [[docs/features/bulk-actions\|Bulk Actions]] |
| POST   | `/api/transactions/bulk-export`          | Stream CSV/NDJSON from one repeatable-read snapshot; filter mode requires confirmed `expected_count`; `X-Exported-Count` is the exact streamed row count (breaking filter request change 2026-08-27)                                                                                                                                                                                                                                                                                       | 30 req/min | [[docs/features/bulk-actions\|Bulk Actions]] |
...
| Method | Path                                                    | Description                                                                  | Rate Limit | Doc                           |
| ------ | ------------------------------------------------------- | ---------------------------------------------------------------------------- | ---------- | ----------------------------- |
| POST   | `/api/import/csv`                                       | Import CSV; `bank_name` is a required multipart body field                   | —          | [[docs/api/imports\|Imports]] |
| POST   | `/api/import/csv/custom`                                | Import with custom mapping                                                   | —          | [[docs/api/imports\|Imports]] |
| POST   | `/api/import/csv/stream`                                | SSE streaming import; `bank_name` is a required multipart body field         | —          | [[docs/api/imports\|Imports]] |
| POST   | `/api/import/recipients`                                | Bulk import recipients; options are multipart body fields                    | —          | [[docs/api/imports\|Imports]] |
| POST   | `/api/import/categories`                                | Bulk import categories; options are multipart body fields                    | —          | [[docs/api/imports\|Imports]] |
...
| ------------------------------ | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `backup:run`                   | `(destDir: string, frontendStateJson?: string \| null)` | `Promise<{ success: boolean; file?: string; encrypted?: boolean; cleanupRemoved?: number; warning?: string; error?: string }>`             | Create `.visionbak` bundle; optionally encrypt to `.visionbak.enc`. Includes DB dump, attachments tree, and serialized frontend state (theme, dismissed toasts, etc.).                                                                                |
| `backup:restore`               | `(bundlePath: string, opts?: { passphrase?: string })`  | `Promise<{ success: boolean; file?: string; frontendState?: { keys: Record<string, string> } \| null; warning?: string; error?: string }>` | Restore from `.visionbak` or `.visionbak.enc` bundle. Validates schema head, drops DB, loads SQL, atomically swaps attachments, returns frontend state for localStorage hydration. Blocks if bundle schema is newer (user must upgrade Vision first). |
| `backup:select-file`           | `()`                                                    | `Promise<string \| null>`                                                                                                                  | Show native file picker dialog; returns the selected backup path or null when cancelled.                                                                                                                                                              |
| `backup:select-dir`            | `()`                                                    | `Promise<string \| null>`                                                                                                                  | Show native folder picker dialog; returns the selected directory or null when cancelled.                                                                                                                                                              |
| `backup:is-encrypted`          | `(bundlePath: string)`                                  | `Promise<boolean>`                                                                                                                         | Detect encryption for a path previously selected through the file picker.                                                                                                                                                                             |
| `backup:save-settings`         | `({ backupDir, backupOnQuit })`                         | `Promise<{ success: boolean; error?: string }>`                                                                                            | Persist backup settings (directory path, auto-backup-on-quit flag) to `settings.json`.                                                                                                                                                                |
...
**Frontend Wrappers:** `apps/frontend/src/lib/api/electron.ts` consumes the shared bridge types and provides async wrappers for all IPC handlers.

**Integration:** `apps/frontend/src/features/settings/sections/BackupSection.tsx` uses these handlers for UI-driven backup/restore, directory/file selection, and passphrase management.
