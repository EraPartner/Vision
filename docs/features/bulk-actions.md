...
// Selection (one of)
{ "ids": [1, 2, 3] }
{ "filter": { "search": "cafe", "start_date": "2026-01-01" }, "expected_count": 42 }
...
```jsonc
// bulk-delete
{ "deleted": 3, "requested": 4, "matched": 4 }

// bulk-update
{ "updated": 5, "requested": 5, "matched": 5 }

// bulk-export → streamed CSV/NDJSON body (no JSON envelope)
// response header: X-Exported-Count: 5
```

`requested` is the explicit ID count or the filter count the user confirmed. `matched` is the number of IDs resolved immediately before the write. Delete and update responses therefore distinguish selection drift (`requested != matched`) from rows that disappeared during the write (`matched != deleted|updated`). Bulk export resolves the selection, counts existing rows, and streams them inside one repeatable-read, read-only database snapshot. `X-Exported-Count` is therefore the exact row count in the completed file. CORS makes the header readable by the browser. A mid-stream database failure destroys the response instead of completing a partial download as success.
...
3. Clicking promotes `selectionMode = 'filter'`. The id list is no longer authoritative; the request body becomes `{ filter, expected_count, fields|format }`, where `expected_count` is the count shown at confirmation time.
