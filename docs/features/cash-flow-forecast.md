...
```json
{
  ...
  "amount": -1200.0, // Amount (positive for income, negative for expense)
...
```json
{
  ...
  "bands": {
    "p10": [{ "date": "2026-04-25", "value": 30.5 }],
...
Imports continue to invalidate Monte Carlo caches synchronously because stale
cash-flow projections are a correctness failure, not merely a performance
trade-off. The methods endpoint also keeps its bounded 12-month walk-forward
backtest by default because the diagnostics are part of method comparison.
Revisit yielding, deferred invalidation, or a shorter default only if measured
forecast p95 exceeds 500 ms, or cache invalidation adds at least 100 ms and 10%
to the measured import-completion p95. Any change must preserve freshness and
the documented diagnostics default.
