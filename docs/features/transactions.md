...
- The uncategorised queue applies the caller's **full** filter set. `recipientGroupId`, `categoryIds`, `tagSlugs`, `transactionType`, `amountMin`, `amountMax` and `amountSigned` were previously parsed by the route and then dropped by the repository, so filtering the queue by tag or amount range narrowed neither the rows nor the total. Both halves now go through the same `buildTransactionWhere` the main list uses, so queue and list cannot apply a filter two different ways. Consequences worth knowing: `recipientId` on the rows half is now alias-inclusive (matching the main list and the queue's own total, where it already was), and `transactionId`, `categoryId`, `categoryIds` and `search` remain deliberately total-only — a category filter cannot narrow a set defined by having no effective category. `total` still counts without the uncategorised predicate, so it is not the size of the queue.
