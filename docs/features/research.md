...
Research Compare stores its ordered, deduplicated symbol list in repeated singular `symbol` parameters (maximum six), plus `range`, `sort`, and the existing `tab`. Symbols are uppercased and limited to provider-compatible characters and 32 characters. The default range and sort are omitted; the existing tab helper writes the selected tab, including its default. Unknown symbols, ranges, and sort metrics fall back safely, and edits preserve unrelated query parameters with replace-writes.
...
| **D — Screening / fundamentals**             | Side-by-side fundamentals comparison; per-symbol heuristic scorecard (0–100 grade); extended fundamentals fields across all three capable adapters; screening is _selected-symbol_ only (no universe scan; free-tier quotas can't support one)                                                                                                                                                                                        | **Shipped** — backend + per-symbol fundamentals tab with scorecard panel + the Compare page's **Fundamentals** tab (Health column + debtToEquity/currentRatio/revenueGrowth/fcfYield, with each metric cell tinted green/amber/red by its scorecard severity); `GET /api/research/scorecard`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
removed from browser history while unrelated query parameters are preserved. Share links disclose
their ticker and macro-series choices to anyone who receives the URL.
...
- **[[docs/features/market-lookup|Market Lookup]]** (`/api/market/*`) — the consolidated security-detail surface at `/research/market`. The Research API and the market routes share the `marketRateLimiter` but remain distinct route files. Market Lookup now hosts the tabbed Details card (Fundamentals / Analyst / News via the research aggregator endpoints) and the Map-provider dialog, and is the deep-link target from all in-app navigation (heat-map tiles, home search, watchlist tiles). The standalone `ResearchSymbolPage` and its `/research/symbol/:symbol` alias are retired. The ⌘K command palette routes here: typing a bare ticker or `$`-cashtag shows an inline price card (symbol + price + percent change) as the first palette result, and pressing Enter opens `/research/market?symbol=<symbol>`.
...
- **`SymbolSearchResultItem`** — the canonical result row (monospaced ticker, company name, asset-type badge, exchange label). Research Home and Market Lookup pass an exact destination so results are real links; Compare, Chart Builder, and Add to Watchlist retain button selection mode.

Market Overview heat tiles, Research Home benchmark/watchlist tiles, View All, and the empty-watchlist action expose real router links. This preserves href preview, keyboard activation, and modified or middle click without changing their visual treatment.

- **`ResearchRangeSelector`** — adapts the shared `ChartPeriodSelector` to research range/interval
  options. Market Lookup, Compare, Chart Builder, and the watchlist chart dialog share the same
  segmented control, 40px touch targets, narrow-width overflow behavior, and localized labels.
