...
- [[docs/features/portfolio-import\|Portfolio CSV Import]] - Bulk import of brokerage/exchange trades from CSV files (ADR-078)
...
- [[docs/features/application-updates\|Application Updates]] - Native, source, and development update behavior with backup-before-update and SHA-256 verification
...
- Field rename: `federalPITTotal` → `federalPITBeforeExemption`; the alias was retired by migration 0106 and ADR-135
...
- The Belgian-tax Zustand slice owns `viewedYear` (transient), `snapshots` (persisted JSONB), and helper actions (`profileForYear`, `calculationForYear`, `snapshotExistsForYear`, `isViewingHistorical`); the former context is now a hydration compatibility boundary (ADR-121)
