...
- **NEW (2026-05-03):** Async factory mock for `LanguageHydration` with synchronous locale dictionary import (avoids module-loading complexity)
...
- `vi.mock("@/stores/hydration/LanguageHydration")` with async factory importing `@/locales/en` for synchronous `t()` function
...
- `vi.mock("@/stores/hydration/LanguageHydration")` with async factory importing `@/locales/en` for synchronous translations
