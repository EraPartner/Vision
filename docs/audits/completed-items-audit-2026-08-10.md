...
| low | `I566` | 4875 | Build context is ~1.2 GB of irrelevant files | venv/, .claude/, .obsidian/, .github/, and TODO.md — all explicitly named in the item's own text as contributing to the ~1.2GB bloat — are still absent from .dockerignore. The bulk of the bloat (packaging/electron/node_modules + dist) is fixed, so this is a real but low-blast-radiu… |
