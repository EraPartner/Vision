...
| PKG-04 | d3-array `sum` in chart data prep | `components/charts/AreaChart.tsx:288,291` and `components/charts/StackedBarChart.tsx:185`: `series.reduce((s, x) => s + (x.accessor(d) ?? 0), 0)` → `sum(series, s => s.accessor(d) ?? 0)`. The `?? 0` already guarantees numbers, so order and float result are identical. d3-array is already a dependency and already imported in these areas. DONE: both AreaChart reduces + StackedBarChart:185 swapped to `sum()` (StackedBarChart gained the d3-array import). Behavior-identical under the app's accessor contract (returns `number \| null \| undefined`, never NaN). | DONE (#105) |
| PKG-05 | d3-array `min`/`max` for chart domains | `AreaChart.tsx:274,309,310` (file already imports `min, max` from d3-array at line 19 and doesn't use them here), `LineChart.tsx:200,224,225`, `ComposedChart.tsx:263,283,284`, `Sparkline.tsx:59,60`. `Math.min(...arr)` returns NaN if any element is NaN and can overflow the stack on very large arrays; d3 `min` SKIPS NaN/null and returns `undefined` on empty. **Only swap where the input array is provably finite** (verify each source); on gappy data the rendered domain would change — that site then belongs to the do-not list. Where safe, the swap also fixes the latent spread-size hazard. Do NOT touch `BarChart.tsx:338` (`Math.max(0, ...)` — the 0 floor is intentional; see do-not list). DONE: converted the three **finite-filtered value-domain** sites — `AreaChart.tsx`, `LineChart.tsx`, `ComposedChart.tsx` `y`-domain `min(values)`/`max(values)` (each `values` array is built with a `Number.isFinite` filter, so d3 min/max equal `Math.min/max(...)` exactly while removing the large-array spread hazard). The x-domain `nums` spreads (`AreaChart:275`, `LineChart:201`, `ComposedChart:263`) were **also converted after verifying call-site finiteness**: the only live numeric-x caller is `ForecastInner → LineChart`, whose accessor is `dayNum = i+1` (`forecastMerge.ts:187`), always a finite positive integer; the AreaChart/ComposedChart numeric-x branches have no `xIsDate={false}` caller at all (dead paths). So the swap is behavior-identical and closes the spread hazard there too. `Sparkline.tsx:59-60` stays hand-rolled — its `data` prop has several callers whose series aren't all finite-provable, so it remains on the do-not list. `StackedBarChart:203` (`Math.max(1, ...totals)`) keeps its intentional 1-floor. | DONE (#105) |
...
All batches landed in PR #105. Every Tier-1/Tier-2/dependency row is `DONE`
except **PKG-13**, resolved `WONTFIX (regression)` — its per-element localStorage
filter is load-bearing and pinned, and the suggested `z.array(...).catch([])` is
all-or-nothing, so migrating would drop valid entries (see the row for the exact
failing pin). Two rows carry implementation findings worth reading before any
follow-up round: **PKG-06** (only `main.js` could move to `node:timers/promises`;
the other two sites are pinned by `vi.useFakeTimers()`, which cannot fake
timers/promises) and **PKG-05** (chart y-domains and the numeric x-domains were swapped after
verifying call-site finiteness; only Sparkline stays on the do-not list).
