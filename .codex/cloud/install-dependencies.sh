...
python_fingerprint="$({
  python3 --version
  cloud_fingerprint 1 config/requirements.txt
} | cloud_hash_stream)"
...
if cloud_marker_matches "$python_marker" "$python_fingerprint" &&
  ...
  cloud_run_step 'Install Python dependencies' \
    cloud_run_with_timeout 600s env \
      PIP_DISABLE_PIP_VERSION_CHECK=1 \
      PIP_DEFAULT_TIMEOUT=30 \
      PIP_RETRIES=3 \
      venv/bin/python -m pip install --no-input -r config/requirements.txt
...
if cloud_marker_matches "$bun_marker" "$bun_fingerprint" &&
  ...
  cloud_run_step 'Install Bun workspace dependencies' \
    cloud_run_with_timeout 900s env \
