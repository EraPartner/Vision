...
function fixture(rootVersion = '1.0.2', electronVersion = rootVersion) {
...
test('updates both enforced version sources and preserves other fields', (t) => {
  const root = fixture();
  ...
  const electron = JSON.parse(fs.readFileSync(path.join(root, 'packaging/electron/package.json'), 'utf8'));
  assert.deepEqual(app, { name: 'vision', version: '2.3.4', private: true });
  assert.deepEqual(electron, { name: 'vision-desktop', version: '2.3.4', main: 'main.js' });
});

test('rejects invalid versions without changing either manifest', (t) => {
  const root = fixture();
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const before = manifestTexts(root);

  for (const invalid of ['v2.3.4', '2.3.4-beta.1', '02.3.4', '2.03.4', '2.3.04']) {
    assert.throws(() => bumpVersions(root, invalid), /must use canonical x\.y\.z/);
    assert.deepEqual(manifestTexts(root), before);
  }
});

test('rejects a pre-existing mismatch instead of concealing it', (t) => {
  const root = fixture('1.0.2', '1.0.1');
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));

  assert.throws(() => bumpVersions(root, '2.0.0'), /existing version mismatch/);
  assert.equal(JSON.parse(fs.readFileSync(path.join(root, 'package.json'), 'utf8')).version, '1.0.2');
  assert.equal(
    JSON.parse(fs.readFileSync(path.join(root, 'packaging/electron/package.json'), 'utf8')).version,
    '1.0.1',
  );
});

test('rolls back both manifests if the second staged replacement fails', (t) => {
  const root = fixture();
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const before = manifestTexts(root);
  let renameCount = 0;
  const failingFileSystem = {
    ...
    renameSync(from, to) {
      renameCount += 1;
      if (renameCount === 4) {
        const error = new Error('injected second replacement failure');
        error.code = 'EACCES';
        throw error;
      }
  ...
  assert.deepEqual(
    fs.readdirSync(path.join(root, 'packaging/electron')).filter((name) =>
      name.includes('.version-bump-'),
    ),
    [],
  );
});
