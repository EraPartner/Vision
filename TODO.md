...
- **Wave R2 (planned):** Electron native surfaces — menu strings vs in-app language, icon/dock/menubar color language, about panel fonts; TableDataEditorPage composition audit.
- **Wave R3 (planned):** nl.json read-through of the never-eyeballed namespaces (tax, research, portfolio, statsPage, customChart, addInv, rebalance, performance, export, admin, cashflow, insights, owesPage, aiChat, dbEditor, recipients*, invDetail, market, networth, exchangeRates) — register/calque/terminology lens.
- **Wave R4 (planned):** post-2026-07-03 diff sweep (utils/safeHref.ts + its 5 consumers, PlannedPaymentForm default-date change, VirtualDataTable a11y fixes) for UX regressions/gaps introduced after the audits.
