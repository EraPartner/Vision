...
- [ ] **Tax Overview page shows "set up your tax profile" on fetch failure, for users who haven't yet saved a tax profile** ⏫ 🔎 partial-#82 2026-07-11 (empty-state now suppressed via !stats.isError so the setup prompt no longer shows on error, but no distinct error banner is rendered — falls through to normal zeroed content) *(precision: doesn't affect users who already completed tax-profile setup)*
  - ↪ _from: Codebase audit 2026-06-30 · Correctness — Frontend_
