...
- [ ] **Three sign dialects for the same minus: ASCII hyphen "-" on the most-viewed money surfaces, true "−" on a few, and `Money`'s own Intl `signDisplay` used exactly once** ⏫ 🔎 partial-08866f5 2026-07-11 (swept TransactionsTable/OwesPage/DashboardPage to `<Money signed>`; the `TransactionQuickLook` 4xl hero glyph and `TransactionInfoDialog`'s string-template value still hand-concatenate — convert those two next)
  - ↪ _from: Design authenticity 2026-07-03 · Wave S3_
