...
**Planned but not yet run when this pass stopped (pick up here):** Wave F2 — backend residues: `transactionExport.js` streamer internals, `getOwedSummary`/`attachmentRepository.listByTransaction`/`savedChartsRepository.getAll` SQL boundedness, perf-lens read of `controllers/`, `jobs/`, `startup/`, `integrations/` internals · Wave F3 — frontend residues: search-box debounce cadence, `PlannedPaymentForm` keystroke behavior, `useUpcomingPlannedPayments` dismissed-keys snapshot, AI-chat frontend components (`ToolResultCard` renderAs/Recharts config, conversation switching), Electron vibrancy-tier × glass interaction (Wave A residue) · Wave F4 — adversarial fresh-ground sweep of anything not on the exclusion map.
