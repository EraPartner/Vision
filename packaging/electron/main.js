...
const { parseEnvKeys, generateFreshEnvContents, ensureAppRoleEnv } = require('./envConfig');
...
//
// The content model (fresh generation + the single-role → two-role upgrade)
// lives in ./envConfig.js so it can be unit-tested without Electron. The
// generated passwords never leave this file (mode 0600) and are never logged.
...
async function ensureEnv(workDir) {
  ...
  // Least-privilege DB roles (security backlog). Fresh installs are already
  // two-role via generateFreshEnvContents(); this upgrades an EXISTING .env
  // (the only thing an already-installed desktop app has) to the same layout.
  // Idempotent, and a no-op on any .env that already declares
  // DATABASE_URL_MIGRATIONS. The role itself is created by the backend's
  // runtime bootstrap on its next boot, which fails open to the privileged
  // role — so a partially-applied upgrade cannot stop the app from starting.
  truth = ensureAppRoleEnv(truth);
